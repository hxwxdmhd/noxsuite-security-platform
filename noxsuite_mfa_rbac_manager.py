...
class NoxSuiteMFAManager:
    ...
    def enable_totp_mfa(self, user_id: str) -> Dict:
        ...
        try:
            ...
            with pymysql.connect(self.db_path) as conn:
                ...
                user = conn.execute(
                    "SELECT username FROM mfa_users WHERE id = ?", (user_id,)
                ).fetchone()
                ...
                mfa_methods = json.dumps(
                    [MFAMethod.TOTP.value, MFAMethod.BACKUP_CODES.value]
                )
                conn.execute(
                    """
                    UPDATE mfa_users 
            ...
            qr_url = totp.provisioning_uri(
                name=user["username"], issuer_name=self.totp_issuer
            )
            ...
            with pymysql.connect(self.db_path) as conn:
                ...
                if not user or not user["mfa_secret"]:
                    self._log_mfa_attempt(
                        user_id, MFAMethod.TOTP.value, False, ip_address
                    )
                    return False
                ...
                if totp.verify(token, valid_window=2):
                    self._log_mfa_attempt(
                        user_id, MFAMethod.TOTP.value, True, ip_address
                    )
                    return True
                ...
                if user["backup_codes"]:
                    ...
                    if token.upper() in backup_codes:
                        ...
                        self._log_mfa_attempt(
                            user_id, MFAMethod.BACKUP_CODES.value, True, ip_address
                        )
                        return True
    ...
    def _log_mfa_attempt(
        self, user_id: str, method: str, success: bool, ip_address: str = None
    ):
        """Log MFA attempt"""
        try:
            with pymysql.connect(self.db_path) as conn:
    ...
    def get_mfa_status(self, user_id: str) -> Dict:
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                methods = json.loads(
                    user["mfa_methods"]) if user["mfa_methods"] else []
...
class NoxSuiteRBACManager:
    ...
    def check_permission(
        self, user_role: str, permission: str, resource: str = None
    ) -> bool:
    ...
    def revoke_permission(
        self, role: str, permission: str, resource: str = None
    ) -> bool:
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                if user["mfa_enabled"]:
                    ...
                    if not self.mfa_manager.verify_totp(
                        user["id"], mfa_token, ip_address
                    ):
...
def main():
    ...
    if "error" not in result:
        ...
        if "mfa_setup" in result:
            ...
            print(
                f"   Backup codes: {len(result['mfa_setup'].get('backup_codes', []))} generated"
            )
    else:
