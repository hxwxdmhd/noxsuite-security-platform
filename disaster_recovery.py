...
def create_emergency_admin():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    credentials_file = os.path.join(BACKUP_DIR, f"emergency_credentials_{timestamp}.json")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Disaster Recovery for NoxSuite MFA and RBAC")
    
    # Add subparsers
    subparsers = parser.add_subparsers(dest="command", help="Command to run")
    
    # Backup parser
    backup_parser = subparsers.add_parser("backup", help="Backup authentication and authorization data")
    backup_parser.add_argument("--type", choices=["rbac", "mfa", "all"], default="all", help="Type of data to backup")
    
    # Restore parser
    restore_parser = subparsers.add_parser("restore", help="Restore authentication and authorization data")
    restore_parser.add_argument("--type", choices=["rbac", "mfa", "all"], default="all", help="Type of data to restore")
    restore_parser.add_argument("--file", type=str, help="Backup file to restore from")
    
    # Reset MFA parser
    reset_parser = subparsers.add_parser("reset-mfa", help="Reset MFA for a user")
    reset_parser.add_argument("user_id", type=str, help="User ID to reset MFA for")
    
    # Emergency admin parser
    emergency_parser = subparsers.add_parser("emergency-admin", help="Create emergency admin access")
