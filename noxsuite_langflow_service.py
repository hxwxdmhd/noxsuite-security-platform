...
class NoxSuiteLangflowService:
    ...
    def start(self):
        ...
        try:
            logger.info(f"Starting NoxSuite Langflow Service on {self.host}:{self.port}")
            ...
            self.status = "running"
            logger.info(f"✅ NoxSuite Langflow Service started successfully on http://{self.host}:{self.port}")
