    ...
    async def get_system_context(self, force_refresh: bool = False) -> SystemContext:
        ...
        try:
            ...
            context = SystemContext(
                ...
                uptime_hours=round((datetime.now(
                ) - datetime.fromtimestamp(psutil.boot_time())).total_seconds() / 3600, 2),
            ...
            self.logger.info(
                f"System context gathered: {context.os_type.value} {context.os_version}")
            return context
        ...
        try:
            for partition in psutil.disk_partitions():
                try:
                    ...
                    disk_usage[partition.device] = round(
    ...
    async def generate_script(self, description: str, target_os: SystemOS,
        ...
        try:
            ...
            if best_template:
                ...
                self.logger.info(
                    f"Generated script using template: {best_template.name}")
    ...
    async def _analyze_health(self, metrics: SystemHealthMetrics):
        ...
        if metrics.cpu_usage_percent > 90:
            ...
            recommendations.append(
                "CPU usage is elevated - consider monitoring")
            health_score -= 5
        ...
        if metrics.process_count > 300:
            recommendations.append(
                "High number of processes - system may be slow")
            health_score -= 3
...
class TaskExecutionEngine:
    ...
    async def execute_task(self, task: Task, confirm: bool = True) -> Task:
        ...
        try:
            ...
            self.logger.info(
                f"Task completed: {task.name} ({'SUCCESS' if task.success else 'FAILED'})")
        ...
        for prerequisite in task.prerequisites:
            ...
            if prerequisite.startswith("os:"):
                ...
                if context.os_type.value != required_os.lower():
                    self.logger.error(
                        f"OS prerequisite not met: required {required_os}, got {context.os_type.value}")
                    return False
...
class SysAdminCopilot:
            ...
            if health_metrics.issues:
                for issue in health_metrics.issues:
                    if "CPU" in issue:
                        task = Task(
                            id=hashlib.md5(
                                f"cpu_analysis_{datetime.now()}".encode()).hexdigest()[:8],
    ...
    async def _create_task_from_template(self, template_key: str, description: str) -> Task:
        ...
        task = Task(
            id=hashlib.md5(
                f"{template_key}_{datetime.now()}".encode()).hexdigest()[:8],
    ...
    async def _generate_health_insights(self, metrics: SystemHealthMetrics,
        ...
        if metrics.cpu_usage_percent > 50:
            insights.append(
                f"CPU usage at {metrics.cpu_usage_percent:.1f}% - consider monitoring background processes")

        if metrics.memory_usage_percent > 75:
            insights.append(
                f"Memory usage at {metrics.memory_usage_percent:.1f}% - may benefit from cleanup or more RAM")

        # Uptime insights
        if metrics.uptime_hours > 24 * 7:  # 1 week
            insights.append(
                f"System uptime is {metrics.uptime_hours/24:.1f} days - consider restart for optimal performance")

        # Context-based insights
        if context.os_type == SystemOS.WINDOWS:
            if metrics.process_count > 150:
                insights.append(
                    "High process count detected - typical for Windows but may indicate resource usage")
        else:
            if metrics.process_count > 200:
                insights.append(
                    "High process count detected - may indicate heavy system load")

        # Disk space insights
        if metrics.disk_usage_percent > 85:
            insights.append(
                "Disk space is critically low - immediate cleanup recommended")
        elif metrics.disk_usage_percent > 70:
            insights.append(
                "Disk space usage is high - consider cleanup maintenance")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_types": {
                task_type.value: sum(
                    1 for t in self.task_history if t.task_type == task_type)
    ...
    def get_task_history(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        recent_tasks = self.task_history[-limit:
                                         ] if limit else self.task_history
...
async def main():
    ...
    if script_result['success']:
        ...
        print("=" * 40)
        print(script_result['script_content'][:200] + "..." if len(
            script_result['script_content']) > 200 else script_result['script_content'])
        print("=" * 40)
