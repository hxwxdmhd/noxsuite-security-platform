...
class DockerNetworkDiagnostics:
    ...
    def _test_port_availability(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Port availability checked: {len(available_ports)} available, {len(occupied_ports)} occupied")
        except Exception as e:
            self.results.append(NetworkDiagnostic(
                test_name="port_availability",
                success=False,
...
class RedisConnectionManager:
    ...
    def test_redis_connections(self) -> List[RedisConnectionResult]:
        ...
        for host, port in test_configs:
            ...
            if result.success:
                logger.info(
                    f"✅ Redis connection successful: {host}:{port} ({result.latency_ms:.2f}ms)")
            else:
                logger.warning(
                    f"❌ Redis connection failed: {host}:{port} - {result.error_message}")
    ...
    def get_best_redis_config(self) -> Optional[Tuple[str, int]]:
        ...
        # Sort by latency
        best_connection = min(successful_connections,
    ...
    def _start_redis_service(self) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker-compose", "-f", "docker-compose.dev-networking.yml",
                    "up", "-d", "redis-dev"],
                capture_output=True,
                text=True,
                cwd=os.getcwd()
...
def main():
    ...
    print(
        f"Redis Tests: {len([r for r in redis_results if r.success])}/{len(redis_results)} successful")
