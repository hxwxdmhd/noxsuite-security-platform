    ...
    async def discover_routers(self) -> List[RouterInfo]:
        ...
        for method in self.discovery_methods:
            try:
                ...
                logger.info(
                    f"Discovery method {method.__name__} found {len(routers)} routers")
...
class RouterRegistry:
    ...
    def load_registry(self):
        ...
        try:
            if self.config_file.exists():
                ...
                logger.info(
                    f"Loaded {len(self.routers)} routers from registry")
            else:
    ...
    def get_registry_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_discovery': max(
                (router.last_seen for router in self.routers.values()
                 if router.last_seen),
                default=None
            )
        }
