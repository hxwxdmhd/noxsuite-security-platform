...
class RouterHealthMonitor:
    ...
    def get_best_router(self, available_routers: List[str]) -> Optional[str]:
        ...
        healthy_routers = [
            r for r in available_routers if self.is_router_healthy(r)]
        ...
        healthy_routers.sort(key=lambda r: self.router_health.get(
            r, {}).get('average_response_time', float('inf')))
...
class TemporaryCredentialManager:
    ...
    def set_temporary_credential(self, router_id: str, username: str, password: str,
        ...
        self.credential_expires[router_id] = datetime.now(
        ) + timedelta(minutes=expires_in_minutes)

        logger.info(
            f"Set temporary credentials for router {router_id} (expires in {expires_in_minutes} minutes)")
    ...
    def get_credentials(self, router_id: str) -> Optional[Dict[str, str]]:
        ...
        if router_id in self.temp_credentials:
            if datetime.now() < self.credential_expires.get(router_id, datetime.now()):
                ...
                logger.info(
                    f"Temporary credentials for router {router_id} expired")
    ...
    async def execute_with_retry(self, func: Callable, *args, router_id: str = None, **kwargs) -> Any:
        ...
        for attempt in range(self.retry_config.max_attempts):
            try:
                ...
                if router_id:
                    self.health_monitor.update_router_health(
                        router_id, True, response_time)
                ...
                if attempt < self.retry_config.max_attempts - 1:
                    ...
                    logger.warning(
                        f"Attempt {attempt + 1} failed for router {router_id}: {e}. Retrying in {delay:.2f}s")
                    await asyncio.sleep(delay)
                else:
                    logger.error(
                        f"All {self.retry_config.max_attempts} attempts failed for router {router_id}: {e}")
            ...
            try:
                ...
                for router_id in router_list:
                    if router_id != best_router and self.health_monitor.is_router_healthy(router_id):
                        try:
                            logger.info(
                                f"Attempting failover to router {router_id}")
                            return await self.execute_with_retry(func, *args, router_id=router_id, **kwargs)
                        except Exception as fe:
                            logger.warning(
                                f"Failover to router {router_id} failed: {fe}")
                            continue
