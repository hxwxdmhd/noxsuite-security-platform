...
try:
    ...
    class HeimnetzCLI:
        ...
        def setup_commands(self):
            ...
            # AetherCore commands
            aethercore_parser = subparsers.add_parser(
                'aethercore', help='AetherCore management')
            aethercore_subparsers = aethercore_parser.add_subparsers(
                dest='aethercore_command')

            aethercore_subparsers.add_parser(
                'status', help='Show AetherCore status')
            aethercore_subparsers.add_parser(
                'health', help='Check AetherCore health')

            models_parser = aethercore_subparsers.add_parser(
                'models', help='Model management')
            models_subparsers = models_parser.add_subparsers(
                dest='models_command')

            models_subparsers.add_parser('list', help='List available models')

            load_parser = models_subparsers.add_parser(
                'load', help='Load a model')
            load_parser.add_argument('model_id', help='Model ID to load')

            unload_parser = models_subparsers.add_parser(
                'unload', help='Unload a model')
            unload_parser.add_argument('model_id', help='Model ID to unload')

            serve_parser = models_subparsers.add_parser(
                'serve', help='Serve inference request')
            ...
            contextforge_parser = subparsers.add_parser(
                'contextforge', help='ContextForge management')
            contextforge_subparsers = contextforge_parser.add_subparsers(
                dest='contextforge_command')

            contextforge_subparsers.add_parser(
                'status', help='Show ContextForge status')
            contextforge_subparsers.add_parser(
                'health', help='Check ContextForge health')

            test_parser = contextforge_subparsers.add_parser(
                'test', help='Run tests')
        ...
        def print_table(self, headers: List[str], rows: List[List[str]]):
            ...
            header_line = " | ".join(header.ljust(
                widths[i]) for i, header in enumerate(headers))
            print(header_line)
            print("-" * len(header_line))

            # Print rows
            for row in rows:
                row_line = " | ".join(str(cell).ljust(
                    widths[i]) for i, cell in enumerate(row))
                print(row_line)
        ...
        def system_health(self):
            ...
            if "error" in aethercore_health:
                ...
                print(
                    f"✅ AetherCore: {aethercore_health.get('status', 'unknown')}")

            # Check ContextForge
            contextforge_health = self.make_request(
                f"{CONTEXTFORGE_URL}/context/health")
            if "error" in contextforge_health:
                print(f"❌ ContextForge: {contextforge_health['error']}")
                all_healthy = False
            else:
                print(
                    f"✅ ContextForge: {contextforge_health.get('status', 'unknown')}")
        ...
        def aethercore_models_load(self, model_id: str):
            ...
            result = self.make_request(
                f"{AETHERCORE_URL}/models/{model_id}/load", method="POST")
            if "error" in result:
        ...
        def aethercore_models_unload(self, model_id: str):
            ...
            result = self.make_request(
                f"{AETHERCORE_URL}/models/{model_id}/unload", method="POST")
            if "error" in result:
        ...
        def aethercore_models_serve(self, model_id: str, text: str):
            ...
            result = self.make_request(
                f"{AETHERCORE_URL}/serve", method="POST", data=data)
            if "error" in result:
            ...
            if result.get("status") == "success":
                print(
                    f"✅ Inference completed in {result.get('processing_time_ms', 0):.2f}ms")
                ...
                print(
                    f"❌ Inference failed: {result.get('error', 'Unknown error')}")
