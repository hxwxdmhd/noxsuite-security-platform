    ...
    <script>
        ...
        setInterval(function() {
            ...
            ''',
                                          ...
                                          cache_hits=self.cache_manager.get(
                                              'cache_hits', 0)
                                          )
...
<body>
    <div class="container">
        ...
        <p style="margin-top: 20px;"><a href="/">← Back to Dashboard</a></p>
                ...
                ''',
                                              docker_environment=self.detector.is_docker,
                                              redis_configs_tested=len(
                                                  self.detector.redis_configs),
                                              redis_connected=self.cache_manager.redis_client is not None,
                                              best_redis_config=self.detector.best_redis_config,
                                              cache_type='redis' if self.cache_manager.redis_client else 'memory',
                                              timestamp=datetime.now().isoformat()
...
<body>
    ...
    def _start_background_tasks(self):
        ...
        def metrics_collector():
            while True:
                try:
                    # Update metrics
                    current_requests = self.cache_manager.get(
                        'total_requests', 0)
    ...
    def run(self, host='127.0.0.1', port=5003, debug=True):
        ...
        logger.info(
            f"Redis connected: {self.cache_manager.redis_client is not None}")
        logger.info(
            f"Cache type: {'Redis' if self.cache_manager.redis_client else 'Memory'}")
        logger.info("=" * 60)
