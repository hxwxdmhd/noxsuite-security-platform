...
try:
    ...
    from plugin_registry_db import PluginMetadata, PluginRegistryDatabase
...
class PluginDocumentationGenerator:
    ...
    def generate_plugin_documentation(self, plugin_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            if not manifest:
                raise ValueError(
                    f"No manifest found for plugin: {plugin_name}")

            # Load plugin metadata from registry
            plugin_metadata = self.registry_db.get_plugin(plugin_name)

            # Generate documentation sections
            sections = self._generate_documentation_sections(
                manifest, plugin_metadata)
            ...
            self.logger.error(
                f"Failed to generate documentation for {plugin_name}: {e}")
            raise
...
"""
        ...
        for plugin_name in os.listdir(self.plugin_dir):
            ...
            if os.path.isdir(plugin_path):
                try:
                    ...
                    self.logger.info(
                        f"Generated documentation for {plugin_name}")
                except Exception as e:
                    self.logger.error(
                        f"Failed to generate documentation for {plugin_name}: {e}")
    ...
    def save_documentation(self, plugin_name: str, documentation: Dict[str, str],
        ...
        self.logger.info(
            f"Documentation saved for {plugin_name} in {output_dir}")
