...
class WebRTCPeerManager:
    ...
    def __init__(self, config: WebRTCConfig):
        ...
        if not HAS_AIORTC:
            self.logger.warning(
                "aiortc not installed - WebRTC features disabled")
    ...
    async def create_peer_connection(self, user_id: str, room_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.info(
                f"Created peer connection: {connection_id} for user: {user_id} in room: {room_id}")
    ...
    async def create_offer(self, connection_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.config.enable_video:
                ...
                self.connections[connection_id].media_types.add(
                    MediaType.VIDEO)
    ...
    def _on_track(self, connection_id: str, track) -> None:
        ...
        try:
            self.logger.info(
                f"Received {track.kind} track from connection: {connection_id}")
    ...
    def _on_data_channel_message(self, connection_id: str, message: str) -> None:
        ...
        try:
            self.logger.info(
                f"Received data message from {connection_id}: {message}")
...
class SignalingServer:
    ...
    def __init__(self, host: str = "localhost", port: int = 8765):
        ...
        if not HAS_WEBSOCKETS:
            self.logger.warning(
                "websockets not installed - Signaling server disabled")
    ...
    async def start_server(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Starting signaling server on {self.host}:{self.port}")
    ...
    async def handle_join_room(self, client_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if connection_id:
                # Get room users
                room_connections = self.peer_manager.get_room_connections(
                    room_id)
                room_users = []
    ...
    async def handle_offer(self, client_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not connection_id or not target_user or not offer:
                raise ValueError(
                    "connection_id, target_user, and offer are required")
    ...
    async def handle_answer(self, client_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not connection_id or not target_user or not answer:
                raise ValueError(
                    "connection_id, target_user, and answer are required")
    ...
    async def handle_ice_candidate(self, client_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not connection_id or not target_user or not candidate:
                raise ValueError(
                    "connection_id, target_user, and candidate are required")
    ...
    async def process_message(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if msg_type == "room-joined":
                ...
                self.logger.info(
                    f"Joined room with connection ID: {self.connection_id}")
