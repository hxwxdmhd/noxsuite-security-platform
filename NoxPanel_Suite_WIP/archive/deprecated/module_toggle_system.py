...
class ModuleToggleManager:
    ...
    def _initialize_configurations(self):
        ...
        if not self.main_config_file.exists():
            ...
            self.logger.info(
                f"✅ Created main configuration: {self.main_config_file}")
        ...
        for env, config_file in self.environment_configs.items():
            if not config_file.exists():
                ...
                self.logger.info(
                    f"✅ Created {env} configuration: {config_file}")
    ...
    def get_module_status(self, module_name: str, environment: str = "development") -> Dict[str, Any]:
        ...
        config_file = self.environment_configs.get(
            environment, self.main_config_file)
    ...
    def toggle_module(self, module_name: str, enabled: bool, environment: str = "development") -> bool:
        ...
        if module_name in config.get("modules", {}):
            ...
            self.logger.info(
                f"✅ Module '{module_name}' {action} in {environment}")
            return True
        else:
    ...
    def toggle_component(self, module_name: str, component_name: str, enabled: bool, environment: str = "development") -> bool:
        ...
        if component_name in components:
            ...
            action = "enabled" if enabled else "disabled"
            self.logger.info(
                f"✅ Component '{module_name}.{component_name}' {action} in {environment}")
            return True
        else:
            self.logger.error(
                f"❌ Component '{module_name}.{component_name}' not found")
            return False
    ...
    def set_feature_flag(self, flag_type: str, flag_name: str, enabled: bool, environment: str = "development") -> bool:
        ...
        if flag_type in config.get("feature_flags", {}):
            ...
            action = "enabled" if enabled else "disabled"
            self.logger.info(
                f"✅ Feature flag '{flag_type}.{flag_name}' {action} in {environment}")
            return True
        else:
    ...
    def generate_toggle_report(self) -> Dict[str, Any]:
        ...
        for env, config_file in self.environment_configs.items():
            if config_file.exists():
                ...
                enabled_modules = sum(
                    1 for m in modules.values() if m.get("enabled", False))
                ...
                for module in modules.values():
                    ...
                    enabled_components += sum(1 for c in components.values()
    ...
    def export_configuration(self, output_file: str = None) -> str:
        ...
        if output_file is None:
            output_file = self.config_dir / \
                f"full_config_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        for env, config_file in self.environment_configs.items():
            if config_file.exists():
                export_data["environments"][env] = self._load_config(
                    config_file)
...
def main():
    ...
    for env, stats in report["environments"].items():
        print(
            f"  {env.title()}: {stats['enabled_modules']}/{stats['total_modules']} modules ({stats['module_utilization']}%)")
