...
class PluginPermissionsV2:
    ...
    allowed_file_extensions: List[str] = field(
        default_factory=lambda: ['.txt', '.json', '.log'])
...
class PluginMetadataV2:
    ...
    api_version: str = "2.0"
    security_level: SecurityLevel = SecurityLevel.MEDIUM
    permissions: PluginPermissionsV2 = field(
        default_factory=PluginPermissionsV2)
        ...
        while self.monitoring:
            try:
                ...
                if memory_used > self.limits.max_memory_mb:
                    logger.warning(
                        f"Plugin exceeded memory limit: {memory_used:.1f}MB > {self.limits.max_memory_mb}MB")
                    self.monitoring = False
                    raise MemoryError(
                        f"Plugin exceeded memory limit: {memory_used:.1f}MB")
                ...
                if self.start_time:
                    ...
                    if execution_time > self.limits.max_execution_time_seconds:
                        logger.warning(
                            f"Plugin exceeded time limit: {execution_time:.1f}s")
                        self.monitoring = False
                        raise TimeoutError(
                            f"Plugin exceeded execution time limit: {execution_time:.1f}s")

                # Check CPU usage
                cpu_percent = self.process.cpu_percent()
                if cpu_percent > self.limits.max_cpu_percent:
                    logger.warning(
                        f"Plugin exceeded CPU limit: {cpu_percent:.1f}%")
...
class RestrictedImporterV2:
    ...
    def _restricted_import(self, name, *args, **kwargs):
        ...
        if not any(allowed in name for allowed in self.allowed_modules):
            raise ImportError(
                f"Module '{name}' is not in allowed modules list")
...
class PluginSandboxV2:
    ...
    def __enter__(self):
        ...
        self.temp_dir = tempfile.mkdtemp(
            prefix=f"noxpanel_plugin_{self.sandbox_id}_")
        ...
        logger.info(
            f"🔒 Plugin sandbox activated: {self.sandbox_id} ({self.temp_dir})")
        return self
            ...
            with RestrictedImporterV2(self.limits.allowed_modules, self.limits.blocked_modules):
                ...
                for entry_point in entry_points:
                    if hasattr(plugin_module, entry_point):
                        logger.debug(
                            f"Executing plugin entry point: {entry_point}")
                    ...
                    try:
                        ...
                        if metadata:
                            ...
                            if self.security_manager.validate_plugin_security(plugin_path):
                                ...
                                logger.info(
                                    f"✅ Plugin discovered: {plugin_path.name}")
                            else:
                                logger.warning(
                                    f"🚨 Plugin security validation failed: {plugin_path.name}")
                                self._quarantine_plugin(plugin_path.name)
                    except Exception as e:
                        logger.error(
                            f"Error discovering plugin {plugin_path.name}: {e}")
    ...
    def _load_plugin_metadata(self, plugin_path: Path) -> Optional[PluginMetadataV2]:
        ...
        try:
            ...
            metadata.permissions = PluginPermissionsV2(
                ...
                can_execute_commands=perms_config.get(
                    'can_execute_commands', False),
                can_access_network=perms_config.get(
                    'can_access_network', False),
                can_access_database=perms_config.get(
                    'can_access_database', False),
                can_modify_system=perms_config.get('can_modify_system', False),
                can_create_threads=perms_config.get(
                    'can_create_threads', False),
                can_spawn_processes=perms_config.get(
                    'can_spawn_processes', False),
                allowed_directories=perms_config.get(
                    'allowed_directories', []),
                allowed_file_extensions=perms_config.get(
                    'allowed_file_extensions', ['.txt', '.json', '.log']),
                allowed_network_hosts=perms_config.get(
            ...
            metadata.limits = PluginLimitsV2(
                ...
                max_execution_time_seconds=limits_config.get(
                    'max_execution_time_seconds', 60),
                max_file_operations=limits_config.get(
                    'max_file_operations', 1000),
                max_network_connections=limits_config.get(
                    'max_network_connections', 5),
                max_subprocess_count=limits_config.get(
                    'max_subprocess_count', 0)
            )

            # Add custom allowed/blocked modules
            if 'allowed_modules' in limits_config:
                metadata.limits.allowed_modules.update(
                    limits_config['allowed_modules'])
            if 'blocked_modules' in limits_config:
                metadata.limits.blocked_modules.update(
                    limits_config['blocked_modules'])
            ...
            logger.error(
                f"Failed to load plugin metadata from {plugin_path}: {e}")
            return None
    ...
    async def load_plugin(self, plugin_id: str, config: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if not security_result['is_safe']:
                logger.error(
                    f"Plugin {plugin_id} failed security validation: {security_result['reason']}")
            ...
            if not dependencies['resolved']:
                logger.error(
                    f"Plugin {plugin_id} dependency resolution failed: {dependencies['missing']}")
                return False
            ...
            logger.debug(
                f"Plugin loading error details: {traceback.format_exc()}")
    ...
    def _quarantine_plugin(self, plugin_id: str):
        ...
        try:
            ...
            if plugin_path.exists():
                ...
                log_file = self.plugin_directory / \
                    "logs" / f"quarantine_{plugin_id}.json"
        ...
        try:
            ...
            for pattern in self.dangerous_patterns:
                if re.search(pattern, code):
                    logger.warning(
                        f"Potentially dangerous code pattern found: {pattern}")
                    return False
