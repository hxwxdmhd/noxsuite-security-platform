...
• GPU Available: {result.get('system_info', {}).get('gpu_available', False)}
    ...
    console.print(
        Panel(panel_content, title="🔍 ContextForge Service Info", border_style="blue"))
...
def context_health():
    ...
    if "error" in result:
        console.print(
            f"[bold red]❌ ContextForge is unhealthy: {result['error']}[/bold red]")
        return
    ...
    if status == 'healthy':
        ...
        console.print(
            f"[bold yellow]⚠️ ContextForge status: {status}[/bold yellow]")
...
def context_test(test_name: str = typer.Argument(..., help="Test name to run")):
    ...
    if not service_integration:
        console.print(
            "[bold red]❌ Service integration module not available[/bold red]")
        return
    ...
    if result.get('success', False):
        ...
        console.print(
            f"[bold red]❌ Test '{test_name}' failed: {result.get('error', 'Unknown error')}[/bold red]")
...
def aethercore_info():
    ...
    if "error" in result.get("health", {}):
        console.print(
            f"[bold red]❌ Error: {result['health']['error']}[/bold red]")
        return
...
• Failed Requests: {metrics.get('failed_requests', 0)}
    ...
    console.print(
        Panel(panel_content, title="🤖 AetherCore Service Info", border_style="magenta"))
...
def aethercore_load_model(model_id: str = typer.Argument(..., help="Model ID to load")):
    ...
    if "error" in result:
        console.print(
            f"[bold red]❌ Error loading model: {result['error']}[/bold red]")
        return

    status = result.get("status", "unknown")
    if status == "loading":
        console.print(
            f"[bold yellow]🔄 Loading model '{model_id}'...[/bold yellow]")
    elif status == "already_loaded":
        console.print(
            f"[bold green]✅ Model '{model_id}' is already loaded[/bold green]")
    else:
        console.print(
            f"[bold blue]ℹ️ Model '{model_id}' status: {status}[/bold blue]")
...
def aethercore_unload_model(model_id: str = typer.Argument(..., help="Model ID to unload")):
    ...
    if "error" in result:
        console.print(
            f"[bold red]❌ Error unloading model: {result['error']}[/bold red]")
        return

    console.print(
        f"[bold green]✅ Model '{model_id}' unloaded successfully[/bold green]")
...
def aethercore_serve(
    ...
    if "error" in result:
        console.print(
            f"[bold red]❌ Error serving request: {result['error']}[/bold red]")
        return

    if result.get("status") == "success":
        console.print(
            f"[bold green]✅ Inference completed in {result.get('processing_time_ms', 0):.2f}ms[/bold green]")
        ...
        console.print(
            f"[bold red]❌ Inference failed: {result.get('error', 'Unknown error')}[/bold red]")
...
def aethercore_health():
    ...
    if "error" in result:
        console.print(
            f"[bold red]❌ AetherCore is unhealthy: {result['error']}[/bold red]")
        return
    ...
    if status == 'healthy':
        ...
        console.print(
            f"[bold yellow]⚠️ AetherCore status: {status}[/bold yellow]")
...
"""
    ...
    console.print(
        Panel(panel_content, title="🖥️ System Status", border_style="green"))
...
def system_services():
    ...
    for service_name, info in result.items():
        if "error" in info:
            ...
            status = "✅ Healthy" if info.get(
                "status") == "healthy" else f"⚠️ {info.get('status', 'Unknown')}"
