...
class ContextForgeDeployment:
    ...
    async def pre_deployment_checks(self) -> bool:
        ...
        if not all_passed:
            console.print(
                "[bold red]❌ Some checks failed. Please resolve issues before deployment.[/bold red]")
            return False

        console.print(
            "[bold green]✅ All pre-deployment checks passed![/bold green]")
        return True
    ...
    async def check_python_version(self) -> str:
        ...
        if version.major == 3 and version.minor >= 8:
            ...
            raise Exception(
                f"Python {version.major}.{version.minor}.{version.micro} not supported. Requires Python 3.8+")
    ...
    async def check_configuration(self) -> str:
        ...
        required_keys = ["model_registry",
                         "routing_strategies", "context_sources"]
    ...
    async def check_port_availability(self) -> str:
        ...
        try:
            ...
            raise Exception(
                f"Port {self.deployment_config['api_port']} already in use")
    ...
    async def deploy_contextforge(self) -> bool:
        ...
        try:
            ...
            console.print(
                f"[bold green]Starting server with command:[/bold green]")
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        console.print(
                            "[bold green]✅ ContextForge server started successfully![/bold green]")
                        return True
                    else:
                        console.print(
                            f"[bold red]❌ Server health check failed: {response.status_code}[/bold red]")
                        return False
            except Exception as e:
                console.print(
                    f"[bold red]❌ Cannot connect to server: {str(e)}[/bold red]")
                return False

        except Exception as e:
            console.print(
                f"[bold red]❌ Deployment failed: {str(e)}[/bold red]")
            return False
            ...
            for test_name, url in tests:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        results.append(
                            (test_name, "❌", f"HTTP {response.status_code}"))
                except Exception as e:
...
• OpenAPI: http://localhost:{self.deployment_config['api_port']}/openapi.json
        ...
        console.print(Panel(
            panel_content, title="🚀 ContextForge Deployment Summary", border_style="green"))
    ...
    async def run_deployment(self):
        ...
        if not await self.pre_deployment_checks():
            console.print(
                "[bold red]❌ Deployment aborted due to failed checks.[/bold red]")
            return False
        ...
        console.print(
            "[bold green]✅ ContextForge deployment completed successfully![/bold green]")
        return True
