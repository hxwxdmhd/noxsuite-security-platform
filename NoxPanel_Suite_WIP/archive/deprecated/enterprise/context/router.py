...
class ContextRouter:
    ...
    def __init__(self, contextforge: ContextForge):
        ...
        logger.info(
            "ContextRouter initialized with dynamic dispatch capabilities")
    ...
    def initialize_endpoints(self):
        ...
        default_endpoints = {
            ModelType.GPT4O: [
                ModelEndpoint(
                    ModelType.GPT4O, "https://api.openai.com/v1/chat/completions", "api_key_1"),
                ModelEndpoint(
                    ModelType.GPT4O, "https://api.openai.com/v1/chat/completions", "api_key_2")
            ],
            ModelType.COPILOT: [
                ModelEndpoint(
                    ModelType.COPILOT, "https://api.github.com/copilot/chat/completions", "copilot_key"),
            ],
            ModelType.CLAUDE: [
                ModelEndpoint(
                    ModelType.CLAUDE, "https://api.anthropic.com/v1/messages", "claude_key"),
            ]
        }

        self.endpoints = default_endpoints
        logger.info(
            f"Initialized {sum(len(v) for v in self.endpoints.values())} model endpoints")
    ...
    async def route_request(self, request: ContextRequest) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Routing request {request.metadata['request_id']} with intent {request.intent}")

        # Get routing rule for this intent
        routing_rule = self.routing_rules.get(
            request.intent, self.get_default_routing_rule())
    ...
    async def select_endpoint(self, request: ContextRequest, routing_rule: Dict[str, Any]) -> ModelEndpoint:
        ...
        """
        preferred_models = routing_rule.get(
        ...
        if not available_endpoints:
            logger.warning(
                f"No healthy endpoints for preferred models {preferred_models}")
    ...
    async def calculate_adaptive_score(self, endpoint: ModelEndpoint, request: ContextRequest) -> float:
        ...
        base_score = endpoint.health_score * 0.4 + \
            (1.0 - endpoint.get_load_factor()) * 0.3
        ...
        total_score = base_score + compatibility_score * \
            0.2 + intent_bonus + response_time_factor * 0.1
    ...
    async def execute_routing_strategy(self, request: ContextRequest, endpoint: ModelEndpoint,
        ...
        try:
            ...
            endpoint.update_metrics(
                end_time - start_time, result.get("status") == "success")
    ...
    async def update_performance_metrics(self, request: ContextRequest, result: Dict[str, Any]):
        ...
        logger.info(
            f"Updated performance metrics for request {request.metadata['request_id']}")
        ...
        for model_type, endpoints in self.endpoints.items():
            for endpoint in endpoints:
                try:
                    ...
                    logger.warning(
                        f"Health check failed for {endpoint.endpoint_url}: {e}")
                    endpoint.health_score = max(
                        0.0, endpoint.health_score - 0.1)
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        successful_requests = len(
            [m for m in self.performance_history if m["status"] == "success"])

        avg_response_time = 0.0
        if total_requests > 0:
            avg_response_time = sum(m["execution_time"]
                                    for m in self.performance_history) / total_requests
...
if __name__ == "__main__":
    ...
    print(
        f"Available Endpoints: {sum(len(v) for v in router.endpoints.values())}")
