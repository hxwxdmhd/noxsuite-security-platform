...
def check_dependencies():
    ...
    if missing_modules:
        print_status(
            f"Missing modules: {', '.join(missing_modules)}", "warning")
...
def show_interactive_menu():
    ...
    options = [
        ...
        ("🚀", "Project Bootstrap",
         "Create new project or reset environment", "bootstrap"),
    ...
    while True:
        try:
            ...
            print_status("Invalid choice. Please enter a number between 1 and {}.".format(
                len(options)), "error")
...
def start_web_dashboard():
    ...
    try:
        ...
        if integrated_server.exists():
            print_status(
                "Starting integrated Heimnetz dashboard...", "success")
            ...
            print_status(
                "Web dashboard starting on http://localhost:5000", "success")
            start_webpanel()
        ...
        if web_root.exists():
            os.chdir(web_root)
            print_status(
                "Starting simple HTTP server on http://localhost:8000", "info")
            subprocess.run([sys.executable, "-m", "http.server", "8000"])
        else:
            print_status(
                "No web files found. Please check installation.", "error")
...
def start_voice_assistant():
    ...
    try:
        ...
        try:
            ...
            print_status(
                "Install: pip install SpeechRecognition pyttsx3", "info")
    ...
    try:
        ...
        for scanner_path in scanner_paths:
            if scanner_path.exists():
                print_status(
                    f"Running network scanner: {scanner_path.name}", "success")
                subprocess.run([sys.executable, str(scanner_path)])
                return

        print_status(
            "Network scanner not found. Creating basic scanner...", "warning")
    ...
    try:
        ...
        for admin_path in admin_paths:
            if admin_path.exists():
                print_status(
                    f"Launching admin panel: {admin_path.name}", "success")
                subprocess.run([sys.executable, str(admin_path)])
                return

        print_status(
            "Admin panel not found. Using web dashboard instead.", "warning")
        start_web_dashboard()
...
def start_project_bootstrap():
    ...
    try:
        ...
        if bootstrap_script.exists():
            ...
            print_status(
                "Bootstrapper not found. Please check installation.", "error")
    except Exception as e:
    ...
    for doc_path in doc_paths:
        if doc_path.exists():
            ...
            if platform.system() == "Windows":
                ...
                subprocess.run(["open" if platform.system() ==
                               "Darwin" else "xdg-open", str(doc_path)])
            return
...
def start_integrated_web():
    ...
    try:
        ...
        if (backend_path / "webpanel" / "app_v5.py").exists():
            ...
            print_status(
                "Integrated dashboard starting on http://localhost:5000", "success")
            print_status(
                "Heimnetz frontend available at http://localhost:5000/heimnetz", "info")
            app.run(host="0.0.0.0", port=5000, debug=False)

        else:
            print_status(
                "AI backend not found, using basic integration", "warning")
            start_web_dashboard()
...
Examples:
  ...
  python main.py --scan         # Network scanner
    ...
    parser.add_argument("--ai", action="store_true",
                        help="Start AI assistant (text mode)")
    parser.add_argument("--voice", action="store_true",
                        help="Start voice-enabled AI assistant")
    parser.add_argument("--assistant", type=str,
                        help="Send single command to AI assistant")
