...
class EnhancedResourceMonitor:
    ...
    def start_monitoring(self, process_id: Optional[int] = None):
        ...
        logger.info(
            f"🔍 Enhanced resource monitoring started for PID: {self.process_id}")
            ...
            try:
                ...
                if elapsed_time > self.limits.max_execution_time_seconds:
                    ...
                    if self.config.quarantine_on_violation:
                        logger.error(
                            "⏰ Plugin execution timeout - forcing termination")
                ...
                if len(self.violations) > self.config.violation_threshold:
                    logger.error(
                        f"❌ Too many violations ({len(self.violations)}) - terminating monitoring")
                    break
    ...
    def _handle_violation_sync(self, violation: SandboxViolation):
        ...
        logger.warning(
            f"🚨 Sandbox violation detected: {violation.description}")
...
class EnhancedPluginSandbox:
    ...
    async def initialize_sandbox(self):
        ...
        if self.config.enable_filesystem_isolation and WATCHDOG_AVAILABLE:
            ...
            logger.warning(
                "📁 Filesystem monitoring requested but watchdog not available")
    ...
    async def cleanup_sandbox(self):
        ...
        try:
            ...
            if self.telemetry:
                ...
                self.telemetry.network_operations_count = len(
                    self.network_isolation.network_operations)
            ...
            logger.info(
                f"🧹 Enhanced sandbox cleanup complete: {self.sandbox_id} ({cleanup_time:.2f}s)")
    ...
    async def execute_plugin_safe(self, plugin_function: Callable,
        ...
        try:
            ...
            logger.info(
                f"✅ Plugin executed successfully in sandbox: {self.sandbox_id}")
            return result

        except TimeoutError:
            logger.error(
                f"⏰ Plugin execution timeout in sandbox: {self.sandbox_id}")
            violation = SandboxViolation(
                type=SandboxViolationType.TIMEOUT_EXCEEDED,
                timestamp=datetime.now(),
            ...
            logger.error(
                f"❌ Plugin execution error in sandbox {self.sandbox_id}: {e}")
            violation = SandboxViolation(
    ...
    def get_telemetry(self) -> SandboxTelemetry:
        ...
        if self.telemetry:
            ...
            if self.resource_monitor:
                self.telemetry.violations.extend(
                    self.resource_monitor.violations)
...
class SandboxRecoveryManager:
    ...
    async def auto_recovery_violation(self, violation: SandboxViolation) -> bool:
        ...
        logger.info(
            f"🔧 Attempting auto-recovery for violation: {violation.type.value}")
    ...
    async def _recover_from_timeout_violation(self, violation: SandboxViolation) -> bool:
        ...
        try:
            # Log timeout details
            logger.warning(
                f"⏰ Plugin timeout recovery for: {violation.plugin_id}")
...
class EnhancedPluginFramework:
    ...
    def __init__(self, plugin_directory: str = "plugins",
        ...
        logger.info(
            f"🏰 Enhanced Plugin Framework initialized with {self.isolation_config.level.value} isolation")
        ...
        try:
            async with sandbox:
                ...
                def plugin_function(env):
                    ...
                    logger.info(
                        f"🔌 Executing plugin {plugin_id} in enhanced sandbox")
                ...
                logger.info(
                    f"✅ Enhanced plugin execution completed: {plugin_id}")
            ...
            logger.error(
                f"❌ Enhanced plugin execution failed: {plugin_id} - {e}")
...
def get_enhanced_plugin_framework(plugin_directory: str = "plugins",
    ...
    if enhanced_plugin_framework is None:
        enhanced_plugin_framework = EnhancedPluginFramework(
            plugin_directory, isolation_config)
...
async def main():
    ...
    print(
        f"📊 Real-time Monitoring: {'Enabled' if config.enable_real_time_monitoring else 'Disabled'}")
    print(
        f"🔧 Auto Recovery: {'Enabled' if config.auto_recovery_enabled else 'Disabled'}")
