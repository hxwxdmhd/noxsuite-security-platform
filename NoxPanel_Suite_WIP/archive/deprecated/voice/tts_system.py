    ...
    def detect_voice_activity(self, audio_data: bytes) -> bool:
        ...
        try:
            if not HAS_NUMPY:
                ...
                amplitude = sum(abs(byte)
                                for byte in audio_data) / len(audio_data)
...
class TextToSpeechEngine:
    ...
    def _initialize_engine(self) -> None:
        ...
        try:
            if self.config.engine == TTSEngine.PYTTSX3:
                ...
                raise ValueError(
                    f"Unsupported TTS engine: {self.config.engine}")

            self.logger.info(
                f"TTS engine initialized: {self.config.engine.value}")
    ...
    def _initialize_azure(self) -> None:
        ...
        try:
            ...
            self.engine = speechsdk.SpeechSynthesizer(
    ...
    def recognize_speech(self, audio_data: Optional[bytes] = None,
        ...
        try:
            if audio_data:
                ...
                audio = sr.AudioData(
                    audio_data, self.audio_config.sample_rate, 2)
            else:
...
class VoiceAssistant:
    ...
    def speak(self, text: str, interrupt: bool = False) -> bool:
        ...
        try:
            if self.is_speaking and not interrupt:
                self.logger.warning(
                    "Already speaking, use interrupt=True to override")
                return False
    ...
    def listen(self, timeout: float = 5.0) -> Optional[str]:
        ...
        try:
            ...
            # Recognize speech
            text = self.stt_engine.recognize_speech(
            ...
            while True:
                ...
                if user_input:
                    ...
                    if result is None:
                        ...
                        self.speak(
                            "I'm sorry, I didn't understand that command. Say 'help' for available commands.")
