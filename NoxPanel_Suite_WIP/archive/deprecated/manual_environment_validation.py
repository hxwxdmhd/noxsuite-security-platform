...
def validate_environment():
    ...
    if plugins_dir.exists():
        ...
        validation_results["environment_checks"]["plugin_count"] = len(
            plugin_files)
    ...
    if devcontainer_json.exists():
        ...
        validation_results["recommendations"].append(
            "Use Remote Container for optimal development")
    else:
        print("⚠️ Remote Container not configured")
        validation_results["recommendations"].append(
            "Configure Remote Container for better performance")
    ...
    if workspace_file.exists():
        ...
        validation_results["recommendations"].append(
            "Multi-root workspace ready as fallback")
    else:
    ...
    try:
        ...
        if total_files > 1000:
            validation_results["recommendations"].append(
                "Large project - recommend Remote Container or multi-root workspace")
        elif total_files > 500:
            validation_results["recommendations"].append(
                "Medium project - VS Code optimization recommended")
        else:
            validation_results["recommendations"].append(
                "Small project - native development suitable")
