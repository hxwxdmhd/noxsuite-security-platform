...
async def demonstrate_sysadmin_copilot():
    ...
    try:
        ...
        if 'error' not in health:
            print(
                f"   • Health Score: {health.get('overall_score', 'N/A')}/100")
            print(
                f"   • Status: {health.get('health_status', 'Unknown').title()}")
            print(f"   • Issues Found: {len(health.get('issues', []))}")

            if health.get('metrics'):
                metrics = health['metrics']
                print(
                    f"   • CPU Usage: {metrics.get('cpu_percent', 'N/A'):.1f}%")
                print(
                    f"   • Memory Usage: {metrics.get('memory_percent', 'N/A'):.1f}%")
                print(
                    f"   • Process Count: {metrics.get('process_count', 'N/A')}")
        else:
        ...
        for i, suggestion in enumerate(suggestions[:3], 1):
            print(
                f"   {i}. {suggestion.get('description', 'N/A')} (Priority: {suggestion.get('priority', 'N/A')})")
        ...
        if script_result.get('script_generated'):
            ...
            print(
                f"   • Estimated Runtime: {script_info.get('estimated_runtime', 'N/A')}")
            print(f"   • Task ID: {script_result.get('task_id', 'N/A')}")
        else:
            print(
                f"   ⚠️ Script generation failed: {script_result.get('error', 'Unknown error')}")
...
def demonstrate_plugin_framework():
    ...
    try:
        ...
        for plugin in plugins[:3]:  # Show first 3 plugins
            print(
                f"   • {plugin.get('name', 'Unknown')} v{plugin.get('version', '0.0.0')} - {plugin.get('status', 'Unknown')}")
        ...
        template_result = plugin_manager.generate_plugin_template(
            "demo_plugin", "utility")
        if template_result.get('success'):
            print(
                f"   • Template created: {template_result.get('plugin_path', 'N/A')}")
            print("   • Template includes: Interface, metadata, examples")
        else:
            print(
                f"   ⚠️ Template generation failed: {template_result.get('error', 'Unknown error')}")
