...
class UltimateWebApp:
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                try:
                    response = requests.get(
                        f"{self.config.ollama_host}/api/tags", timeout=2)
        ...
        def api_network_devices():
            ...
            try:
                ...
                common_ips = [f"{base_ip}.{i}" for i in [
                ...
                devices = [
                    ...
                    {'id': 2, 'name': 'Switch', 'ip': '192.168.1.2',
                        'status': 'online', 'type': 'switch'},
                    {'id': 3, 'name': 'PC-1', 'ip': '192.168.1.100',
    ...
    def run(self):
        ...
        logger.info(
            f"🚀 Starting Ultimate Heimnetz/NoxPanel/NoxGuard Suite v{self.config.version}")
        logger.info(
            f"🌐 Available at: http://{self.config.host}:{self.config.port}")
        logger.info(f"🎨 Default theme: {self.config.default_theme}")
        logger.info(
            f"🧠 AI models: {'Enabled' if self.config.ai_models_enabled else 'Disabled'}")
        logger.info(
            f"🌐 Network monitoring: {'Enabled' if self.config.network_monitoring else 'Disabled'}")
