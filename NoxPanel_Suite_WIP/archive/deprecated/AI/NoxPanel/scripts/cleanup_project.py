...
class ProjectCleaner:
    ...
    def move_file_safely(self, source: Path, destination: Path) -> bool:
        ...
        try:
            ...
            self.log(
                f"✅ Moved: {source.name} → {destination.relative_to(self.project_root)}")
            return True
        ...
        for pattern in backup_patterns:
            for backup_file in self.project_root.rglob(pattern):
                ...
                if backup_file.is_file():
                    destination = self.archive_dirs["backups"] / \
                        backup_file.name
        ...
        for pattern in test_patterns:
            for test_file in self.project_root.rglob(pattern):
                ...
                if test_file.is_file():
                    destination = self.archive_dirs["test_results"] / \
                        test_file.name
        ...
        for pattern in report_patterns:
            for report_file in self.project_root.rglob(pattern):
                ...
                if report_file.is_file() and report_file.suffix == ".md":
                    destination = self.archive_dirs["reports"] / \
                        report_file.name
    ...
    def organize_deprecated_files(self):
        ...
        for pattern in deprecated_patterns:
            for dep_file in self.project_root.rglob(pattern):
                ...
                if dep_file.is_file():
                    destination = self.archive_dirs["deprecated"] / \
                        dep_file.name
            ...
            for dir_name in dirs:
                ...
                try:
                    if not any(dir_path.iterdir()):  # Directory is empty
                        dir_path.rmdir()
                        self.log(
                            f"🗂️  Removed empty directory: {dir_path.relative_to(self.project_root)}")
                        removed_count += 1
    ...
    def generate_cleanup_report(self):
        ...
        self.log(
            f"📄 Cleanup report saved: {report_file.relative_to(self.project_root)}")
    ...
    def run_full_cleanup(self):
        ...
        # Summary
        self.log(
            f"📁 Archive directory: {self.archive_dir.relative_to(self.project_root)}")
