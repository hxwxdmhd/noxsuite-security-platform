...
class RouteTester:
        ...
        for py_file in project_root.rglob("*.py"):
            try:
                ...
                app_routes = re.findall(
                    r'@app\.route\(["\']([^"\']+)["\'](?:.*methods\s*=\s*\[([^\]]+)\])?', content)
                for route_match in app_routes:
                    path = route_match[0]
                    methods = ['GET']  # Default method
                    if route_match[1]:
                        methods = [m.strip().strip('\'"')
                                   for m in route_match[1].split(',')]
    ...
    def get_known_routes(self) -> List[Dict]:
        ...
        core_routes = [
            {'path': '/', 'method': 'GET', 'type': 'core',
                'description': 'Main dashboard'},
            {'path': '/chat', 'method': 'GET', 'type': 'core',
                'description': 'Chatbot interface'},
            {'path': '/api/health', 'method': 'GET',
                'type': 'api', 'description': 'Health check'},
            {'path': '/api/chat/status', 'method': 'GET',
                'type': 'api', 'description': 'Chat status'},
            {'path': '/api/chat/message', 'method': 'POST',
                'type': 'api', 'description': 'Send chat message'},
            {'path': '/api/scripts', 'method': 'GET',
                'type': 'api', 'description': 'List scripts'},
            {'path': '/api/stats', 'method': 'GET', 'type': 'api',
                'description': 'System statistics'},
            {'path': '/admin', 'method': 'GET',
                'type': 'module', 'description': 'Admin panel'},
            {'path': '/api/models', 'method': 'GET',
                'type': 'api', 'description': 'Models API'},
        ]
    ...
    def test_route(self, route_info: Dict) -> RouteTestResult:
        ...
        try:
            ...
            is_functional = self._analyze_functionality(
                response, path, content_type)
            # REASONING: Variable assignment with validation criteria
    ...
    def test_all_routes(self) -> Dict:
        ...
        for i, route in enumerate(routes, 1):
            print(
                f"Testing {i}/{len(routes)}: {route['method']} {route['path']}")
            ...
            if result.is_functional:
                functional_count += 1
                print(
                    f"  ✅ {result.status_code} - Functional ({result.response_time:.3f}s)")
            else:
                error_count += 1
                print(
                    f"  ❌ {result.status_code} - Issues: {', '.join(result.issues)}")
    ...
    def print_summary(self, results: Dict):
        ...
        for route_type, stats in by_type.items():
            success_rate = (stats['functional'] / stats['total']
                            * 100) if stats['total'] > 0 else 0
            print(
                f"  {route_type}: {stats['functional']}/{stats['total']} ({success_rate:.1f}%)")
