...
class AuditSystemController:
    ...
    def run_available_sequence(self) -> None:
        ...
        for gate_num in range(next_gate, 9):
            ...
            if not success:
                self.log(
                    f"🛑 Sequence stopped at Gate {gate_num} due to failure", "ERROR")
                break
    ...
    def display_system_status(self) -> None:
        ...
        print(
            f"📊 Progress: {status['gates_passed']}/{status['total_gates']} gates passed")
        print(f"🎯 Completion: {(status['gates_passed']/8)*100:.1f}%")

        if status["next_available_gate"]:
            print(
                f"▶️  Next Gate: {status['next_available_gate']} - {self.gate_config[status['next_available_gate']]['name']}")
        else:
        ...
        for gate_num in range(1, 9):
            ...
            print(
                f"  {status_icon} Gate {gate_num}: {gate_info['name']} - {gate_status['status']} ({gate_status['score']}/100)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NoxPanel 8-Gate Audit System Launcher")
    ...
    parser.add_argument("--run-sequence", action="store_true",
                        help="Run available gates in sequence")
