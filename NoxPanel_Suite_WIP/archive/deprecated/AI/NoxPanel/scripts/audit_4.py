...
class APISecurityAuditor:
    ...
    def test_security_headers_validation(self):
        ...
        try:
            ...
            for endpoint in endpoints:
                try:
                    ...
                    security_headers = {
                        ...
                        # Check for HSTS
                        'Strict-Transport-Security': ('max-age=', 4),
                    ...
                    print(
                        f"    {endpoint}: {endpoint_score}/20 security points")
            ...
            self.log_result("Security Headers Validation",
                            avg_security_score >= 12, points, details)

        except Exception as e:
            self.log_result("Security Headers Validation",
                            False, 0, f"Test error: {str(e)}")
        ...
        try:
                    ...
                    if response.status_code == 401:
                        ...
                        try:
                            ...
                            if isinstance(data, dict) and len(data) > 0:
                                ...
                                auth_score += 1
                                auth_details.append(
                                    f"{endpoint}: ✓ (limited access)")
                        except:
                            auth_score += 1
                            auth_details.append(
                                f"{endpoint}: ✓ (non-JSON response)")
                    else:
                        auth_details.append(
            ...
            try:
                ...
                login_response = session.get(
                    f"{self.base_url}/login", timeout=10)
            ...
            try:
                ...
                test_tokens = [
                    ...
                    # Strong token
                    hashlib.sha256(b"test_secret_key").hexdigest()
                ]
            ...
            self.log_result("Authentication Token Security",
                            auth_score >= 8, points, details)
            ...
            for test_case in test_cases:
                try:
                    ...
                    validation_details.append(
                        f"{test_case['name']}: {status} ({reason})")

                except Exception as e:
                    validation_details.append(
                        f"{test_case['name']}: ✗ (error: {str(e)[:50]})")

            # Convert to 20-point scale
            points = min(20, validation_score)

            details = f"Validation score: {validation_score}/21, Tested {len(test_cases)} attacks"
            self.log_result("Input Validation and Sanitization",
                            validation_score >= 14, points, details)
    ...
    def test_rate_limiting_implementation(self):
        ...
        try:
                ...
                try:
                    ...
                    if response.status_code == 429:  # Too Many Requests
                        ...
                        rate_limit_details.append(
                            f"Request {i+1}: ✓ (429 - rate limited)")
                        break
                    elif response.status_code == 503:  # Service Unavailable
                        rate_limit_score += 2
                        rate_limit_details.append(
                            f"Request {i+1}: ⚠ (503 - service unavailable)")
                    elif 'retry-after' in response.headers.keys():
                        rate_limit_score += 2
                        rate_limit_details.append(
                            f"Request {i+1}: ✓ (retry-after header)")
                    elif 'x-ratelimit' in str(response.headers).lower():
                        rate_limit_score += 1
                        rate_limit_details.append(
                            f"Request {i+1}: ✓ (rate limit headers)")
                    ...
                    rate_limit_details.append(
                        f"Request {i+1}: Error - {str(e)[:30]}")
                    break

            # Test 2: Check for rate limiting headers
            try:
                response = session.get(
                    f"{self.base_url}{test_endpoint}?q=header_test", timeout=10)
                headers = response.headers
                ...
                if found_headers:
                    rate_limit_details.append(
                        f"Rate limit headers: ✓ ({', '.join(found_headers)})")
                else:
                    rate_limit_details.append(
            ...
            try:
                ...
                different_session.headers.update(
                ...
                rate_limit_details.append(
                    f"Different client test: Error - {str(e)}")
            ...
            if len(request_times) > 5:
                ...
                if avg_response_time > 0.5:  # Slower responses might indicate throttling
                    rate_limit_score += 2
                    rate_limit_details.append(
                        f"Response time: ✓ (avg {avg_response_time:.2f}s - possible throttling)")
                else:
                    rate_limit_details.append(
            ...
            if 429 in unique_status_codes or 503 in unique_status_codes:
                rate_limit_score += 3
                rate_limit_details.append(
                    "Status codes: ✓ (rate limiting detected)")

            # Convert to 20-point scale
            points = min(20, rate_limit_score)

            details = f"Rate limit score: {rate_limit_score}, Made {len(request_times)} requests"
            self.log_result("Rate Limiting Implementation",
                            rate_limit_score >= 5, points, details)
        ...
        try:
                    ...
                    try:
                        ...
                        for params in test_params:
                            ...
                            if response.status_code == 400:
                                ...
                                sql_details.append(
                                    f"Payload {i+1}: ✓ (400 - bad request)")
                                break
                            elif response.status_code == 422:
                                sql_injection_score += 2
                                sql_details.append(
                                    f"Payload {i+1}: ✓ (422 - validation error)")
                                break
                            elif any(indicator in response_text for indicator in bad_indicators):
                                sql_details.append(
                                    f"Payload {i+1}: ✗ (SQL error exposed)")
                                break
                            elif response.status_code == 200:
                                ...
                                try:
                                    ...
                                    if isinstance(data, dict) and data.get('error'):
                                        ...
                                        sql_details.append(
                                            f"Payload {i+1}: ✓ (error handled)")
                                    else:
                                        sql_injection_score += 1
                                        sql_details.append(
                                            f"Payload {i+1}: ✓ (safe response)")
                                except:
                                    sql_injection_score += 1
                                    sql_details.append(
                                        f"Payload {i+1}: ✓ (non-JSON response)")
                                break
                            elif response.status_code >= 500:
                                sql_details.append(
                                    f"Payload {i+1}: ⚠ (server error)")
                                break

                    except Exception as e:
                        sql_details.append(
                            f"Payload {i+1}: Error - {str(e)[:40]}")
            ...
            try:
                ...
                if response.status_code in [200, 400, 422]:
                    ...
                    sql_details.append(
                        f"Special chars: ? (status {response.status_code})")
            ...
            if sql_injection_score >= 10:
                sql_injection_score += 2
                sql_details.append(
                    "Parameterized queries: ✓ (likely implemented)")

            # Convert to 20-point scale
            points = min(20, sql_injection_score)

            details = f"SQL injection score: {sql_injection_score}, Tested {len(sql_payloads)} payloads"
            self.log_result("SQL Injection Prevention",
                            sql_injection_score >= 12, points, details)
    ...
    def generate_report(self):
        ...
        report_path = report_dir / \
            f"gate_4_security_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run_audit(self):
        ...
        print("=" * 60)
        print(
            f"Final Score: {self.score}/{self.max_score} ({report['percentage']}%)")
        ...
        if report['passed']:
            ...
            print(
                "💡 Focus on implementing security headers, input validation, and rate limiting.")
