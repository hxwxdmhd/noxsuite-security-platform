...
class ArchitecturalEnforcer:
    ...
    def validate_merge_plan_compliance(self) -> Tuple[bool, List[str]]:
        ...
        if current_phase == "unknown":
            violations.append(
                "❌ Current audit phase not defined in project_state.json")

        # Check compliance score
        compliance_score = self.project_state.get(
            "architectural_compliance", {}).get("compliance", 0)
        if compliance_score < 100:
            violations.append(
                f"❌ Architectural compliance below 100%: {compliance_score}%")
    ...
    def update_project_state(self, violations_count: int):
        ...
        if self.project_state_file.exists():
            try:
                ...
                state["architectural_compliance"]["last_validation"] = datetime.now(
                ).isoformat()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="U.A.C.M.S. Architectural Structure Enforcer")
