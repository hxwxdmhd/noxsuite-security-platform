...
class NoxPanelEnvironmentSetup:
    ...
    def run_performance_validation(self):
        ...
        try:
            ...
            print(
                f"📁 Python files found: {file_count} (scan time: {file_scan_time:.2f}s)")
        except Exception as e:
        ...
        try:
            ...
            print(
                f"🔧 Git status: {'OK' if git_ok else 'FAILED'} ({git_time:.2f}s)")
        except (subprocess.TimeoutExpired, FileNotFoundError):
            git_time = 10.0
            git_ok = False
    ...
    def recommend_setup(self, performance_results):
        ...
        if docker_available and performance_score >= 60:
            ...
            print(
                f"   • Run: code --folder-uri vscode-remote://dev-container%2Bk%3A%5CProject%20Heimnetz")
            ...
            print(
                f"   • Open: {self.project_root / 'noxpanel-modular-workspace.code-workspace'}")
            return "multi-root"
...
'''
    ...
    def run_setup(self):
        ...
        docker_ok = self.setup_remote_container(
        ) if self.check_docker_availability() else False
