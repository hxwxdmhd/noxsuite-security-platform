...
class PluginValidator:
    ...
    def __init__(self, workspace_path: str):
        ...
        self.security_log = self.workspace_path / \
            "security" / "validation_security.log"
    ...
    def init_validation_database(self):
        ...
        with pymysql.connect(str(self.validation_db)) as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def calculate_plugin_hash(self, plugin_path: Path) -> str:
        ...
        try:
            ...
            self.log_security_event(
                "ERROR", f"Hash calculation failed for {plugin_path}: {str(e)}")
            return ""
    ...
    def assess_plugin_risk(self, plugin_path: Path) -> Tuple[int, List[str]]:
        ...
        try:
            ...
            for pattern in medium_risk_patterns:
                if pattern in content:
                    risk_factors.append(
                        f"Medium-risk pattern detected: {pattern}")
                    risk_score += 15
    ...
    def quarantine_plugin(self, plugin_path: Path, reason: str) -> Path:
        ...
        self.log_security_event(
            "QUARANTINE", f"Plugin quarantined: {plugin_path} -> {quarantine_path}, Reason: {reason}")
    ...
    def validate_plugin(self, plugin_path: Path, expected_hash: Optional[str] = None) -> Dict[str, Any]:
        ...
        signature_valid, signature_message = self.verify_plugin_signature(
            plugin_path, expected_hash)
    ...
    def add_trusted_signature(self, signature_hash: str, plugin_name: str, added_by: str):
        ...
        with pymysql.connect(str(self.validation_db)) as conn:
            ...
            try:
                ...
                conn.commit()
                self.log_security_event(
                    "TRUST", f"Added trusted signature for {plugin_name}")
                return True
            except sqlite3.IntegrityError:
                self.log_security_event(
                    "WARNING", f"Signature already exists for {plugin_name}")
                return False
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        with pymysql.connect(str(self.validation_db)) as conn:
            ...
            cursor.execute(
                "SELECT status, COUNT(*) FROM plugin_validations GROUP BY status")
    ...
    def validate_workspace_plugins(self) -> List[Dict[str, Any]]:
        ...
        for plugin_file in plugin_files:
            try:
                ...
                self.log_security_event(
                    "ERROR", f"Validation failed for {plugin_file}: {str(e)}")
...
def main():
    ...
    try:
        ...
        print(f"\nRisk Statistics:")
        print(
            f"  Average Risk: {summary['risk_statistics']['average_risk']}/100")
        print(
            f"  Risk Range: {summary['risk_statistics']['minimum_risk']}-{summary['risk_statistics']['maximum_risk']}/100")
        ...
        for result in results[-5:]:  # Show last 5 results
            ...
            print(
                f"  {status_emoji} {result['plugin_name']}: {result['status']}")
