...
async def main():
    ...
    logger.info("🤖 RLVR VALIDATOR AGENT v4.0")
    logger.info("=" * 50)
    logger.info()
    ...
    try:
        ...
        # Display summary
        compliance_rate = report['compliance_metrics']['overall_compliance']
        logger.info(f"✅ Scan complete: {compliance_rate:.1%} RLVR compliance")
        logger.info(f"📈 Components validated: {report['total_components']}")
        logger.info(f"🎯 Average confidence: {report['compliance_metrics']['average_confidence']:.1%}")
        ...
        logger.info(f"❌ Validator Agent error: {str(e)}")
