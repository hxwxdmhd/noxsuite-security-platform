...
async def main():
    ...
    try:
        ...
        logger.info(
            f"Final Compliance: {summary.get('final_compliance', '0.00%')}")
        logger.info(
            f"Improvement Factor: {summary.get('improvement_factor', '1.00x')}")
        logger.info(
            f"Target Achieved: {summary.get('target_achieved', False)}")
        logger.info(
            f"Deployment Ready: {summary.get('deployment_ready', False)}")
        logger.info(
            f"Ultimate Success: {summary.get('ultimate_success', False)}")
