...
.EXAMPLE
    .\project_cleanup_migration_working.ps1 -DryRun
    .\project_cleanup_migration_working.ps1 -BackupPath "C:\Backup\ProjectCleanup"
#>
...
if (-not $BackupPath) {
    ...
    $BackupPath = Join-Path (Get-Location) "backup_$timestamp"
}

if (-not $LogPath) {
    $timestamp = Get-Date -Format 'yyyyMMdd_HHmmss'
    $LogPath = Join-Path (Get-Location) "cleanup_log_$timestamp.txt"
}
    ...
    try {
        ...
        foreach ($item in $items) {
            ...
            foreach ($keepPath in $keepPaths) {
                try {
                    $resolvedKeepPath = Resolve-Path $keepPath -ErrorAction SilentlyContinue
                    if ($resolvedKeepPath -and $fullPath -eq $resolvedKeepPath.Path) {
                        $isProtected = $true
                        break
                    }
                } catch {}
            ...
            if (-not $DryRun) {
                try {
                    ...
                    Write-Log "   ❌ Failed to delete: $fullPath - Error: $($_.Exception.Message) ($fullPath)" "ERROR"
                }
            } else {
        ...
        Write-Log "Error processing target '$target': $($_.Exception.Message) ($target)" "ERROR"
    }
}
