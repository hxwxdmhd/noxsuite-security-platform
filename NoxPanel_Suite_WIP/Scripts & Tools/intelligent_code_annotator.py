...
class IntelligentCodeAnnotator:
    ...
    """
      self.workspace_root = Path(workspace_root)
       self.kb_path = knowledge_base_path or self.workspace_root / \
            "mcp" / "knowledgebase" / "knowledge.json"
    ...
    """
      """
       ...
       logger.info("🧠 Starting intelligent code annotation process")
        ...
        python_files = [
            f for f in python_files if self._should_process_file(f)]
        ...
        logger.info(
            f"✅ Code annotation complete: {results['annotations_added']} annotations added")
        return results
       ...
       results = {
        ...
        try:
            ...
            enhanced_content = self._enhance_content(
                original_content, tree, file_path)
            ...
            if enhanced_content != original_content:
                ...
                results["annotations"] = enhanced_content.count(
                    "REASONING CHAIN:") - original_content.count("REASONING CHAIN:")
                results["kb_refs"] = enhanced_content.count(
                    "KB_REF:") - original_content.count("KB_REF:")

                logger.debug(
                    f"Enhanced {file_path}: +{results['annotations']} annotations")
       ...
       existing_docstring = ast.get_docstring(node)
        ...
        if not existing_docstring:
            ...
            enhancement_lines.append(
                f"    {self._generate_function_description(node)}")
            enhancement_lines.append("    ")
        ...
        enhancement_lines.append(
            f"    ENHANCED: {datetime.now().strftime('%Y-%m-%d')} - AI-generated reasoning")
        enhancement_lines.append('    """')
    ...
    """
      """Enhance a class with RLVR documentation"""
       ...
       existing_docstring = ast.get_docstring(node)
        ...
        if not existing_docstring:
            enhancement_lines.append(
                f"    {self._generate_class_description(node)}")
            enhancement_lines.append("    ")
            ...
            for entry in entries:
                if isinstance(entry, dict):
                    ...
                    if name.lower() in entry_content or any(tag in name.lower() for tag in entry.get('tags', [])):
                        kb_refs.append(
                            f"mcp/knowledgebase/{category}.json#{entry.get('source_id', 'unknown')}")
