...
class NoxPanelWorkspaceLauncher:
    ...
    """
      """Analyze current directory to suggest best workspace"""
       cwd = Path.cwd()
        relative_path = cwd.relative_to(
            self.project_root) if cwd.is_relative_to(self.project_root) else None
    ...
    """
      """Launch specified workspace"""
       if workspace not in self.workspaces:
            ...
            logger.info(
                f"Available workspaces: {', '.join(self.workspaces.keys())}")
            return False

        if not self.check_workspace_exists(workspace):
            logger.info(
                f"❌ Workspace file not found: {self.workspaces[workspace]['file']}")
            return False
        ...
        logger.info(
            f"📁 Files: {stats['total_files']} total, {stats['python_files']} Python")
        ...
        try:
            ...
            logger.info(
                "❌ VS Code not found in PATH. Please install VS Code or add it to PATH.")
            return False
    ...
    """
      """Show interactive workspace selection menu"""
       logger.info("🏠 NOXPANEL MULTI-WORKSPACE LAUNCHER")
        ...
        logger.info(
            f"💡 Suggested workspace based on current directory: {suggested}")
        logger.info()
        ...
        for i, (key, info) in enumerate(self.workspaces.items(), 1):
            ...
            logger.info(
                f"   Files: {stats['total_files']} | Memory: {stats['estimated_memory_mb']}MB | Profile: {stats['performance_profile']}")
            logger.info()

        logger.info(
            f"{len(self.workspaces) + 1}. 🎯 Launch Suggested Workspace ({suggested})")
        ...
        while True:
            try:
                choice = input(
                    "Enter your choice (1-{}): ".format(len(self.workspaces) + 2)).strip()

                if choice.isdigit():
                    choice_num = int(choice)

                    if 1 <= choice_num <= len(self.workspaces):
                        workspace_key = list(self.workspaces.keys())[
                            choice_num - 1]
      ...
      """Show status of all workspaces"""
       logger.info("📊 NOXPANEL WORKSPACE STATUS")
        ...
        for key, info in self.workspaces.items():
            ...
            stats = self.get_workspace_stats(key) if exists else {
                "total_files": 0, "python_files": 0, "estimated_memory_mb": 0}
            ...
            logger.info(
                f"   ⚡ Profile: {stats.get('performance_profile', 'unknown')}")
            logger.info()
        ...
        logger.info(
            f"📈 TOTALS: {total_files:,} files, ~{total_memory}MB if all workspaces loaded")
        logger.info(
            f"💡 Recommended: Use focused workspaces to reduce memory usage by 60-75%")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NoxPanel Multi-Workspace Launcher")
