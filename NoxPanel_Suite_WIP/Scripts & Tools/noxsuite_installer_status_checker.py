    ...
    """
      """Print human-readable status summary"""
       logger.info("\n" + "=" * 60)
        ...
        if missing_opt:
            logger.info(
                f"⚠️  Missing optional files: {', '.join(missing_opt)}")
        ...
        logger.info(
            f"📊 Platform: {sys_info['platform']['system']} {sys_info['platform']['architecture']}")
        ...
        for log_file, info in report["logs"].items():
            if info["exists"]:
                if "last_session" in info:
                    ...
                    status = "✅" if session_info.get(
                        "last_completion") else "⚠️"
                    logger.info(
