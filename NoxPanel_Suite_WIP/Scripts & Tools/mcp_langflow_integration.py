...
class MCPLangflowIntegrationAgent:
            ...
            if config_file.exists():
                ...
                for server_name, server_config in mcp_config.get("mcpServers", {}).items():
                    agent_config = AgentConfig(
                        ...
                        description=server_config.get("description", "NoxSuite MCP Agent"),
    ...
    def _create_default_agent_configs(self):
        ...
        default_agents = [
            ...
            },
            {
                ...
                "capabilities": ["knowledge_extraction", "semantic_search", "conversation_parsing"],
            ...
            },
            {
                ...
                "capabilities": ["code_annotation", "rlvr_injection", "documentation_enhancement"],
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    elif proc.info['cmdline'] and any('langflow' in str(cmd).lower() for cmd in proc.info['cmdline']):
                        langflow_processes.append(proc.info)
            ...
            try:
                response = requests.get(
                    self.langflow_status.health_endpoint,
                    timeout=5
                )
                
                self.langflow_status.response_time_ms = (time.time() - start_time) * 1000
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        self.langflow_status.version = health_data.get("version", "unknown")
                    except:
                        self.langflow_status.version = "detected"
                        
                    logger.info(f"✅ Langflow HEALTHY - Response: {self.langflow_status.response_time_ms:.1f}ms")
                    ...
                    self.langflow_status.error_message = f"Unexpected HTTP {response.status_code}"
                    
            except requests.exceptions.ConnectionError:
                if langflow_processes:
                    self.langflow_status.state = LangflowState.DEGRADED
                    self.langflow_status.error_message = "Process running but API unreachable"
                    logger.warning("⚠️ Langflow process detected but API unreachable")
                else:
                    self.langflow_status.state = LangflowState.UNREACHABLE
                    self.langflow_status.error_message = "No process found and API unreachable"
    ...
    async def attempt_langflow_repair(self) -> bool:
        ...
        logger.info(f"🔧 Starting Langflow repair attempt #{self.langflow_status.repair_attempts}")
        ...
        try:
            ...
            if self.langflow_status.repair_attempts == 1:
                ...
                for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                    try:
                        if proc.info['name'] and 'langflow' in proc.info['name'].lower():
                            logger.info(f"🛑 Terminating Langflow process {proc.info['pid']}")
                            proc.terminate()
                ...
                try:
                    ...
                    logger.warning("⚠️ Langflow command not found - proceeding to reinstall")
                except Exception as e:
            ...
            # Stage 2: Reinstall dependencies
                ...
                try:
                    ...
                    logger.info(f"🚀 Langflow startup after reinstall (PID: {process.pid})")
                    ...
                    status = await self.check_langflow_health()
                    if status.state == LangflowState.HEALTHY:
                        logger.info("✅ Stage 2 repair successful - Langflow reinstalled")
                        return True
    ...
    def switch_to_deferred_mode(self):
        ...
        try:
            ...
            deferred_file = self.deferred_dir / f"langflow_integration_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(f"""# 🎯 NoxSuite-Langflow Integration Status
...
""")
            ...
            for agent in self.agent_configs:
                try:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        logger.warning(f"⚠️ Failed to integrate {agent.name}: HTTP {response.status_code}")
                        
                except Exception as e:
                    logger.error(f"❌ Integration failed for {agent.name}: {e}")
            
            if successful_integrations > 0:
                logger.info(f"🎉 Direct integration successful: {successful_integrations}/{len(self.agent_configs)} agents")
                self.integration_mode = IntegrationMode.DIRECT_API
                return True
            else:
            ...
            "data": {
                "node": {
                    "template": {
                        ...
                        "mcp_command": {"value": agent.metadata.get("command", "python"), "type": "str"},
                        "mcp_args": {"value": agent.metadata.get("args", []), "type": "list"},
                        "system_prompt": {
                ...
                if status.state == LangflowState.HEALTHY:
                    ...
                    if self.integration_mode == IntegrationMode.DEFERRED:
                        ...
                        if await self.attempt_direct_integration():
                            ...
                            logger.warning("⚠️ Re-integration failed - staying in deferred mode")
                    
                elif status.state in [LangflowState.DEGRADED, LangflowState.UNREACHABLE]:
                    consecutive_failures += 1
                    
                    if consecutive_failures >= 3:
                        logger.warning("⚠️ Multiple consecutive failures - attempting repair")
                        if await self.attempt_langflow_repair():
                            consecutive_failures = 0
                        else:
                            if self.integration_mode != IntegrationMode.DEFERRED:
                                logger.info("🔄 Switching to deferred mode due to persistent failures")
                                self.switch_to_deferred_mode()
                
                # Log status summary
                logger.info(f"📊 Status: {status.state.name} | Mode: {self.integration_mode.name} | Failures: {consecutive_failures}")
        ...
        try:
            ...
            if status.state == LangflowState.HEALTHY:
                ...
                if await self.attempt_direct_integration():
                    ...
                    logger.warning("⚠️ Direct integration failed - switching to deferred mode")
                    self.switch_to_deferred_mode()
                ...
                logger.info("📋 Langflow not detected - creating deferred configuration")
                self.switch_to_deferred_mode()
                results["integration_mode"] = "DEFERRED"
                results["next_steps"] = [
    ...
    def _generate_final_status_report(self, results: Dict[str, Any]):
        ...
        with open(status_file, 'w', encoding='utf-8') as f:
            f.write(f"""# 🎯 NoxSuite MCP-Langflow Integration Report
...
async def main():
    ...
    logger.info(f"🎉 Success: {'YES' if results['success'] else 'PARTIAL (with fallbacks)'}")
    logger.info("="*60)
