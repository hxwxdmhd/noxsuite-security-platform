...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(
        "cleanup_report.log"), logging.StreamHandler()],
)
...
class NoxSuiteCleanup:
                    ...
                    if isinstance(node, (ast.Import, ast.ImportFrom)):
                        if isinstance(node, ast.Import):
                            for alias in node.names:
                                self._check_import(
                                    alias.name, py_file, import_errors)
                        elif isinstance(node, ast.ImportFrom):
                            if node.module:
                                self._check_import(
                                    node.module, py_file, import_errors)
    ...
    def _check_import(
        self, module_name: str, file_path: Path, import_errors: List[str]
    ):
            ...
            try:
                ...
                for pattern, issue_type in dangerous_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        ...
                        self.issues_found["security_issues"].append(
                            str(py_file))
    ...
    def archive_files(self, file_list: List[str], archive_subdir: str, reason: str):
        ...
        logger.info(
            f"📦 Archiving {len(file_list)} files to {archive_subdir} ({reason})"
        )
