...
class Gate8Validator:
    ...
    def run_all_checks(self, environments=None):
        ...
        logger.info(f"🔍 Running Gate 8 validation for environments: {', '.join(environments)}")
        ...
        if self.results["max_score"] > 0:
            self.results["score"] = int((self.results["score"] / self.results["max_score"]) * 100)
        
        # Determine critical blockers
        critical_blockers = [b for b in self.results["blockers"] if b.startswith("[CRITICAL]")]
        
        # Print summary
        print("\n" + "=" * 60)
        print(f"Gate 8 Validation Summary: {self.results['score']}%")
        print("=" * 60)
        print(f"Checks passed: {sum(1 for c in self.results['checks'] if c[1])} of {len(self.results['checks'])}")
    ...
    def check_git_status(self):
        ...
        try:
            ...
            is_valid_branch = branch in ["main", "master"] or branch.startswith("release")
            ...
            if is_valid_branch and is_clean:
                ...
                self.add_recommendation(f"Checkout a release branch (main, master, or release*) instead of '{branch}'")
            elif not is_clean:
                message = f"Working tree has uncommitted changes"
                self.add_check_result("Git Status", False, message, weight=10)
                self.add_recommendation("Commit or stash all changes before release")
            
        except Exception as e:
            logger.error(f"Failed to check Git status: {e}")
            self.add_check_result("Git Status", False, f"Error checking Git status: {e}", weight=10)
            self.add_recommendation("Ensure Git is installed and repository is initialized")
    
    def check_deployment_config(self, environment):
        """Check if deployment configuration exists for the specified environment"""
        print(f"🔍 Checking deployment configuration files for environment '{environment}'...")
        ...
        for pattern in config_patterns:
            ...
            if config_file.exists():
                found_configs.append(str(config_file.relative_to(self.workspace_path)))
        ...
        if found_configs:
            ...
            self.add_check_result(f"Deployment Config ({environment})", True, message, weight=15)
        else:
            message = f"No deployment configuration found for '{environment}' environment"
            self.add_check_result(f"Deployment Config ({environment})", False, message, weight=15, critical=True)
    ...
    def check_documentation(self):
        ...
        # Check README
        try:
            readme_file = find_file(root_path, ["README.md", "Readme.md", "readme.md"])
            ...
            if readme_file:
                with open(readme_file, 'r') as f:
                    ...
                    has_installation = re.search(r'#.*install|\binstall\b|installation', content, re.IGNORECASE) is not None
                    has_usage = re.search(r'#.*usage|\busage\b|#.*example|\bexample\b', content, re.IGNORECASE) is not None
                    has_api = re.search(r'#.*api|\bapi\b|endpoint', content, re.IGNORECASE) is not None
                    
                    sections_found = [s for s, found in [("Installation", has_installation), ("Usage", has_usage), ("API", has_api)] if found]
                    
                    if len(sections_found) >= 2:  # At least 2 key sections
                        self.add_check_result("README", True, f"README found with sections: {', '.join(sections_found)}", weight=10)
                    else:
                        self.add_check_result("README", False, "README is missing key sections", weight=10)
                        self.add_recommendation("Add Installation, Usage, and API sections to README")
            else:
                self.add_check_result("README", False, "README not found", weight=10)
                self.add_recommendation("Create a README.md file with project documentation")
        
        except Exception as e:
            logger.error(f"Error checking README: {e}")
            self.add_check_result("README", False, f"Error checking README: {e}", weight=10)
        
        # Check API documentation
        try:
            api_docs = find_file(root_path, ["api.md", "API.md", "docs/api.md", "docs/API.md"])
            
            if api_docs:
                self.add_check_result("API Documentation", True, f"API documentation found: {os.path.basename(api_docs)}", weight=10)
            else:
                self.add_check_result("API Documentation", False, "API documentation not found", weight=10)
                self.add_recommendation("Create API documentation with endpoint specifications")
        
        except Exception as e:
            logger.error(f"Error checking API docs: {e}")
            self.add_check_result("API Documentation", False, f"Error checking API docs: {e}", weight=10)
        
        # Check CHANGELOG
        try:
            changelog = find_file(root_path, ["CHANGELOG.md", "CHANGELOG", "CHANGES.md"])
            
            if changelog:
                self.add_check_result("Changelog", True, f"Changelog found: {os.path.basename(changelog)}", weight=5)
            else:
                self.add_check_result("Changelog", False, "Changelog not found", weight=5)
                self.add_recommendation("Create a CHANGELOG.md file to track version changes")
        
        except Exception as e:
            logger.error(f"Error checking Changelog: {e}")
            self.add_check_result("Changelog", False, f"Error checking Changelog: {e}", weight=5)
        
        # Check security and compliance docs
        print("🔍 Checking for required security and compliance documentation...")
        try:
            # Check for security policy
            security_file = find_file(root_path, ["SECURITY.md", "security.md", "docs/security.md"])
            
            if security_file:
                self.add_check_result("Security Policy", True, f"Security policy found: {os.path.basename(security_file)}", weight=10)
            else:
                self.add_check_result("Security Policy", False, "Security policy not found", weight=10)
                self.add_recommendation("Create a SECURITY.md file with security reporting guidelines")
            
            # Check for license
            license_file = find_file(root_path, ["LICENSE", "LICENSE.md", "license.md"])
            
            if license_file:
                self.add_check_result("License", True, f"License found: {os.path.basename(license_file)}", weight=5)
            else:
                self.add_check_result("License", False, "License not found", weight=5)
                self.add_recommendation("Add a LICENSE file to the project")
        
        except Exception as e:
            logger.error(f"Error checking security docs: {e}")
            self.add_check_result("Security Documentation", False, f"Error checking security docs: {e}", weight=10)
        ...
        try:
            ...
            for test_dir in test_dirs:
                ...
                if test_path.exists() and test_path.is_dir():
                    test_files.extend([str(f) for f in test_path.glob("**/*.py") if f.is_file()])
                    test_files.extend([str(f) for f in test_path.glob("**/*.js") if f.is_file()])
                    test_files.extend([str(f) for f in test_path.glob("**/*.ts") if f.is_file()])
            
            # This is a simplified example - in a real implementation, would parse coverage files
            coverage_files = [f for f in os.listdir(root_path) if f.endswith('.coverage') or f.endswith('coverage.xml')]
            
            if test_files:
                self.add_check_result("Test Suite", True, f"Found {len(test_files)} test files", weight=10)
                
                if coverage_files:
                    self.add_check_result("Test Coverage", True, "Coverage reports found", weight=10)
                else:
                    self.add_check_result("Test Coverage", False, "No coverage reports found", weight=10)
                    self.add_recommendation("Generate test coverage reports before release")
            else:
                self.add_check_result("Test Suite", False, "No test files found", weight=10, critical=True)
                self.add_check_result("Test Coverage", False, "No tests to measure coverage", weight=10, critical=True)
                self.add_recommendation("Create comprehensive test suite before release")
        
        except Exception as e:
            logger.error(f"Error checking tests: {e}")
            self.add_check_result("Test Suite", False, f"Error checking tests: {e}", weight=10)
        ...
        try:
            ...
            if versions:
                ...
                if len(unique_versions) == 1:
                    ...
                    self.add_check_result("Version Consistency", True, f"Consistent version ({version}) across {len(versions)} files", weight=10)
                else:
                    msg = "Inconsistent versions: " + ", ".join(f"{f}={v}" for f, v in versions.items())
                    self.add_check_result("Version Consistency", False, msg, weight=10, critical=True)
                    self.add_recommendation("Ensure version is consistent across all files")
            else:
                self.add_check_result("Version Consistency", False, "No version information found", weight=10)
                self.add_recommendation("Add version information to setup.py or package.json")
        
        except Exception as e:
            logger.error(f"Error checking version consistency: {e}")
            self.add_check_result("Version Consistency", False, f"Error checking version consistency: {e}", weight=10)
    ...
    def check_security(self):
        ...
        try:
            ...
            security_scan_files = find_files(self.workspace_path, ["security_scan.json", "security_audit.json"])
                ...
                with open(security_scan_files[0], 'r') as f:
                    try:
                        ...
                        if "vulnerabilities" in scan_data:
                            high_vulns = [v for v in scan_data["vulnerabilities"] 
                                         if v.get("severity", "").lower() in ["critical", "high"]]
                            
                            if high_vulns:
                                security_blockers = [v.get("description", "Unnamed vulnerability") for v in high_vulns[:3]]
                                if len(high_vulns) > 3:
                                    security_blockers.append(f"...and {len(high_vulns) - 3} more")
                    except json.JSONDecodeError:
                        security_blockers = ["Error parsing security scan results"]
            
            if security_blockers:
                self.add_check_result("Security Issues", False, 
                ...
                self.add_check_result("Security Issues", True, 
    ...
    def check_dependencies(self):
        ...
        try:
            ...
            requirements_file = find_file(self.workspace_path, ["requirements.txt", "pyproject.toml"])
            ...
            if requirements_file:
                ...
                with open(package_json, 'r') as f:
                    try:
                        data = json.load(f)
                        has_deps = "dependencies" in data and len(data["dependencies"]) > 0
                        has_dev_deps = "devDependencies" in data and len(data["devDependencies"]) > 0
                        ...
                        if has_deps or has_dev_deps:
                            ...
                            if not any(v == "*" or v.startswith("^") for v in all_deps.values()):
                                deps_pinned = True
                                deps_message = "Dependencies pinned in package.json"
                            else:
                                deps_message = "Some dependencies not pinned in package.json"
                    except json.JSONDecodeError:
                        deps_message = "Error parsing package.json"
            
            self.add_check_result("Pinned Dependencies", deps_pinned, deps_message, weight=10)
            
            if not deps_pinned:
                self.add_recommendation("Pin all dependencies to specific versions before release")
        
        except Exception as e:
            logger.error(f"Error checking dependencies: {e}")
            self.add_check_result("Pinned Dependencies", False, f"Error checking dependencies: {e}", weight=10)
    ...
    def generate_report(self, output_file=None):
        ...
        passed = score_percent >= 80 and not any(b.startswith("[CRITICAL]") for b in self.results["blockers"])
        ...
        if self.results["blockers"]:
            critical_count = sum(1 for b in self.results["blockers"] if b.startswith("[CRITICAL]"))
            if critical_count > 0:
        ...
        try:
            ...
            report_content = report_template.replace("<!-- DATE -->", datetime.now().strftime("%Y-%m-%d"))
            report_content = report_content.replace("<!-- VERSION -->", version)
            report_content = report_content.replace("<!-- AUDITOR -->", os.environ.get("USER", "automated"))
            
            # Add scores and status
            if max_score > 0:
                report_content = report_content.replace("<!-- SCORE -->", f"{score}/{max_score} ({score_percent:.1f}%)")
                report_content = report_content.replace("<!-- STATUS -->", "PASSED" if passed else "FAILED")
                report_content = report_content.replace("<!-- SUMMARY -->", summary)
            
            # Add check results
            checks = self.results["checks"]
            checks_content = "\n".join([f"- {'✅' if check[1] else '❌'} **{check[0]}**: {check[2]}" for check in checks])
            report_content = report_content.replace("<!-- CHECKS -->", checks_content)
            
            # Add blockers
            blockers = self.results["blockers"]
            if blockers:
                blockers_content = "\n".join([f"- ❌ {blocker}" for blocker in blockers])
                report_content = report_content.replace("<!-- BLOCKERS -->", blockers_content)
            else:
                report_content = report_content.replace("<!-- BLOCKERS -->", "None! ✅")
            
            # Add recommendations
            recommendations = self.results["recommendations"]
            if recommendations:
                recommendations_content = "\n".join([f"- 📝 {rec}" for rec in recommendations])
                report_content = report_content.replace("<!-- RECOMMENDATIONS -->", recommendations_content)
            else:
                report_content = report_content.replace("<!-- RECOMMENDATIONS -->", "None! ✅")
            ...
            for check_name, check_passed, check_message in checks:
                if "Version" in check_name:
                ...
                elif "Documentation" in check_name or "README" in check_name or "API Doc" in check_name:
                    test_statuses["DOCUMENTATION"] = "PASSED" if check_passed else "FAILED"
                    test_details["DOCUMENTATION"] = test_details.get("DOCUMENTATION", "") + f"- {check_message}\n"
                    ...
                    test_statuses["PERFORMANCE"] = "PASSED" if check_passed else "FAILED"
                    test_details["PERFORMANCE"] = test_details.get("PERFORMANCE", "") + f"- {check_message}\n"
            
            # Replace test section placeholders
            for section in ["VERSION", "DEPLOYMENT", "DOCUMENTATION", "SECURITY", "PERFORMANCE"]:
                status = test_statuses.get(section, "UNKNOWN")
                details = test_details.get(section, "No data available")
                
                report_content = report_content.replace(f"<!-- {section}_STATUS -->", status)
                report_content = report_content.replace(f"<!-- {section}_DETAILS -->", details)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gate 8: Release Readiness Validation")
    ...
    parser.add_argument("--threshold", type=int, default=80,
                       help="Passing threshold percentage")
    parser.add_argument("--publish", action="store_true",
                       help="Trigger publish if validation passes")
    ...
    try:
        ...
        critical_blockers = [b for b in results["blockers"] if b.startswith("[CRITICAL]")]
        
        # Generate report if requested
        if args.report:
            report_output = args.output or f"gate8_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        if args.publish and score_percent >= passing_threshold and not critical_blockers:
            ...
            print(f"❌ Publishing skipped due to failing checks ({score_percent:.1f}% < {passing_threshold}%)")
            if critical_blockers: 
