...
class NoxSuiteAuditAgent:
    ...
    def generate_audit_report(self) -> Dict[str, Any]:
        ...
        self.system_health_score = (critical_modules_healthy / total_modules_scanned) * 100
    ...
    def save_comprehensive_report(self):
        ...
        # Save JSON report
        json_report_file = self.audit_logs_dir / f"code_audit_installer_report_{self.timestamp}.json"
        with open(json_report_file, 'w', encoding='utf-8') as f:
            json.dump(audit_report, f, indent=2, ensure_ascii=False)
        
        # Generate Langflow workflow
        langflow_workflow = self.create_langflow_automation_workflow()
        workflow_file = self.audit_logs_dir / f"noxsuite_audit_repair_workflow_{self.timestamp}.json"
        ...
        md_report_file = self.audit_logs_dir / f"ADHD_VISUAL_AUDIT_REPORT_{self.timestamp}.md"
        with open(md_report_file, 'w', encoding='utf-8') as f:
            f.write(visual_report)
        
        # Update counters for final output
        self.critical_issues = audit_report["executive_summary"]["critical_issues_count"]
        self.high_issues = audit_report["executive_summary"]["high_issues_count"] 
        self.system_health_score = audit_report["executive_summary"]["system_health_score"]
