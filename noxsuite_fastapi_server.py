...
try:
    from backend.models.mariadb_user import User, Role, UserRole, UserSession, AuditLog, Base, engine, SessionLocal, get_db, log_audit_event
except ImportError:
    try:
        ...
        print("❌ CRITICAL: MariaDB models not available - run mariadb_dev_setup.py first")
        User = Role = UserRole = UserSession = AuditLog = SessionLocal = Base = engine = None
        def get_db():
            raise HTTPException(status_code=500, detail="MariaDB not configured")
...
def get_current_user(
    ...
    if not User:
        raise HTTPException(status_code=500, detail="Database models not available")
...
async def login_handler(
    ...
    if not user:
        log_audit_event(db, None, "login_failed", "auth", 
                       {"email": user_login.email, "reason": "user_not_found"}, request)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    if not user.verify_password(user_login.password):
        log_audit_event(db, user.id, "login_failed", "auth", 
    ...
    if user.mfa_enabled:
        if not user_login.mfa_code:
            ...
            log_audit_event(db, user.id, "login_mfa_required", "auth", 
                           {"mfa_session_id": mfa_session_id}, request)
...
async def register(
    ...
    if existing_user:
        log_audit_event(db, None, "register_failed", "auth", 
                       {"email": user_register.email, "reason": "user_exists"}, request)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    log_audit_event(db, user.id, "register_success", "auth", 
...
async def startup_event():
    ...
    if Base and engine:
        ...
        logger.warning("Database models not available - run setup_database_simple.py")
    ...
    try:
        ...
        logger.info("✅ RBAC and MFA extensions loaded and routes registered successfully")
