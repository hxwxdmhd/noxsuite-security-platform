...
class EnhancedMonitoringSystem:
    ...
    def __init__(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler('logs/enhanced_monitoring.log', mode='a', encoding='utf-8')
            ]
        )
    ...
    def _initialize_metrics(self):
        ...
        metric_definitions = [
            MetricDefinition(
                ...
                category="system",
                unit="percentage",
                threshold=MetricThreshold(warning=75.0, critical=85.0, duration="5m")
            ),
            MetricDefinition(
                id="API_PERF_001",
                ...
                unit="milliseconds",
                threshold=MetricThreshold(warning=150.0, critical=200.0, duration="1m")
            ),
            MetricDefinition(
                id="SYS_CPU_001",
                ...
                category="system",
                unit="percentage",
                threshold=MetricThreshold(warning=70.0, critical=85.0, duration="3m")
            ),
            MetricDefinition(
                id="SYS_DISK_001",
                ...
                category="system",
                unit="percentage",
                threshold=MetricThreshold(warning=80.0, critical=90.0, duration="10m")
            ),
            MetricDefinition(
                id="API_ERR_001",
                name="api_error_rate",
                category="performance",
                unit="percentage",
                threshold=MetricThreshold(warning=1.0, critical=5.0, duration="5m")
            )
        ]
    ...
    def _check_server_health(self):
        ...
        try:
            ...
            response = requests.get(f"{self.flask_server_url}/health", timeout=5)
            ...
            self._create_incident(
                ...
                context=["Server health check failed", "Connection timeout or refused"],
    ...
    def _update_metric(self, metric_id: str, value: float):
        ...
        if metric_id in self.metrics:
            ...
            self.metrics[metric_id].last_updated = datetime.now(timezone.utc).isoformat()
    
    def _scan_logs_for_patterns(self):
        """Scan logs for monitoring patterns"""
        log_files = [
            "logs/enhanced_monitoring.log",
            "logs/noxpanel_test.log"
        ]
    ...
    def _handle_pattern_match(self, pattern: MonitoringPattern, matched_line: str, 
                            all_lines: List[str], line_index: int):
    ...
    def _create_incident(self, pattern_id: str, severity: Severity, category: Category,
                        message: str, context: List[str], action_taken: str):
    ...
    def _intervention_immediate_halt(self, pattern: MonitoringPattern, matched_line: str):
    ...
    def _intervention_analyze_report(self, pattern: MonitoringPattern, matched_line: str):
    ...
    def _intervention_performance_analysis(self, pattern: MonitoringPattern, matched_line: str):
    ...
    def _trigger_threshold_intervention(self, metric: MetricDefinition, level: str):
        ...
        if level == "critical":
            self._create_incident(
                ...
                context=[f"Threshold: {metric.threshold.critical}", f"Current: {metric.current_value}"],
    ...
    def generate_enhanced_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "incidents": [asdict(incident) for incident in self.incidents[-10:]],  # Last 10 incidents
            "metrics": {
                metric_id: {
                    "name": metric.name,
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if health.cpu_usage_percent > 70:
            recommendations.append("Monitor CPU-intensive processes and consider optimization")
        
        if health.api_response_time_ms > 200:
            recommendations.append("API response times exceeding target - investigate endpoint optimization")
        
        if health.error_rate_percent > 1:
            recommendations.append("Elevated error rate detected - review application logs")
        
        # Check recent incidents
        recent_incidents = [i for i in self.incidents if 
                          datetime.fromisoformat(i.timestamp.replace('Z', '+00:00')) > 
                          datetime.now(timezone.utc) - timedelta(hours=1)]
        
        if len(recent_incidents) > 5:
            recommendations.append("High incident rate detected - review system stability")
    ...
    try:
        ...
        if report["incidents"]:
            ...
            for incident in report["incidents"][-3:]:  # Show last 3
                ...
                print(f"   {emoji} {incident['pattern_id']}: {incident['message'][:60]}...")
