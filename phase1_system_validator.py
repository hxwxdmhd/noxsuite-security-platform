...
class Phase1SystemValidator:
    ...
    async def validate_security(self) -> SystemCheck:
        ...
        try:
            response = requests.get(
                f"{self.flask_server_url}/api/emergency/status", timeout=5
            )
        ...
        try:
            auth_response = requests.get(
                f"{self.flask_server_url}/api/knowledge/search", timeout=5
            )
        ...
        try:
            headers_response = requests.get(
                f"{self.flask_server_url}/", timeout=5)
            security_headers = [
            ...
            present_headers = sum(
                1 for header in security_headers if header in headers_response.headers
            )
            metrics["security_headers_score"] = (
    ...
    async def validate_stability(self) -> SystemCheck:
        ...
        if metrics["disk_usage_percent"] > 85:
            ...
            print(
                f"  ✅ Disk usage: {metrics['disk_usage_percent']:.1f}% (healthy)")
        ...
        try:
            ...
            if python_processes:
                total_memory = sum(
                    p.info["memory_info"].rss for p in python_processes)
    ...
    async def validate_performance(self) -> SystemCheck:
        ...
        endpoints_to_test = ["/", "/api/emergency/status"]
        ...
        for endpoint in endpoints_to_test:
            try:
                ...
                response = requests.get(
                    f"{self.flask_server_url}{endpoint}", timeout=10
                )
                end_time = time.time()
                ...
                if response_time_ms < 200:
                    ...
                    issues.append(
                        f"Slow response {endpoint}: {response_time_ms:.1f}ms")
        ...
        try:
            ...
            if write_time < 10 and read_time < 5:
                print(
                    f"  ✅ File I/O: Write {write_time:.1f}ms, Read {read_time:.1f}ms"
                )
            else:
                issues.append(
                    f"Slow file I/O: Write {write_time:.1f}ms, Read {read_time:.1f}ms"
                )
        ...
        performance_score = (
            (sum(perf_factors) / len(perf_factors) * 100) if perf_factors else 0
        )
    ...
    def validate_phase1_gates(self) -> List[GateRequirement]:
        ...
        # Security Gate
        security_check = next(
            (r for r in self.results if r.component == "Security Foundation"), None
        )
        if security_check:
            security_score = security_check.metrics.get(
            ...
            print(
                f"{status_icon} Security Gate: {security_score:.1f}/100 (Required: 90)"
            )

        # Stability Gate
        stability_check = next(
            (r for r in self.results if r.component == "System Stability"), None
        )
        if stability_check:
            ...
            print(
                f"{status_icon} Stability Gate: {stability_score:.1f}/100 (Required: 85)"
            )

        # Performance Gate
        performance_check = next(
            (r for r in self.results if r.component == "Performance Baseline"), None
        )
        if performance_check:
            performance_score = performance_check.metrics.get(
                "performance_score", 0)
            ...
            print(
                f"{status_icon} Performance Gate: {performance_score:.1f}/100 (Required: 80)"
            )
    ...
    def can_proceed_to_phase2(self, gates: List[GateRequirement]) -> bool:
        ...
        if not all_passed:
            ...
            for gate in gates:
                if not gate.passed:
                    print(
                        f"  • {gate.name}: {gate.current_score:.1f}/{gate.required_score}"
                    )
    ...
    def generate_report(self, gates: List[GateRequirement]) -> Dict[str, Any]:
        ...
        report = {
            ...
            "overall_status": (
                "PASSED" if self.can_proceed_to_phase2(gates) else "BLOCKED"
            ),
...
async def main():
    ...
    for check in checks:
        ...
        print(
            f"{status_emoji[check.status.value]} {check.component}: {check.status.value.upper()}"
        )
