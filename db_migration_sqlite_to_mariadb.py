...
from sqlalchemy import create_engine, inspect, MetaData, Table, Column, String, Text, Integer
...
try:
    from backend.models.user import Base, User, Role, UserRole, UserSession, AuditLog
...
def setup_mariadb(mysql_url, mysql_db_name):
    ...
    with engine.connect() as conn:
        conn.execute(f"CREATE DATABASE IF NOT EXISTS {mysql_db_name} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
...
def migrate_data(mariadb_engine, mysql_engine, include_tables=None):
    ...
    for table_name in table_order:
        ...
        for row in rows:
            # Convert row to dict
            row_dict = {column.name: getattr(row, column.name) for column in mariadb_table.columns}
            ...
            try:
                ...
                logger.error(f"Error inserting row {row_dict} into {table_name}: {e}")
...
def validate_migration(mariadb_engine, mysql_engine):
    ...
    for table_name in mariadb_meta.tables:
        ...
        for mariadb_row in mariadb_rows:
            ...
            mysql_row = mysql_session.query(mysql_table).filter(getattr(mysql_table.c, pk_col) == pk_val).first()
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Migrate NoxSuite database from SQLite to MariaDB")
    parser.add_argument("--sqlite-path", type=str, default="./database/noxsuite.db", help="Path to SQLite database")
    parser.add_argument("--mysql-host", type=str, default="localhost", help="MariaDB host")
    parser.add_argument("--mysql-port", type=int, default=3306, help="MariaDB port")
    parser.add_argument("--mysql-user", type=str, default="root", help="MariaDB user")
    parser.add_argument("--mysql-password", type=str, default="noxsuite", help="MariaDB password")
    parser.add_argument("--mysql-db", type=str, default="noxsuite", help="MariaDB database name")
    ...
    if args.validate:
        ...
        logger.info(f"Validation results: {success_count}/{total_count} tables successfully migrated")
