...
class EmergencySecurityValidator:
        ...
        for endpoint in knowledge_endpoints:
            ...
            try:
                ...
                if response.status_code == 401:
                    ...
                    logger.info(
                        f"INFO: {endpoint} returns 404 - endpoint may not be integrated"
                    )
                else:
                    self.test_results.append(
                        {
                    ...
                    logger.error(
                        f"FAIL: {endpoint} allows unauthenticated access!")
    ...
    def test_emergency_login(self):
        ...
        try:
            response = requests.post(
                login_url, json=test_credentials, timeout=5)
            ...
            logger.warning(
                "SKIP: Cannot test emergency login - Flask app not running")
        except Exception as e:
    ...
    def test_authenticated_access(self):
        ...
        if not self.auth_token:
            logger.warning(
                "No auth token available - skipping authenticated access tests"
            )
            return
        ...
        for endpoint in knowledge_endpoints:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"PASS: {endpoint} allows authenticated access")
                else:
                    self.test_results.append(
                        {
                            "test": f"Authenticated access to {endpoint}",
                ...
                logger.warning(
                    f"SKIP: Cannot test authenticated access to {endpoint}")
            except Exception as e:
                logger.error(
                    f"Error testing authenticated access to {endpoint}: {e}")
    ...
    def test_security_headers(self):
        ...
        try:
            ...
            if len(headers_found) >= 3:  # At least 3 of 5 security headers
                ...
                logger.info(
                    f"PASS: Found {len(headers_found)} security headers")
            else:
                self.test_results.append(
                    {
                        "test": "Security headers",
                ...
                logger.warning(
                    f"WARN: Only {len(headers_found)} security headers found"
                )

        except requests.ConnectionError:
            logger.warning(
                "SKIP: Cannot test security headers - Flask app not running")
        except Exception as e:
    ...
    def generate_validation_report(self):
        ...
        warned_tests = len(
            [t for t in self.test_results if t["status"] == "WARN"])
        skipped_tests = len(
            [t for t in self.test_results if t["status"] == "SKIP"])
