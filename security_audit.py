...
class SecurityAuditor:
    ...
    def log_finding(self, category: str, severity: str, description: str, details: Dict = None):
    ...
    def scan_python_dependencies(self):
        ...
        try:
            ...
            try:
                ...
                self.log_finding('dependency_scan', 'WARNING', 
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_finding('dependency_scan', 'WARNING',
                               'pip-audit scan failed', {'error': result.stderr})
                
        except Exception as e:
            self.log_finding('dependency_scan', 'ERROR',
                           f'Python dependency scan failed: {e}')
    ...
    def scan_node_dependencies(self):
        ...
        for package_file in package_files:
            ...
            try:
                ...
                self.log_finding('dependency_scan', 'INFO',
    ...
    def scan_docker_images(self):
        ...
        docker_files = (
            list(self.workspace_root.glob('**/Dockerfile*')) +
            list(self.workspace_root.glob('**/docker-compose*.yml'))
        )
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if re.search(r'(password|secret|key|token)=\w+', line, re.IGNORECASE):
                    self.log_finding(
                        'docker_security', 'CRITICAL',
                        f"Possible hardcoded secret in {docker_file.name}:{i}",
                        {'line': line, 'file': str(docker_file)}
                    )
                
        except Exception as e:
            self.log_finding('docker_security', 'WARNING',
                           f'Could not analyze {docker_file}: {e}')
    ...
    def scan_python_file(self, py_file: Path):
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                # Check for hardcoded secrets
                if re.search(r'(password|secret|key|token)\s*=\s*["\'][^"\']+["\']', 
            ...
            self.log_finding('code_security', 'WARNING',
                           f'Could not analyze {py_file}: {e}')
    ...
    def analyze_javascript_security(self):
        ...
        js_files = (
            ...
            list(self.workspace_root.glob('**/*.jsx'))
        )
        ...
        for py_file in python_files:
            try:
                ...
                if re.search(r'jwt.*secret.*["\']secret["\']', content, re.IGNORECASE):
                    self.log_finding(
                        'authentication_security', 'CRITICAL',
                ...
                if 'jwt.encode' in content and 'exp' not in content:
                    self.log_finding(
                        'authentication_security', 'MEDIUM',
        ...
        for py_file in python_files:
            try:
                ...
                if 'password' in content.lower():
                    if not any(hash_func in content for hash_func in 
    ...
    def generate_security_report(self):
        ...
        report_file = self.workspace_root / f"security_audit_report_{self.audit_timestamp}.json"
    ...
    def generate_recommendations(self, severity_counts: Dict[str, int]) -> List[str]:
        ...
        if severity_counts['CRITICAL'] > 0:
            ...
            recommendations.append("Review and remove any eval() or exec() usage")
            recommendations.append("Remove hardcoded secrets and use environment variables")
        
        if severity_counts['HIGH'] > 0:
            recommendations.append("HIGH: Fix high-severity issues within 24 hours")
            recommendations.append("Implement proper password hashing (bcrypt/scrypt)")
            recommendations.append("Add HTTPS redirects for all HTTP endpoints")
    ...
    def generate_security_summary(self, severity_counts: Dict[str, int], report_file: Path):
...
"""
        ...
        if severity_counts['CRITICAL'] > 0:
            ...
            summary += "🟡 **MEDIUM RISK** - High-severity issues need prompt resolution\n"
...
"""
        ...
        summary_file = self.workspace_root / f"security_audit_summary_{self.audit_timestamp}.md"
