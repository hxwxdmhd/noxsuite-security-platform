...
class EmergencyAuthMiddleware:
    ...
    def __init__(self, app=None):
        self.app = app
        self.secret_key = os.environ.get('EMERGENCY_AUTH_SECRET', 'emergency-temp-key-change-immediately')
...
def emergency_auth_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if 'user_id' in session and session.get('authenticated', False):
            logger.info(f"✅ Session auth success for user {session['user_id']}")
            return f(*args, **kwargs)
        ...
        if auth_header and auth_header.startswith('Bearer '):
            ...
            try:
                ...
                if payload.get('exp', 0) > time.time():
                    logger.info(f"✅ Token auth success for user {payload.get('user_id')}")
                    return f(*args, **kwargs)
                else:
        ...
        logger.error(f"❌ Authentication failed for {request.endpoint} from {request.remote_addr}")
        return jsonify({
...
class EmergencyInputValidator:
        ...
        if isinstance(input_data, str):
            ...
            for pattern in dangerous_patterns:
                if pattern.lower() in input_data.lower():
                    ...
                    raise ValueError(f"Invalid input detected: contains dangerous pattern")
    ...
    def validate_api_input(data):
        ...
        if isinstance(data, dict):
            ...
            data = [EmergencyInputValidator.sanitize_input(item) for item in data]
        else:
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            if request.is_json and request.json:
                request.json = EmergencyInputValidator.validate_api_input(request.json)
            
            # Validate form data
            if request.form:
                validated_form = {}
                for key, value in request.form.items():
                    validated_form[key] = EmergencyInputValidator.sanitize_input(value)
            ...
            if request.args:
                ...
                for key, value in request.args.items():
                    validated_args[key] = EmergencyInputValidator.sanitize_input(value)
...
def create_emergency_token(user_id, is_admin=False):
    ...
    return jwt.encode(payload, os.environ.get('EMERGENCY_AUTH_SECRET', 'emergency-temp-key'), algorithm='HS256')
...
def emergency_login_endpoint():
    ...
    try:
        ...
        if username == 'emergency_admin' and password == os.environ.get('EMERGENCY_PASSWORD', 'emergency123'):
...
def configure_emergency_sessions(app):
    ...
    app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(hours=1)  # 1 hour sessions
    app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'emergency-session-key-change-immediately')
