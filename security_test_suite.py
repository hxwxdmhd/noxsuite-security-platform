...
class SecurityTestSuite:
    ...
    def setup(self):
        ...
        self.user_token = self.authenticate(NORMAL_USER_EMAIL, NORMAL_USER_PASSWORD)
    ...
    def test_jwt_security(self):
        ...
        try:
            ...
            response = requests.get(f"{self.api_url}/api/auth/me", headers={
    ...
    def test_mfa_security(self):
        ...
        response = requests.get(f"{self.api_url}/api/users/mfa-enabled", headers={
        ...
        self.print_result("API requires MFA for MFA-enabled user", requires_mfa)
        ...
        if requires_mfa:
            ...
            for i in range(attempts):
                response = requests.post(f"{self.api_url}/api/auth/verify-mfa", json={
            ...
            self.print_result("MFA protected against brute force", brute_force_protected)
            results.append(("MFA brute force protection", brute_force_protected))
    ...
    def test_rbac_security(self):
        ...
        response = requests.get(f"{self.api_url}/api/admin/users", headers={
            "Authorization": f"Bearer {self.user_token}"
        })
        
        access_control_enforced = response.status_code in [401, 403]
        self.print_result("Access control enforced for admin resources", access_control_enforced)
        ...
        if response.status_code == 200:
            ...
            response = requests.post(f"{self.api_url}/api/auth/roles", json={
            ...
            self.print_result("Users cannot self-elevate privileges", role_protection)
            results.append(("Privilege escalation protection", role_protection))
        else:
    ...
    def test_session_security(self):
        ...
        response = requests.post(f"{self.api_url}/api/auth/logout", headers={
        ...
        self.print_result("Session properly invalidated after logout", session_invalidated)
    ...
    def test_input_validation(self):
        ...
        sql_injection_attempts = [
            ...
            {"email": "admin@noxsuite.local'; DROP TABLE users; --", "password": "password"}
        ]
        
        injection_blocked = True
        for attempt in sql_injection_attempts:
            response = requests.post(f"{self.api_url}/api/auth/login", json=attempt)
            if response.status_code == 200:
        ...
        response = requests.post(f"{self.api_url}/api/users", json={
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            # Get user details
            response = requests.get(f"{self.api_url}/api/users/{user_id}", headers={
                "Authorization": f"Bearer {self.admin_token}"
            })
            ...
            if response.status_code == 200:
                ...
                xss_prevented = xss_payload not in username or username != f"user_{xss_payload}"
            ...
            xss_prevented = True
            self.print_result("XSS attempt rejected by validation", xss_prevented)
            results.append(("XSS prevention", xss_prevented))
    ...
    def test_rate_limiting(self):
        ...
        def make_request():
            response = requests.post(f"{self.api_url}/api/auth/login", json=request_data)
            return response.status_code
        
        with ThreadPoolExecutor(max_workers=10) as executor:
            status_codes = list(executor.map(lambda _: make_request(), range(num_requests)))
        ...
        def make_api_request():
            response = requests.get(f"{self.api_url}/api/users", headers={
                "Authorization": f"Bearer {self.admin_token}"
            })
            return response.status_code
        
        with ThreadPoolExecutor(max_workers=10) as executor:
            status_codes = list(executor.map(lambda _: make_api_request(), range(num_requests)))
    ...
    def print_summary(self):
        ...
        for category, tests in self.results.items():
            ...
            if category_total > 0:
                ...
                print(f"{category}: {category_passed}/{category_total} passed ({success_rate:.2f}%)")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="NoxSuite Security Test Suite")
    parser.add_argument("--url", type=str, default=API_URL, help="API URL")
    parser.add_argument("--admin-email", type=str, default=ADMIN_EMAIL, help="Admin email")
    parser.add_argument("--admin-password", type=str, default=ADMIN_PASSWORD, help="Admin password")
    parser.add_argument("--user-email", type=str, default=NORMAL_USER_EMAIL, help="Normal user email")
    parser.add_argument("--user-password", type=str, default=NORMAL_USER_PASSWORD, help="Normal user password")
