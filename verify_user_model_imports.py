...
def verify_user_model_imports():
    ...
    try:
        ...
        if callable(getattr(User(password_hash="$2b$12$test"), "verify_password", None)):
...
def verify_fastapi_imports():
    ...
    try:
        ...
        if User:
            ...
            verify_password = getattr(User, "verify_password", None)
            if verify_password:
                logger.info(f"✅ verify_password method found in FastAPI User model: {verify_password}")
            else:
                logger.error("❌ verify_password method not found in FastAPI User model")
                return False
        else:
...
def verify_rbac_mfa_imports():
    ...
    try:
        ...
        if register_func:
            logger.info(f"✅ register_rbac_mfa_routes function found: {register_func}")
        else:
            logger.error("❌ register_rbac_mfa_routes function not found in RBAC/MFA extension")
            return False
...
if __name__ == "__main__":
    ...
    for module in ["mariadb_dev_setup", "noxsuite_fastapi_server", "rbac_mfa_extension"]:
    ...
    if user_model_ok and fastapi_ok and rbac_mfa_ok and password_ok:
        ...
        print("You should now be able to restart the FastAPI server and authentication should work.")
    else:
