...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(
        "deployment.log"), logging.StreamHandler(sys.stdout)],
)
...
class NoxSuiteDeploymentManager:
    ...
    def build_images(self) -> bool:
        ...
        frontend_build = self.run_command(
            "npm run build", cwd=str(self.project_root / "frontend"), timeout=600
        )
    ...
    def deploy_infrastructure(self) -> bool:
        ...
        infrastructure_services = ["postgresql",
    ...
    def run_health_checks(self) -> bool:
        ...
        for service, endpoint in health_endpoints.items():
            ...
            health_result = self.run_command(
                f'curl -f -s -o /dev/null -w "%{{http_code}}" {endpoint}', timeout=30
            )
    ...
    def verify_deployment(self) -> bool:
        ...
        if running_services >= expected_services * 0.8:  # 80% threshold
            logger.info(
                f"✅ Deployment verified: {running_services}/{expected_services} services running"
            )
            self.deployment_status["overall_status"] = "completed"
            return True
        else:
            logger.error(
                f"❌ Deployment verification failed: {running_services}/{expected_services} services running"
            )
    ...
    def deploy(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "🎉 NoxSuite Ultimate deployment completed successfully!")
    ...
    def print_deployment_summary(self):
        ...
        logger.info("\n📚 Useful Commands:")
        logger.info(
            "   🔍 Check status: docker-compose -f docker-compose.ultimate.yml ps"
        )
        logger.info(
            "   📋 View logs: docker-compose -f docker-compose.ultimate.yml logs [service]"
        )
        logger.info(
            "   🛑 Stop all: docker-compose -f docker-compose.ultimate.yml down"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NoxSuite Ultimate Deployment Manager")
    parser.add_argument("--config", help="Custom configuration file path")
    parser.add_argument("--rollback", action="store_true",
                        help="Rollback deployment")
    parser.add_argument(
        "--environment", default="production", help="Deployment environment"
    )
