...
class EnhancedMonitoringDashboard:
    ...
    def setup_overview_tab(self):
        ...
        status_frame = ttk.LabelFrame(
            overview_frame, text="System Status", padding=10)
    ...
    def setup_incidents_tab(self):
        ...
        # Incidents table
        incidents_table_frame = ttk.LabelFrame(
            incidents_frame, text="Recent Incidents", padding=10
        )
        ...
        self.incidents_tree = ttk.Treeview(
            incidents_table_frame, columns=columns, show="headings", height=15
        )
        ...
        incidents_scrollbar = ttk.Scrollbar(
            incidents_table_frame, orient="vertical", command=self.incidents_tree.yview
        )
        ...
        details_frame = ttk.LabelFrame(
            incidents_frame, text="Incident Details", padding=10
        )
        details_frame.pack(fill="x", padx=10, pady=5)

        self.incident_details = scrolledtext.ScrolledText(
            details_frame, height=6, font=("Consolas", 9), bg="#2d2d2d", fg="#ff4444"
        )
    ...
    def setup_logs_tab(self):
        ...
        # Log display
        logs_display_frame = ttk.LabelFrame(
            logs_frame, text="System Logs", padding=10)
        logs_display_frame.pack(fill="both", expand=True, padx=10, pady=5)

        self.logs_text = scrolledtext.ScrolledText(
            logs_display_frame, font=("Consolas", 9), bg="#2d2d2d", fg="#ffffff"
        )
        ...
        ttk.Button(
            controls_frame, text="🗑️ Clear Display", command=self.clear_logs
        ).pack(side="left", padx=5)
    ...
    def setup_patterns_tab(self):
        ...
        # Patterns list
        patterns_list_frame = ttk.LabelFrame(
            patterns_frame, text="Active Monitoring Patterns", padding=10
        )
        ...
        self.patterns_tree = ttk.Treeview(
            patterns_list_frame, columns=pattern_columns, show="headings", height=12
        )
        ...
        # Pattern details
        pattern_details_frame = ttk.LabelFrame(
            patterns_frame, text="Pattern Details", padding=10
        )
    ...
    def setup_control_buttons(self, parent):
        ...
        ttk.Button(
            control_frame, text="📊 Generate Report", command=self.generate_report
        ).pack(side="left", padx=5)
        ttk.Button(
            control_frame, text="🔄 Refresh Data", command=self.refresh_data
        ).pack(side="left", padx=5)
        ttk.Button(control_frame, text="⚙️ Settings", command=self.open_settings).pack(
            side="left", padx=5
        )
    ...
    def update_status(self):
        ...
        try:
            ...
            self.root.after(
                0,
                lambda: self.status_labels["monitoring_status"].configure(
                    text="✅ ACTIVE"
                ),
            )
            self.root.after(
                0,
                lambda: self.status_labels["memory_status"].configure(
                    text=f"📊 {health.memory_usage_percent:.1f}%"
                ),
            )
            self.root.after(
                0,
                lambda: self.status_labels["cpu_status"].configure(
                    text=f"⚡ {health.cpu_usage_percent:.1f}%"
                ),
            )
            self.root.after(
                0,
                lambda: self.status_labels["api_status"].configure(
                    text=f"🌐 {health.api_response_time_ms:.1f}ms"
                ),
            )
            self.root.after(
                0,
                lambda: self.status_labels["uptime_status"].configure(
                    text=f"⏰ {health.uptime_hours:.1f}h"
                ),
            )
...
"""
            ...
            self.root.after(
                0, lambda: self.update_text_widget(
                    self.health_text, health_summary)
            )
    ...
    def update_plots(self):
        ...
        try:
            ...
            # Plot data
            self.ax1.plot(
                self.time_data, self.memory_data, color="#ff4444", linewidth=2
            )
            self.ax1.set_title("Memory Usage (%)", color="white")
            self.ax1.set_ylim(0, 100)

            self.ax2.plot(self.time_data, self.cpu_data,
                          color="#44ff44", linewidth=2)
            self.ax2.set_title("CPU Usage (%)", color="white")
            self.ax2.set_ylim(0, 100)

            self.ax3.plot(
                self.time_data, self.api_response_data, color="#4444ff", linewidth=2
            )
            ...
            if len(self.memory_data) > 0 and len(self.cpu_data) > 0:
                load_data = [
                    (m + c) / 2 for m, c in zip(self.memory_data, self.cpu_data)
                ]
                self.ax4.plot(self.time_data, load_data,
    ...
    def update_incidents(self):
        ...
        try:
            ...
            self.root.after(
                0,
                lambda: self.incidents_tree.delete(
                    *self.incidents_tree.get_children()),
            )

            # Add incidents
            # Last 20 incidents
            for incident in self.monitoring_system.incidents[-20:]:
                ...
                self.root.after(
                    0,
                    lambda i=incident, t=incident_time, e=emoji: self.incidents_tree.insert(
                        "",
                        0,
                        values=(
                            t,
                            i.id,
                            f"{e} {i.severity.value}",
                            i.category.value,
                            i.message[:50] + "...",
                        ),
                    ),
            ...
            self.root.after(
                0,
                lambda: self.status_labels["incidents_status"].configure(
                    text=f"🚨 {incident_count} total"
                ),
            )
    ...
    def on_incident_select(self, event):
        ...
        try:
            ...
            incident = next(
                (i for i in self.monitoring_system.incidents if i.id == incident_id),
                None,
            )
