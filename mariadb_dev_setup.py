...
class MariaDBDevSetup:
        ...
        try:
            ...
            admin_role = Role(
                ...
                permissions=[
                    ...
                    "role.manage", "audit.read", "system.admin"
                ]
            )
            ...
            if not db.query(User).filter(User.username == "admin").first():
                admin_password = bcrypt.hashpw("AdminPassword123!".encode(), bcrypt.gensalt()).decode()
                admin_user = User(
                    username="admin",
                ...
                # Assign admin role
                admin_role_record = db.query(Role).filter(Role.name == "admin").first()
            ...
            if not db.query(User).filter(User.username == "mfa_test").first():
                mfa_password = bcrypt.hashpw("MfaUser123!".encode(), bcrypt.gensalt()).decode()
                mfa_user = User(
                    username="mfa_test",
    ...
    def health_check(self):
        ...
        try:
            ...
            logger.info(f"✅ Database health check: {user_count} users, {role_count} roles")
            return True
...
__all__ = ['User', 'Role', 'UserRole', 'UserSession', 'AuditLog', 'Base', 'get_db_session', 'MariaDBDevSetup']
