...
class NoxSuiteSystemValidator:
        ...
        for service_key, service_info in self.services.items():
            try:
                response = requests.get(
                    f"{service_info['url']}/health", timeout=10)
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"✅ {service_info['name']} health check passed")
                else:
                    self.test_results[f"{service_key}_health"] = {
                        "status": "FAIL",
                        "response_code": response.status_code,
                    ...
                    logger.warning(
                        f"⚠️ {service_info['name']} health check failed: {response.status_code}"
                    )
                ...
                logger.error(
                    f"❌ {service_info['name']} health check error: {e}")
    ...
    def test_service_integration(self) -> None:
        ...
        try:
            ...
            langflow_response = requests.get(
                "http://localhost:7860/health", timeout=5)
        ...
        try:
            aethercore_response = requests.get(
                "http://localhost:8001/models", timeout=5
            )
            mcp_response = requests.get(
                "http://localhost:8000/contexts", timeout=5)
    ...
    def test_performance(self) -> None:
        ...
        for service_key, service_info in self.services.items():
            try:
                start_time = time.time()
                response = requests.get(
                    f"{service_info['url']}/health", timeout=5)
                ...
                if response_time < 1.0:  # Less than 1 second is good
                    logger.info(
                        f"✅ {service_info['name']} performance: {response_time*1000:.2f}ms"
                    )
                else:
                    logger.warning(
                        f"⚠️ {service_info['name']} slow response: {response_time*1000:.2f}ms"
                    )
    ...
    def test_langflow_readiness(self) -> None:
        ...
        try:
            ...
            health_response = requests.get(
                "http://localhost:7860/health", timeout=10)
            flows_response = requests.get(
                "http://localhost:7860/api/flows", timeout=10)
    ...
    def test_mcp_compliance(self) -> None:
        ...
        try:
            ...
            protocol_response = requests.get(
                "http://localhost:8000/protocol", timeout=5
            )
            contexts_response = requests.get(
                "http://localhost:8000/contexts", timeout=5
            )
                ...
                try:
                    ...
                    if "version" in protocol_data:
                        compliance_issues.append(
                            f"✅ MCP Protocol version: {protocol_data.get('version', 'unknown')}"
                        )
                    else:
                        compliance_issues.append(
                            "⚠️ Protocol version not specified")
                except:
                    compliance_issues.append("⚠️ Protocol response not JSON")
            else:
                mcp_compliant = False
                compliance_issues.append(
                    f"❌ Protocol endpoint failed: {protocol_response.status_code}"
                )
    ...
    def test_docker_services(self) -> None:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"✅ Docker services check passed ({services_running} NoxSuite containers)"
                )
            else:
                self.integration_results["docker_services"] = {
                    "status": "WARN",
    ...
    def generate_validation_report(self, total_time: float) -> Dict[str, Any]:
        ...
        passed_tests = len(
            [r for r in self.test_results.values() if r.get("status") == "PASS"]
        )
        failed_tests = len(
            [r for r in self.test_results.values() if r.get("status") == "FAIL"]
        )
        warning_tests = len(
            [r for r in self.test_results.values() if r.get("status") == "WARN"]
        )

        # Count integrations
        total_integrations = len(self.integration_results)
        passed_integrations = len(
            [r for r in self.integration_results.values() if r.get("status")
             == "PASS"]
        )
        ...
        health_score = (
            ((passed_tests + passed_integrations) /
             (total_tests + total_integrations))
            * 100
            if (total_tests + total_integrations) > 0
            else 0
        )
        ...
        report = {
            ...
            "service_availability": {
                service_key: {
                    "available": service_key + "_health" in self.test_results
                    and self.test_results[service_key + "_health"].get("status")
                    == "PASS",
            ...
            "langflow_integration_status": {
                "ready": self.integration_results.get("langflow_readiness", {}).get(
                    "ready_for_integration", False
                ),
                "mcp_compatible": self.integration_results.get(
                    "mcp_compliance", {}
                ).get("compliant", False),
                "integration_possible": (
                    self.integration_results.get("langflow_readiness", {}).get(
                        "ready_for_integration", False
                    )
                    and self.integration_results.get("mcp_compliance", {}).get(
                        "compliant", False
                    )
                ),
            },
            ...
            if health_key in self.test_results:
                if self.test_results[health_key].get("status") == "FAIL":
                    recommendations.append(
                        f"🔧 Restart {service_info['name']} - health check failed"
                    )
                elif self.test_results[health_key].get("response_time", 0) > 1.0:
                    recommendations.append(
                        f"⚡ Optimize {service_info['name']} - slow response time"
                    )

        # Check integration status
        if not self.integration_results.get("langflow_readiness", {}).get(
            "ready_for_integration", False
        ):
            recommendations.append(
                "🔗 Complete Langflow configuration for MCP integration"
            )

        if not self.integration_results.get("mcp_compliance", {}).get(
            "compliant", False
        ):
            recommendations.append("📋 Verify MCP protocol implementation")

        # Check Docker services
        if not self.integration_results.get("docker_services", {}).get(
            "docker_available", False
        ):
            recommendations.append(
                "🐳 Start Docker services for full functionality")

        # If no issues found
        if not recommendations:
            recommendations.append(
                "✅ System is healthy - consider running performance optimization"
            )
            recommendations.append(
                "📊 Monitor system metrics for continued optimal performance"
            )
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info(
            f"Status: {report['validation_summary']['status_emoji']} {report['validation_summary']['system_status']}"
        )
        logger.info(
            f"Health Score: {report['validation_summary']['health_score']}%")
        logger.info(
            f"Total Time: {report['validation_summary']['total_time_seconds']}s"
        )
        logger.info(
            f"Tests Passed: {report['service_tests']['passed']}/{report['service_tests']['total_tests']}"
        )
        logger.info(
            f"Integrations Passed: {report['integration_tests']['passed']}/{report['integration_tests']['total_integrations']}"
        )

        logger.info("\n🔗 INTEGRATION STATUS:")
        langflow_status = report["langflow_integration_status"]
        logger.info(
            f"  Langflow Ready: {'✅' if langflow_status['ready'] else '❌'}")
        logger.info(
            f"  MCP Compatible: {'✅' if langflow_status['mcp_compatible'] else '❌'}"
        )
        logger.info(
            f"  Integration Possible: {'✅' if langflow_status['integration_possible'] else '❌'}"
        )
