...
class PythonVersionAuditor:
    ...
    def scan_version_references(self):
        ...
        exclude_dirs = {'.git', '__pycache__', 'node_modules', 'venv', '.venv', 'archived'}
        include_extensions = {'.py', '.txt', '.md', '.yml', '.yaml', '.json', '.cfg', '.ini', '.dockerfile'}
        ...
        for root, dirs, files in os.walk('.'):
            ...
            for file in files:
                ...
                if file_path.suffix.lower() not in include_extensions and not file_path.name.lower().startswith('dockerfile'):
                    continue
                ...
                try:
                    ...
                    for pattern in version_patterns:
                        ...
                        for match in matches:
                            version = match.group(1) if len(match.groups()) == 1 else f"{match.group(1)}.{match.group(2)}"
                            
                            self.audit_results["version_references"].append({
                                "file": str(file_path),
                                "line_context": content.split('\n')[content[:match.start()].count('\n')],
                                "version_found": version,
                                "needs_update": version != self.baseline_version and not version.startswith("3.12")
                            })
    ...
    def check_deprecated_imports(self):
        ...
        for root, dirs, files in os.walk('.'):
            dirs[:] = [d for d in dirs if d not in {'.git', '__pycache__', 'archived'}]
                    ...
                    for issue_type, pattern in deprecated_patterns.items():
                        ...
                        for match in matches:
                            ...
                            self.audit_results["deprecated_imports"].append({
                                ...
                                "recommendation": self.get_fix_recommendation(issue_type)
                            })
    ...
    def fix_docker_versions(self):
        ...
        for root, dirs, files in os.walk('.'):
            for file in files:
                if 'dockerfile' in file.lower() or file.lower().startswith('dockerfile'):
    ...
    def run_comprehensive_audit(self):
        ...
        # Generate summary
        total_issues = len(self.audit_results["version_references"]) + len(self.audit_results["deprecated_imports"])
        needs_update = sum(1 for ref in self.audit_results["version_references"] if ref["needs_update"])
        ...
        print(f"✅ Version references found: {len(self.audit_results['version_references'])}")
        print(f"⚠️  Outdated references: {needs_update}")
        print(f"⚠️  Deprecated imports: {len(self.audit_results['deprecated_imports'])}")
...
if __name__ == "__main__":
    ...
    outdated_refs = sum(1 for ref in results["version_references"] if ref["needs_update"])
