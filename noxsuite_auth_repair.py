...
class NoxSuiteAuthRepair:
    ...
    def check_api_server(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"⚠️ API server returned non-200 status: {response.status_code}"
                )
                self.repair_results["api_server_status"] = {
                    "status": "warning",
    ...
    def check_database_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Database connection successful (SQLite {version[0]})")
            self.repair_results["database_repairs"].append(
                {
                    "action": "check_connection",
                    "success": True,
    ...
    def check_users_table(self) -> List[Dict]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT name FROM mariadb_master WHERE type='table' AND name='users';"
            )
            ...
            if missing_columns:
                logger.error(
                    f"❌ Missing columns in users table: {', '.join(missing_columns)}"
                )
                self.repair_results["database_repairs"].append(
                    {
                        "action": "check_users_table",
            ...
            # Get user data
            cursor.execute(
                "SELECT id, username, email, password_hash, mfa_enabled FROM users"
            )
            users = []
    ...
    def fix_admin_password(self) -> bool:
        ...
        try:
            ...
            new_password = (
                "admin123"  # In production, would use a secure random password
            )
    ...
    def fix_user_passwords(self) -> bool:
        ...
        try:
            ...
            for user_id, username in users:
                ...
                new_password = (
                    "user123"  # In production, would use unique secure random passwords
                )
                new_hash = bcrypt.hashpw(
                    new_password.encode("utf-8"), bcrypt.gensalt()
                ).decode("utf-8")
    ...
    def _test_login(self, credentials: Dict, user_type: str) -> bool:
        ...
        try:
            response = requests.post(
                f"{API_URL}/auth/login", json=credentials, timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                if "access_token" in data and "refresh_token" in data:
                    ...
                    self.repair_results["authentication_tests"][
                        f"{user_type}_login"
                    ] = {
                        "success": True,
                    ...
                    logger.error(
                        f"❌ {user_type.capitalize()} login response missing tokens"
                    )
                    self.repair_results["authentication_tests"][
                        f"{user_type}_login"
                    ] = {
                ...
                logger.error(
                    f"❌ {user_type.capitalize()} login failed: {response.status_code}"
                )
            ...
            logger.error(
                f"❌ {user_type.capitalize()} login request failed: {e}")
            self.repair_results["authentication_tests"][f"{user_type}_login"] = {
                "success": False,
    ...
    def restart_api_server(self) -> bool:
        ...
        logger.info(
            "⚠️ API server restart requested - this is a mock implementation")
        self.repair_results["repair_actions"].append(
            {
                "type": "server_restart",
                "timestamp": datetime.now().isoformat(),
    ...
    def run_comprehensive_repair(self) -> Dict:
        ...
        success_count = sum(
            1
            for item in self.repair_results["database_repairs"]
