...
class DirectAPIClient:
    ...
    def __init__(self):
        ...
        self.admin_creds = {
            "email": "admin@noxsuite.local", "password": "Admin123!"}
        self.user_creds = {
            "email": "user@noxsuite.local", "password": "User123!"}
        self.tests_passed = 0
        self.tests_failed = 0
    ...
    def test_admin_login(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/auth/login", json=self.admin_creds
            )
            ...
            try:
                ...
                logger.info(
                    f"Response JSON: {json.dumps(resp_json, indent=2)}")
    ...
    def test_user_login(self):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/auth/login", json=self.user_creds
            )
    ...
    def run_all_tests(self):
        ...
        logger.info("\n===== Test Summary =====")
        logger.info(
            f"Tests passed: {self.tests_passed}/{total} ({pass_rate:.1f}%)")
        logger.info(
            f"Tests failed: {self.tests_failed}/{total} ({100 - pass_rate:.1f}%)"
        )
