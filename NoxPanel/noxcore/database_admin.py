...
class DatabaseAdmin:
                ...
                "user_summary": {
                    ...
                    "recent_logins": len(
                        [
                            ...
                            for u in users
                            if u["last_login"]
                            and datetime.fromisoformat(u["last_login"])
    ...
    def _generate_recommendations(
        self, status: Dict[str, Any], health: Dict[str, Any]
    ) -> List[str]:
        ...
        if status.get("database_size_mb", 0) > 500:
            recommendations.append(
                "Database size is large (>500MB). Consider archiving old data."
            )
        ...
        if status.get("recent_activity_24h", 0) == 0:
            recommendations.append(
                "No recent activity detected. Check if the system is being used."
            )

        # Check for pending migrations
        if status.get("migrations_pending", 0) > 0:
            recommendations.append(
                "Pending database migrations detected. Apply migrations."
            )

        # Check integrity
        if health.get("health_status") != "healthy":
            recommendations.append(
                "Database integrity issues detected. Run optimization or restore from backup."
            )

        # Performance recommendations
        if status.get("table_counts", {}).get("audit_logs", 0) > 100000:
            recommendations.append(
                "Large number of audit logs. Consider enabling automatic cleanup."
            )

        return (
            recommendations
            if recommendations
            else ["Database is healthy and well-maintained."]
        )
...
def main():
    ...
    # Reset password
    reset_password_parser = user_subparsers.add_parser(
    ...
    import_parser = subparsers.add_parser(
        "import-knowledge", help="Import knowledge items"
    )
    ...
    try:
        ...
        if args.command == "status":
            ...
            if args.user_command == "create":
                ...
                for user in users:
                    logger.info(
                        f"{user['id']:3d} | {user['username']:20s} | {user['role']:10s} | {user['active']}"
                    )
