...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.summaries: Dict[str, deque] = defaultdict(
    ...
    def _init_default_metrics(self):
        ...
        # Server metrics
        self.register_counter("heimnetz_server_requests_total",
                              "Total number of server requests")
        self.register_histogram(
            "heimnetz_server_request_duration_seconds", "Server request duration in seconds")
        self.register_gauge("heimnetz_server_active_connections",
                            "Number of active connections")
        self.register_gauge("heimnetz_server_memory_usage_bytes",
                            "Server memory usage in bytes")
        self.register_gauge("heimnetz_server_cpu_usage_percent",
                            "Server CPU usage percentage")

        # Plugin metrics
        self.register_counter("heimnetz_plugin_executions_total",
                              "Total number of plugin executions")
        self.register_histogram(
            "heimnetz_plugin_execution_duration_seconds", "Plugin execution duration in seconds")
        self.register_gauge("heimnetz_plugin_memory_usage_bytes",
                            "Plugin memory usage in bytes")
        self.register_counter("heimnetz_plugin_errors_total",
                              "Total number of plugin errors")
        self.register_gauge("heimnetz_plugin_active_count",
                            "Number of active plugins")

        # System metrics
        self.register_gauge("heimnetz_system_load_average",
                            "System load average")
        self.register_gauge("heimnetz_system_disk_usage_percent",
                            "System disk usage percentage")
        self.register_gauge(
            "heimnetz_system_network_bytes_sent", "Network bytes sent")
        self.register_gauge(
            "heimnetz_system_network_bytes_received", "Network bytes received")

    def register_counter(self, name: str, help: str = "") -> None:
        """Register a counter metric"""
        with self.lock:
            self.metrics[name] = Metric(
                name, MetricType.COUNTER, 0.0, help=help)
    ...
    def register_histogram(self, name: str, help: str = "") -> None:
        ...
        with self.lock:
            self.metrics[name] = Metric(
                name, MetricType.HISTOGRAM, 0.0, help=help)
    ...
    def register_summary(self, name: str, help: str = "") -> None:
        ...
        with self.lock:
            self.metrics[name] = Metric(
                name, MetricType.SUMMARY, 0.0, help=help)
    ...
    def collect_system_metrics(self) -> None:
        ...
        try:
            ...
            self.set_gauge("heimnetz_system_network_bytes_sent",
                           net_io.bytes_sent)
            self.set_gauge(
                "heimnetz_system_network_bytes_received", net_io.bytes_recv)
            ...
            self.logger.warning(
                "psutil not available for system metrics collection")
        ...
        def collect_loop():
            while True:
                try:
                    ...
                    self.logger.error(
                        f"Background metrics collection error: {e}")
                    time.sleep(interval)

        thread = threading.Thread(target=collect_loop, daemon=True)
        thread.start()
        self.logger.info(
            f"Started background metrics collection (interval: {interval}s)")
...
class PerformanceMonitor:
    ...
    def track_request(self, duration: float, success: bool = True) -> None:
        ...
        self.metrics.observe_histogram(
            "heimnetz_server_request_duration_seconds", duration)
    ...
    def track_plugin_execution(self, plugin_name: str, duration: float,
        ...
        self.metrics.increment_counter(
            "heimnetz_plugin_executions_total", labels=labels)
        self.metrics.observe_histogram(
            "heimnetz_plugin_execution_duration_seconds", duration, labels=labels)
        self.metrics.set_gauge(
            "heimnetz_plugin_memory_usage_bytes", memory_used, labels=labels)

        if not success:
            self.metrics.increment_counter(
                "heimnetz_plugin_errors_total", labels=labels)
...
class StressTestSuite:
    ...
    def run_load_test(self, target_function: Callable, concurrent_requests: int = 100,
        ...
        self.logger.info(
            f"Starting load test: {concurrent_requests} concurrent requests for {duration}s")
        ...
        if results["response_times"]:
            ...
            results["p95_response_time"] = sorted(
                times)[int(len(times) * 0.95)]
            results["p99_response_time"] = sorted(
                times)[int(len(times) * 0.99)]

        results["success_rate"] = results["successful_requests"] / \
            results["total_requests"] if results["total_requests"] > 0 else 0
        results["requests_per_second"] = results["total_requests"] / duration

        self.test_results["load_test"] = results
        self.logger.info(
            f"Load test completed: {results['success_rate']:.2%} success rate, {results['requests_per_second']:.2f} RPS")
    ...
    def run_memory_stress_test(self, target_function: Callable,
        ...
        self.logger.info(
            f"Starting memory stress test (max: {max_memory_mb}MB)")
        ...
        for i in range(1000):
            try:
                ...
                if current_memory > max_memory_mb * 1024 * 1024:
                    self.logger.warning(
                        f"Memory limit exceeded: {current_memory / 1024 / 1024:.2f}MB")
                    break
        ...
        self.logger.info(
            f"Memory stress test completed: {results['executions']} executions, peak: {results['peak_memory'] / 1024 / 1024:.2f}MB")
    ...
    def run_cpu_stress_test(self, target_function: Callable,
        ...
        if results["cpu_usage_samples"]:
            results["avg_cpu_usage"] = sum(
                results["cpu_usage_samples"]) / len(results["cpu_usage_samples"])

        self.test_results["cpu_stress"] = results
        self.logger.info(
            f"CPU stress test completed: {results['executions']} executions, avg CPU: {results['avg_cpu_usage']:.2f}%")
...
def main():
    ...
    try:
        ...
        for i in range(5):
            ...
            duration = time.time() - start_time
            performance.track_plugin_execution(
                "test_plugin", duration, 1024 * 1024)
        ...
        print(prometheus_output[:500] +
              "..." if len(prometheus_output) > 500 else prometheus_output)
        ...
        print("\nRunning stress test...")
        load_results = stress_tester.run_load_test(
            dummy_function, concurrent_requests=10, duration=5)
        print(
            f"Load test results: {load_results['success_rate']:.2%} success rate")
