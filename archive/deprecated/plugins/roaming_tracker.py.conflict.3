...
class RoamingTracker:
    ...
    def load_history(self):
        ...
        try:
            if self.history_file.exists():
                ...
                logger.info(
                    f"Loaded {len(self.events)} roaming events and {len(self.device_profiles)} device profiles")
        ...
        for mac, current_info in current_devices.items():
            ...
            if mac in self.active_sessions:
                ...
                if active_session.router_name != current_router:
                    ...
                    active_session.add_signal_measurement(
                        device.signal_strength)

            else:
                # New device connection
                self._handle_device_connect(
                    device, current_router, current_time)

        # Check for disconnected devices
        disconnected_devices = set(
            self.active_sessions.keys()) - set(current_devices.keys())
    ...
    def _handle_device_connect(self, device: Any, router_name: str, timestamp: datetime):
        ...
        logger.info(
            f"Device {device.hostname} ({mac}) connected to {router_name}")
    ...
    def _handle_device_disconnect(self, mac: str, timestamp: datetime):
        ...
        if mac in self.active_sessions:
            ...
            logger.info(
                f"Device {session.device_hostname} ({mac}) disconnected from {session.router_name}")
    ...
    def _handle_roaming_event(self, device: Any, from_router: str, to_router: str, timestamp: datetime):
        ...
        if mac in self.active_sessions:
            ...
            trigger_reason = self._determine_roaming_trigger(
                old_session, device.signal_strength)
            ...
            logger.info(
                f"Device {device.hostname} ({mac}) roamed from {from_router} to {to_router} ({trigger_reason})")
    ...
    def _update_device_profile(self, mac: str, hostname: str, router_name: str):
        ...
        device_events = [e for e in self.events if e.device_mac ==
                         mac and e.event_type == "roam"]
        if device_events:
            # Calculate roams per hour over last 24 hours
            recent_events = [
                e for e in device_events if e.timestamp > datetime.now() - timedelta(hours=24)]
        ...
        device_sessions = [
            s for s in self.active_sessions.values() if s.device_mac == mac]
        if device_sessions:
    ...
    def _update_statistics(self):
        ...
        self.stats['total_roams'] = len(
            [e for e in self.events if e.event_type == "roam"])
        self.stats['total_devices'] = len(self.device_profiles)
        self.stats['active_devices'] = len(self.active_sessions)
        self.stats['roaming_devices'] = len(
            [p for p in self.device_profiles.values() if p.roaming_frequency > 0])
    ...
    def cleanup_old_data(self):
        ...
        recent_macs = set(event.device_mac for event in self.events if event.timestamp >
        ...
        logger.info(
            f"Cleaned up {old_count - len(self.events)} old events and {profiles_removed} inactive profiles")
...
async def test_roaming_tracker():
    ...
    print(
        f"✅ Initial connections: {len(tracker.active_sessions)} active sessions")
    ...
    tracker.update_device_locations(devices_by_router)
    print(
        f"✅ After roaming: {len(tracker.get_roaming_events())} roaming events")

    # Get reports
    mobility_report = tracker.get_mobility_report()
    print(
        f"📊 Mobility report: {len(mobility_report['device_profiles'])} device profiles")
