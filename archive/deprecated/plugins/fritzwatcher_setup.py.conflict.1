...
def setup_keepass_config() -> Dict[str, Any]:
    ...
    use_keepass = get_yes_no(
        "Do you want to use KeePass for credential management?", True)
    ...
    if not Path(db_path).exists():
        ...
        create_db = get_yes_no(
            "Would you like to create a new KeePass database?", False)
        if create_db:
...
def setup_routers(network_info: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        # KeePass reference
        router['keepass_ref'] = get_user_input(
            f"KeePass entry name (optional)", f"FritzBox_{router['name'].replace(' ', '_')}")
...
def setup_monitoring() -> Dict[str, Any]:
    ...
    # Signal monitoring
    config['signal_monitoring'] = get_yes_no(
        "Enable signal strength monitoring?", True)
    ...
    # Guest WiFi control
    config['enable_guest_wifi_control'] = get_yes_no(
        "Enable guest WiFi control?", True)
...
def setup_web_interface() -> Dict[str, Any]:
    ...
    # Bind address
    config['bind_address'] = get_user_input(
        "Bind address (127.0.0.1 for local only)", "127.0.0.1")

    # Remote access
    config['enable_remote_access'] = get_yes_no("Enable remote access?", False)

    # Authentication
    config['authentication_required'] = get_yes_no(
...
def validate_configuration(config: Dict[str, Any]) -> List[str]:
    ...
    for router in config.get('routers', []):
        if router.get('password'):
            warnings.append(
                f"Router '{router['name']}' has hardcoded password - consider using KeePass")

    # Check KeePass setup
    keepass_config = config.get('keepass', {})
    if not keepass_config.get('enabled', False):
        warnings.append(
            "KeePass not enabled - credentials may be stored insecurely")
    ...
    if web_config.get('enabled', False):
        if web_config.get('enable_remote_access', False) and not web_config.get('authentication_required', True):
            warnings.append(
                "Remote access enabled without authentication - security risk")
...
def main():
    ...
    if config_path.exists():
        overwrite = get_yes_no(
            f"Configuration file already exists at {config_path}. Overwrite?", False)
        if not overwrite:
    ...
    if save_configuration(config, config_path):
        ...
        if web_interface.get('enabled', False):
            ...
            print(
                f"5. Web interface will be available at: http://{bind_addr}:{port}/fritzwatcher")
