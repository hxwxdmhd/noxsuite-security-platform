...
class KeePassHelper:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.use_browser_integration = self.config.get(
        ...
        logger.info(
            f"KeePass Helper initialized with database: {self.database_path}")
        ...
        if found_databases:
            ...
            if len(found_databases) == 1:
                ...
                if response in ['y', 'yes', '']:
                    ...
                    db_path = input(
                        "Please enter the full path to your KeePass database (.kdbx): ").strip()
                    if db_path and os.path.exists(db_path):
                        logger.info(
                            f"Using custom KeePass database: {db_path}")
                        return db_path
                    else:
                ...
                while True:
                    try:
                        choice = input(
                            f"Select database (1-{len(found_databases) + 1}): ").strip()
                        choice_num = int(choice)

                        if 1 <= choice_num <= len(found_databases):
                            selected_db = found_databases[choice_num - 1]
                            logger.info(
                                f"Using selected KeePass database: {selected_db}")
                            return selected_db
                            ...
                            if db_path and os.path.exists(db_path):
                                ...
                                logger.error(
                                    "Database not found or invalid path")
                                return ""
                        else:
                            print(
                                f"Invalid choice. Please enter a number between 1 and {len(found_databases) + 1}.")
                    except ValueError:
...
def test_keepass_setup():
    ...
    print(
        f"Database exists: {os.path.exists(helper.database_path) if helper.database_path else False}")
