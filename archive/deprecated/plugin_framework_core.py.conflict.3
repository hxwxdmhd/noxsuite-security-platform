...
try:
    ...
    logging.warning(
        "Enhanced Plugin Sandbox not available - running in compatibility mode")
...
class BasePlugin(ABC):
    ...
    async def _transition_state(self, new_state: PluginState) -> bool:
        ...
        if new_state in valid_transitions.get(self.state, []):
            ...
            self.logger.info(
                f"Plugin state transition: {old_state.value} -> {new_state.value}")
            return True
        else:
            self.logger.warning(
...
class PluginManager:
    ...
    def __init__(self, plugins_directory: Path = None, sandbox_config: IsolationConfig = None):
        ...
        # Sandbox integration
        self.sandbox_config = sandbox_config or IsolationConfig(
    ...
    async def load_plugin(self, plugin_path: Union[str, Path], plugin_config: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            if not metadata:
                self.logger.error(
                    f"Could not discover metadata for plugin: {plugin_path}")
                return False
            ...
            if not await self._verify_dependencies(metadata):
                self.logger.error(
                    f"Dependency verification failed for plugin: {metadata.id}")
                return False
            ...
            if not self._validate_plugin_interface(plugin_instance):
                self.logger.error(
                    f"Plugin interface validation failed: {metadata.id}")
                return False
            ...
            if not await plugin_instance._transition_state(PluginState.LOADED):
                self.logger.error(
                    f"Failed to transition plugin to loaded state: {metadata.id}")
                return False
            ...
            self.logger.info(
                f"Successfully loaded plugin: {metadata.id} v{metadata.version}")
        ...
        try:
            ...
            if plugin.state == PluginState.ACTIVE:
                if not await self.stop_plugin(plugin_id):
                    self.logger.warning(
                        f"Failed to stop plugin {plugin_id} during unload")
            ...
            try:
                ...
                self.logger.warning(
                    f"Plugin cleanup error for {plugin_id}: {e}")
            ...
            if plugin_id in self.active_sandboxes:
                try:
                    ...
                    self.logger.warning(
                        f"Sandbox cleanup error for {plugin_id}: {e}")
    ...
    async def start_plugin(self, plugin_id: str, context: PluginExecutionContext = None) -> bool:
        ...
        try:
            ...
            if plugin.metadata.sandbox_required and SANDBOX_AVAILABLE:
                ...
                context.sandbox_id = sandbox.sandbox_id if hasattr(
                    sandbox, 'sandbox_id') else plugin_id
    ...
    async def execute_plugin_operation(self, plugin_id: str, operation: str,
        ...
        try:
            ...
            self.logger.error(
                f"Error executing {operation} on plugin {plugin_id}: {e}")
    ...
    def _dict_to_metadata(self, metadata_dict: Dict[str, Any]) -> PluginMetadata:
        ...
        return PluginMetadata(
            ...
            estimated_cpu_percent=metadata_dict.get(
            ...
            for dependency in metadata.dependencies:
                if not await self._check_dependency(dependency):
                    if not dependency.optional:
                        self.logger.error(
                            f"Required dependency not satisfied: {dependency.name}")
                        return False
                    else:
                        self.logger.warning(
                            f"Optional dependency not satisfied: {dependency.name}")
    ...
    async def _check_dependency(self, dependency: PluginDependency) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Error checking dependency {dependency.name}: {e}")
            return False
    ...
    async def _create_plugin_sandbox(self, plugin: BasePlugin) -> 'EnhancedPluginSandbox':
        ...
        sandbox = EnhancedPluginSandbox(
            limits, permissions, self.sandbox_config)
        return sandbox
    ...
    async def _import_plugin(self, plugin_path: Path, metadata: PluginMetadata) -> Optional[BasePlugin]:
        ...
        try:
            if plugin_path.is_dir():
                ...
                if not main_file.exists():
                    self.logger.error(
                        f"No main plugin file found in {plugin_path}")
                    return None
                ...
                # Single file plugin
                spec = importlib.util.spec_from_file_location(
                    metadata.id, plugin_path)

            if spec is None:
                self.logger.error(
                    f"Could not create module spec for {plugin_path}")
                return None
        ...
        try:
            ...
            for method_name in required_methods:
                ...
                if not callable(method):
                    self.logger.error(
                        f"Plugin method not callable: {method_name}")
                    return False
    ...
    def get_plugin_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "plugin_states": {
                state.value: sum(
                    1 for p in self.loaded_plugins.values() if p.state == state)
    ...
    def _load_registry(self):
        ...
        try:
            if self.registry_file.exists():
                ...
                logger.info(
                    f"Loaded {len(self.registered_plugins)} plugins from registry")
...
class PluginFramework:
    ...
    def __init__(self, plugins_directory: Path = None, config: Dict[str, Any] = None):
        ...
        sandbox_config = IsolationConfig(
            level=IsolationLevel(self.config.get(
                "sandbox_isolation_level", "standard")),
            enable_real_time_monitoring=self.config.get(
...
async def main():
    ...
    if plugins:
        for plugin_id, info in plugins.items():
            print(
                f"  - {plugin_id}: {info['metadata']['name']} v{info['metadata']['version']}")
    else:
