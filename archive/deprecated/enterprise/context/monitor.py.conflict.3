...
• Bytes Received: {system_data.get('network_io', {}).get('bytes_recv', 0):,}
    ...
    def create_performance_chart(self) -> Panel:
        ...
        if len(self.metrics_history) < 2:
            ...
            response_times = [m.get("metrics", {}).get(
                "average_response_time", 0) for m in recent_metrics]
    ...
    async def collect_data(self):
        ...
        while self.is_running:
            try:
                ...
                console.print(
                    f"[bold red]Error collecting data: {str(e)}[/bold red]")
                await asyncio.sleep(self.update_interval)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="ContextForge Monitoring Dashboard")
    parser.add_argument(
        "--url", default="http://localhost:8000", help="ContextForge API URL")
