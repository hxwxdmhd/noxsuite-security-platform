...
def status(
    service: Optional[str] = typer.Argument(
        None, help="Specific service to check"),
    watch: bool = typer.Option(
        False, "--watch", "-w", help="Watch mode - continuously monitor status")
):
    ...
    with console.status("[bold green]Checking service status..."):
        if service_name:
            if service_name not in ENTERPRISE_CONFIG.services:
                console.print(
                    f"[red]❌ Service '{service_name}' not found[/red]")
                return
...
def _show_single_service_status(service_name: str):
    ...
    try:
        response = httpx.get(
            f"{service.url}{service.health_endpoint}", timeout=3.0)
...
🔌 Port: {service.port}
    ...
    console.print(
        Panel(panel_content, title=f"Service: {service_name}", border_style=color))
...
def _show_all_services_status():
    ...
    # Create status table
    table = Table(title="🏢 Heimnetz Enterprise Services Status",
...
"""
    ...
    console.print(
        Panel(metrics_content, title="System Metrics", border_style="blue"))
...
def _watch_status():
    ...
    console.print(
        "[bold green]📊 Entering watch mode... Press Ctrl+C to exit[/bold green]")

    try:
        while True:
            console.clear()
            console.print(
                f"[bold]📊 Heimnetz Enterprise Status - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}[/bold]")
            console.print("=" * 80)
...
def start(
    service: Optional[str] = typer.Argument(
        None, help="Specific service to start"),
    all_services: bool = typer.Option(
        False, "--all", "-a", help="Start all services")
):
    ...
    if all_services:
        ...
        console.print(
            "[red]❌ Please specify a service name or use --all[/red]")
...
def _start_all_services():
    ...
    console.print(
        "[bold green]🚀 Starting all enterprise services...[/bold green]")
    ...
    with Progress(
        ...
        task = progress.add_task(
            "Starting services...", total=len(ENTERPRISE_CONFIG.services))
...
def logs(
    service: Optional[str] = typer.Argument(
        None, help="Service to show logs for"),
...
def _follow_logs(log_file: Path):
    ...
    console.print(
        "[bold green]📋 Following logs... Press Ctrl+C to exit[/bold green]")
...
def health():
    ...
    console.print(
        "[bold green]🏥 Running comprehensive health check...[/bold green]")
...
def context():
    ...
    console.print(
        "[bold green]🧠 ContextForge - Model Context Protocol Server[/bold green]")
    ...
    try:
        ...
        table = Table(title="🧠 ContextForge Service Information",
                      show_header=True, header_style="bold magenta")
        ...
        table.add_row("Protocol Versions", ", ".join(
            service_info["supported_protocols"]))
        table.add_row("Supported Models", ", ".join(
            service_info["supported_models"]))
...
🔗 Healthy Endpoints: {health['components']['router']['healthy_endpoints']}/{health['components']['router']['active_endpoints']}
        ...
        console.print(
            Panel(health_content, title="ContextForge Health Status", border_style="green"))
        ...
        with Progress(
            ...
            task = progress.add_task(
                "Processing context request...", total=None)
        ...
        if test_results["test_summary"]["total_tests"] > 0:
            ...
            for result in test_results["results"][:3]:  # Show first 3 tests
                ...
                compatibility = f"{result.get('compatibility_score', 0):.1%}" if result.get(
                    'compatibility_score') else "N/A"
                test_table.add_row(
                    result["test_name"],
        ...
        console.print(
            "[red]❌ ContextForge service not available. Make sure it's properly installed.[/red]")
    except Exception as e:
...
- Modern Python libraries (typer, rich, pydantic, etc.)
    ...
    console.print(
        Panel(version_info, title="Version Information", border_style="blue"))
