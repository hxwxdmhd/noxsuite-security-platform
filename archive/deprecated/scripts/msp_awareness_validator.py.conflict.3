    ...
    def generate_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        ...
        if not validation_results["endpoints"].get("CoreServer", {}).get("health", {}).get("healthy", False):
            recommendations.append(
                "🚀 Start AetherCore MSP Server: docker-compose up -d aethercore")
            recommendations.append(
                "📊 Monitor CoreServer health: python heimnetz_cli.py aethercore health")

        # Check ContextForge integration
        if not validation_results["endpoints"].get("ContextForge", {}).get("healthy", False):
            recommendations.append(
                "🔗 Start ContextForge MCP Server: docker-compose up -d contextforge")

        # Check system resources
        if validation_results["system"]["memory"]["percent"] > 80:
            recommendations.append(
                "⚠️ High memory usage detected - consider scaling down services")

        if validation_results["system"]["cpu_percent"] > 90:
            recommendations.append(
                "⚠️ High CPU usage detected - optimize model loading")

        # Check Docker status
        if not validation_results["docker"]["docker_available"]:
            recommendations.append(
                "🐳 Docker not available - install Docker for container orchestration")

        # Check file structure
        missing_files = [
            f for f, status in validation_results["files"].items() if not status["exists"]]
        if missing_files:
            recommendations.append(
                f"📁 Missing files detected: {', '.join(missing_files[:3])}")
    ...
    async def validate_msp_protocol(self) -> Dict[str, Any]:
        ...
        with Progress(
            ...
            task1 = progress.add_task(
                "Validating service endpoints...", total=None)
            endpoints = await self.validate_endpoints()
            progress.update(task1, completed=True)

            # Check system resources
            task2 = progress.add_task(
                "Checking system resources...", total=None)
            ...
            task4 = progress.add_task(
                "Validating file structure...", total=None)
        ...
        for service, data in results["endpoints"].items():
            ...
            if isinstance(data, dict) and "health" in data:
                ...
                service_table.add_row(
                    service, codename, "Active", health_status, response_time)
                ...
                service_table.add_row(
                    service, codename, "❌ Error", "Unknown", "N/A")
...
async def main():
    ...
    results_file = Path(
        "k:/Project Heimnetz/scripts/msp_validation_results.json")
