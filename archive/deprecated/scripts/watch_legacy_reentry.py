...
class LegacyWatchdog:
    ...
    def scan_for_legacy_files(self) -> Dict[str, List[str]]:
        ...
        if found_legacy:
            print(
                f"  ⚠️  Legacy files found: {sum(len(files) for files in found_legacy.values())}")
        ...
        for category, files in current_scan.items():
            ...
            if new_files:
                ...
                for new_file in new_files:
                    self.log_alert("LEGACY_REINTRODUCED",
                                   f"New {category} file detected", new_file)
    ...
    def monitor_continuous(self, interval: int = 300):
        ...
        print(
            f"🔄 Starting continuous legacy monitoring (checking every {interval} seconds)...")
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        if command == "--scan":
            ...
            print(
                "Usage: python watch_legacy_reentry.py [--scan|--baseline|--check|--monitor [interval]|--report]")
            sys.exit(1)
