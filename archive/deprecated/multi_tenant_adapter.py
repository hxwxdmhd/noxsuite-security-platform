...
class MultiTenantAdapter:
    ...
    def init_tenant_database(self):
        """Initialize tenant database."""
        with pymysql.connect(str(self.tenant_db)) as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def create_tenant(self, tenant_name: str, tenant_type: TenantType,
        ...
        print(
            f"Created tenant: {tenant_name} (ID: {tenant_id}, Type: {tenant_type.value})")
        return tenant_id
    ...
    def validate_tenant_access(self, tenant_id: str, resource: str, action: str) -> bool:
        ...
        if tenant_id not in self.tenants:
            self.log_tenant_access(
                tenant_id, action, resource, False, "Tenant not found")
            return False
        ...
        if not tenant.is_active:
            self.log_tenant_access(
                tenant_id, action, resource, False, "Tenant inactive")
            return False
        ...
        self.log_tenant_access(
            tenant_id, action, resource, True, "Access granted")
        return True
    ...
    def get_tenant_statistics(self) -> Dict[str, Any]:
        ...
        for tenant in self.tenants.values():
            ...
            stats["tenant_types"][tenant_type] = stats["tenant_types"].get(
                tenant_type, 0) + 1
...
def main():
    ...
    try:
        ...
        enterprise_tenant = adapter.create_tenant(
            "Enterprise Corp", TenantType.ENTERPRISE)
        professional_tenant = adapter.create_tenant(
            "Professional Team", TenantType.PROFESSIONAL)
        standard_tenant = adapter.create_tenant(
            "Standard User", TenantType.STANDARD)

        # Test resource allocation
        adapter.allocate_resources(
            enterprise_tenant, "cpu_cores", "main_process", 4)
        adapter.allocate_resources(
            professional_tenant, "memory_gb", "cache", 8)

        # Test access validation
        print(
            f"Enterprise access to own resources: {adapter.validate_tenant_access(enterprise_tenant, f'tenant_{enterprise_tenant}/data', 'READ')}")
        print(
            f"Professional access to enterprise resources: {adapter.validate_tenant_access(professional_tenant, f'tenant_{enterprise_tenant}/data', 'READ')}")
