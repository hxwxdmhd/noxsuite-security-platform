...
class UnifiedPluginSystem:
    ...
    def __init__(self, plugin_directories: List[str] = None):
        ...
        logger.info(
            "Unified plugin system initialized with enhanced features for Audit 2")
    ...
    def discover_and_load_plugins(self) -> None:
        ...
        try:
            ...
            for directory in self.plugin_directories:
                ...
                for item in os.listdir(directory):
                    if item.endswith('.py') and not item.startswith('__'):
                        ...
                        security_result = self.validate_plugin_security(
                            plugin_path)
                        ...
                        logger.info(
                            f"Discovered plugin: {plugin_name} (Security: {security_result['risk_level']})")
            ...
            logger.info(
                f"Plugin discovery complete: {loaded_count} loaded, {failed_count} failed")
                ...
                if plugin_class:
                    ...
                    if plugin_instance.initialize(config):
                        if plugin_instance.start():
                            ...
                            logger.info(
                                f"Plugin {plugin_name} activated successfully")
                            return True
                        else:
        ...
        try:
            ...
            if plugin_name in self.plugin_instances:
                try:
                    ...
                    logger.error(
                        f"Error stopping plugin instance {plugin_name}: {e}")
                ...
                with open(plugin_path, 'r', encoding='utf-8') as f:
                    ...
                    if '"""' in content:
                        ...
                        for line in docstring_lines:
                            if 'Version:' in line:
                                plugin_info['version'] = line.split(':', 1)[
                                    1].strip()
                            elif 'Description:' in line:
                                plugin_info['description'] = line.split(':', 1)[
                                    1].strip()
            except Exception as e:
    ...
    def validate_plugin_security(self, plugin_path: str) -> Dict[str, Any]:
        ...
        try:
            validation_result = self.security_validator.validate_plugin(
                plugin_path)
    ...
    def load_plugin_with_security(self, plugin_path: str) -> bool:
        ...
        if not security_result['valid']:
            logger.warning(
                f"Plugin {plugin_name} failed security validation: {security_result['violations']}")
            if security_result['risk_level'] == 'HIGH':
                logger.error(
                    f"Plugin {plugin_name} rejected due to high security risk")
                return False
        ...
        try:
            ...
            if success:
                self.lifecycle_manager.set_plugin_state(
                    plugin_name, PluginState.LOADED)
                self.lifecycle_manager.trigger_hooks('after_load', plugin_name)

                # Record metrics
                self.plugin_monitor.record_metric(
                    plugin_name, 'load_success', 1)

                return True
            else:
                self.lifecycle_manager.set_plugin_state(
                    plugin_name, PluginState.ERROR)
                self.plugin_monitor.record_metric(
                    plugin_name, 'load_success', 0)
    ...
    def _load_plugin_from_path(self, plugin_path: str) -> bool:
        ...
        try:
            plugin_name = os.path.basename(
                plugin_path)[:-3]  # Remove .py extension
    ...
    def initialize_plugin_with_monitoring(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            # Record metrics
            self.plugin_monitor.record_metric(
                plugin_name, 'init_time_ms', (end_time - start_time) * 1000)
            self.plugin_monitor.record_metric(
                plugin_name, 'init_success', 1 if success else 0)

            if success:
                self.lifecycle_manager.set_plugin_state(
                    plugin_name, PluginState.INITIALIZED)
                self.lifecycle_manager.trigger_hooks(
                    'after_initialize', plugin_name)
    ...
    def activate_plugin_with_monitoring(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            # Record metrics
            self.plugin_monitor.record_metric(
                plugin_name, 'activate_time_ms', (end_time - start_time) * 1000)
            self.plugin_monitor.record_metric(
                plugin_name, 'activate_success', 1 if success else 0)

            if success:
                self.lifecycle_manager.set_plugin_state(
                    plugin_name, PluginState.ACTIVE)
                self.lifecycle_manager.trigger_hooks(
                    'after_activate', plugin_name)
                self.enabled_plugins.add(plugin_name)
    ...
    def validate_plugin_dependencies(self, plugin_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if plugin_name in self.plugin_instances:
                ...
                # Check dependencies
                dep_status = self.dependency_manager.check_dependencies(
                    plugin_name)
            ...
            logger.error(
                f"Error validating dependencies for {plugin_name}: {e}")
            ...
            try:
                ...
                for module in self.restricted_modules:
                    if f"import {module}" in content or f"from {module}" in content:
                        violations.append(
                            f"Restricted import '{module}' in {python_file}")
            except Exception as e:
            ...
            try:
                ...
                for pattern in self.dangerous_patterns:
                    if pattern in content:
                        violations.append(
                            f"Dangerous pattern '{pattern}' in {python_file}")
            except Exception as e:
                violations.append(f"Error reading {python_file}: {e}")
        ...
        for metric, values in metrics.items():
            ...
            if metric in self.alert_thresholds:
                ...
                if latest_value > threshold:
                    alerts.append(
                        f"{metric} ({latest_value}) exceeds threshold ({threshold})")
    ...
    def trigger_hooks(self, event: str, plugin_name: str, *args, **kwargs):
        ...
        for hook in self.lifecycle_hooks.get(event, []):
            try:
                ...
                logger.error(
                    f"Hook {hook.__name__} failed for {plugin_name}: {e}")
...
def initialize_plugin_system(app=None):
    ...
    if app:
        ...
        def activate_plugin(plugin_name):
            """Activate a plugin"""
            success = plugin_system.activate_plugin_with_monitoring(
                plugin_name)
            return jsonify({
                'success': success,
...
if __name__ == '__main__':
    ...
    print(
        f"API Endpoints: {len(plugin_system.get_api_endpoints())} endpoints available")
