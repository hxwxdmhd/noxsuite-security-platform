...
class PluginRegistryDatabase:
    ...
    def init_database(self):
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    'CREATE INDEX IF NOT EXISTS idx_plugin_logs_plugin_id ON plugin_execution_logs(plugin_id)')
                cursor.execute(
                    'CREATE INDEX IF NOT EXISTS idx_plugin_metrics_plugin_id ON plugin_metrics(plugin_id)')

                conn.commit()
                self.logger.info(
                    "Plugin registry database initialized successfully")

        except Exception as e:
            self.logger.error(
                f"Failed to initialize plugin registry database: {e}")
            raise
    ...
    def register_plugin(self, metadata: PluginMetadata) -> bool:
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    'SELECT id FROM plugins WHERE name = ?', (metadata.name,))
                if cursor.fetchone():
                    self.logger.warning(
                ...
                ''', (
                    ...
                    metadata.python_version, json.dumps(
                        metadata.config_schema),
                    json.dumps(metadata.default_config), json.dumps(
            ...
            self.logger.error(
                f"Failed to register plugin {metadata.name}: {e}")
            return False
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                ''', (
                    ...
                    json.dumps(metadata.config_schema), json.dumps(
                ...
                conn.commit()
                self.logger.info(
                    f"Plugin {metadata.name} updated successfully")
                return True
    ...
    def get_plugins_by_category(self, category: PluginCategory) -> List[PluginMetadata]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get plugins by category {category.value}: {e}")
            return []
    ...
    def get_plugins_by_status(self, status: PluginStatus) -> List[PluginMetadata]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get plugins by status {status.value}: {e}")
            return []
    ...
    def update_plugin_status(self, name: str, status: PluginStatus) -> bool:
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                conn.commit()
                self.logger.info(
                    f"Plugin {name} status updated to {status.value}")
                return True
    ...
    def log_plugin_execution(self, plugin_name: str, execution_id: str,
        ...
        try:
            ...
            self.logger.error(
                f"Failed to log execution for plugin {plugin_name}: {e}")
            return False
    ...
    def get_plugin_statistics(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get statistics for plugin {name}: {e}")
            return None
    ...
    def _row_to_metadata(self, row: Tuple) -> PluginMetadata:
        ...
        return PluginMetadata(
            ...
            created_at=datetime.fromisoformat(
                row[22]) if row[22] else datetime.now(),
            updated_at=datetime.fromisoformat(
                row[23]) if row[23] else datetime.now(),
            last_accessed=datetime.fromisoformat(
                row[24]) if row[24] else datetime.now(),
    ...
    def cleanup_old_logs(self, days: int = 30) -> int:
        ...
        try:
            with pymysql.connect(self.db_path) as conn:
                ...
                self.logger.info(
                    f"Cleaned up {deleted_count} old execution logs")
