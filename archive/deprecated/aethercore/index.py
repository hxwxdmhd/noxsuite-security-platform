...
# Configure logging
logger.add("aethercore/logs/aethercore.log",
    ...
    def _setup_routes(self):
        ...
        async def serve_model(request: ModelServeRequest):
            ...
            if request.model_id not in self.models:
                raise HTTPException(
                    status_code=404, detail="Model not found or not loaded")
            ...
            try:
                ...
                logger.info(
                    f"✅ Served model {request.model_id} in {processing_time:.2f}ms")
                ...
                logger.error(
                    f"❌ Error serving model {request.model_id}: {str(e)}")
        ...
        async def get_metrics():
            ...
            successful_requests = len(
                [r for r in recent_requests if r.get("status") == "success"])
            failed_requests = len(
                [r for r in recent_requests if r.get("status") == "error"])

            # Average response time
            response_times = [r.get("processing_time_ms", 0)
                              for r in recent_requests if r.get("processing_time_ms")]
            avg_response_time = sum(response_times) / \
                len(response_times) if response_times else 0

            # Memory usage
            memory_usage = sum(model.get("memory_mb", 0)
