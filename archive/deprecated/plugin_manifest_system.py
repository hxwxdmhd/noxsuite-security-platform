...
class PluginManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'PluginManifest':
        ...
        if 'dependencies' in data:
            data['dependencies'] = [
                PluginDependency.from_dict(
                    dep) if isinstance(dep, dict) else dep
                for dep in data['dependencies']
            ]

        if 'permissions' in data:
            data['permissions'] = [
                PluginPermission.from_dict(
                    perm) if isinstance(perm, dict) else perm
                for perm in data['permissions']
            ]

        if 'config_options' in data:
            data['config_options'] = [
                PluginConfigOption.from_dict(
                    opt) if isinstance(opt, dict) else opt
...
class PluginManifestValidator:
    ...
    def _validate_dependencies(self, manifest_data: Dict[str, Any]) -> List[str]:
        ...
        for dep in dependencies:
            ...
            if version != '*' and not self._is_valid_version_constraint(version):
                errors.append(
                    f"Invalid version constraint for {dep_name}: {version}")
    ...
    def _validate_permissions(self, manifest_data: Dict[str, Any]) -> List[str]:
        ...
        for perm in permissions:
            ...
            if risk_level in ['HIGH', 'CRITICAL'] and not required:
                errors.append(
                    f"High-risk permission {perm_name} cannot be optional")
        ...
        for option in config_options:
            ...
            if default_value is not None:
                if not self._is_valid_type_value(option_type, default_value):
                    errors.append(
                        f"Invalid default value for {option_name}: {default_value}")
    ...
    def _validate_resource_limits(self, manifest_data: Dict[str, Any]) -> List[str]:
        ...
        if max_execution_time > 300:  # 5 minutes maximum
            errors.append(
                f"Execution time limit too high: {max_execution_time}s")
...
class PluginManifestManager:
    ...
    def load_manifest(self, plugin_dir: str) -> Optional[PluginManifest]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to load manifest from {plugin_dir}: {e}")
            return None
