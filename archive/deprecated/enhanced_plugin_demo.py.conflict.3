...
def main():
    ...
    print(
        f"✅ Security Validator: {'Available' if ps.security_validator else 'Missing'}")
    print(
        f"✅ Dependency Manager: {'Available' if ps.dependency_manager else 'Missing'}")
    print(
        f"✅ Performance Monitor: {'Available' if ps.plugin_monitor else 'Missing'}")
    print(
        f"✅ Lifecycle Manager: {'Available' if ps.lifecycle_manager else 'Missing'}")
    ...
    for plugin_name, plugin_info in all_plugins.items():
        print(
            f"   - {plugin_name}: {plugin_info['status']} (Risk: {plugin_info['risk_level']})")
    ...
    for plugin_name in all_plugins.keys():
        ...
        print(
            f"✅ {plugin_name} dependencies: {deps['valid']} (deps: {len(deps['dependencies'])})")
    ...
    for category, results in compliance.items():
        if category != 'overall_compliance':
            ...
            print(
                f"   - {category.replace('_', ' ').title()}: {status} ({results['score']}/100)")
    ...
    if all_plugins:
        ...
        print(
            f"✅ Plugin {plugin_name} activation: {'Success' if success else 'Failed'}")
...
'''
    ...
    try:
        sandbox_result = ps.sandbox.execute_in_sandbox(
            "test_plugin", test_code)
        print(
