...
async def example_2_performance_optimization():
    ...
    try:
        async with sandbox:
            def high_performance_plugin(environment, dataset_size: int = 10000):
                ...
                for i in range(dataset_size):
                    record = {
                        ...
                        # Some text data
                        "metadata": f"record_{i}_metadata" * 10
                    }
            ...
            print(
                f"⏱️ Processing time: {result['processing_time_seconds']:.3f}s")
...
async def example_3_plugin_chaining():
    ...
    def data_analyzer_plugin(environment, input_data: List[Dict[str, Any]]):
        ...
        # Analyze data
        high_category_count = sum(
            1 for record in input_data if record["category"] == "high")
        low_category_count = sum(
            1 for record in input_data if record["category"] == "low")

        total_processed_value = sum(
            record["processed_value"] for record in input_data)
    ...
    try:
        async with sandbox:
            ...
            print(
                f"  Average Value: {stage3_result['average_processed_value']:.2f}")

            # Pipeline telemetry
            telemetry = sandbox.get_telemetry()
            print(f"  Peak Memory: {telemetry.peak_memory_mb:.2f}MB")
            print(
                f"  Total Time: {telemetry.end_time - telemetry.start_time:.3f}s")
...
async def example_4_dynamic_configuration():
    ...
    for plugin_info in test_plugins:
        ...
        config, limits, permissions = config_manager.get_config_for_plugin(
            metadata)
        ...
        try:
            async with sandbox:
                ...
                print(f"  📈 Peak memory: {telemetry.peak_memory_mb:.2f}MB")
                print(
                    f"  ⏱️ Execution time: {telemetry.end_time - telemetry.start_time:.3f}s")
                print(f"  🔥 Peak CPU: {telemetry.peak_cpu_percent:.1f}%")
...
async def example_5_environment_customization():
    ...
    try:
        async with sandbox:
            ...
            print(
                f"🔒 Environment isolated: {env_info['environment_isolated']}")
