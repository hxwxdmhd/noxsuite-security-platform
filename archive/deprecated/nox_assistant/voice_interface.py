...
class VoiceInterface:
    ...
    def __init__(self, wake_words: list = None, voice_settings: Dict[str, Any] = None):
        if not VOICE_AVAILABLE:
            raise ImportError(
                "Voice capabilities not available. Install: pip install SpeechRecognition pyttsx3")

        self.wake_words = wake_words or [
            "hey nox", "nox", "assistant", "jarvis"]
    ...
    def setup_tts_engine(self):
        ...
        try:
            ...
            rate = self.voice_settings.get('rate', 180)
    ...
    def interactive_session(self, assistant_callback: Callable):
        ...
        """
        self.speak(
            "Hello! I'm NoxAssistant. Say 'Hey Nox' to get my attention, or 'exit' to quit.")

        try:
            while True:
                # Listen for wake word or direct command
                # Longer timeout for interactive mode
                user_input = self.listen_once(timeout=30)
