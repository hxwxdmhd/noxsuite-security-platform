    ...
    def refresh_available_models(self) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"🤖 Found {len(self.available_models)} available models: {', '.join(self.available_models)}")
...
"""
        ...
        task_prompt = task_specific_prompts.get(
            task_type, task_specific_prompts["general"])
...
def main():
    ...
    # Connection status
    print(
        f"🔗 Ollama Connection: {'✅ Connected' if llm.connected else '❌ Not available'}")
