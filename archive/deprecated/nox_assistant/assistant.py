...
class NoxAssistant:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.task_registry_path = self.project_root / \
...
- Task automation and script execution"""
    ...
    def execute_task(self, task_name: str, user_input: str) -> str:
        ...
        self.log_action("task_execution",
                        f"Executing {task_name}: {user_input}")
    ...
    def run_network_scan(self) -> str:
        ...
        mock_devices = [
            {"name": "🌐 Router", "ip": "192.168.1.1",
                "status": "online", "type": "gateway"},
            {"name": "💻 Desktop PC", "ip": "192.168.1.100",
                "status": "online", "type": "computer"},
            {"name": "📱 Phone", "ip": "192.168.1.150",
                "status": "offline", "type": "mobile"},
            {"name": "📺 Smart TV", "ip": "192.168.1.200",
                "status": "online", "type": "media"},
            {"name": "🖨️ Printer", "ip": "192.168.1.250",
                "status": "offline", "type": "printer"}
        ]

        online_count = sum(
            1 for device in mock_devices if device["status"] == "online")
        total_count = len(mock_devices)

        results.append(
            f"📊 **Summary**: {online_count}/{total_count} devices online")
        results.append("\n📋 **Device List**:")

        for device in mock_devices:
            status_emoji = "🟢" if device["status"] == "online" else "🔴"
            results.append(
                f"  {status_emoji} {device['name']} ({device['ip']}) - {device['status']}")
    ...
    def run_security_analysis(self) -> str:
        ...
        high_risk = sum(
            1 for check in security_checks if check["risk"] == "high")
        medium_risk = sum(
            1 for check in security_checks if check["risk"] == "medium")

        results.append(
            f"🚨 **Risk Summary**: {high_risk} high, {medium_risk} medium risk items")
        results.append("\n📋 **Security Checks**:")

        for check in security_checks:
            risk_emoji = {"high": "🚨", "medium": "⚠️",
                          "low": "✅"}[check["risk"]]
            results.append(
                f"  {risk_emoji} {check['check']}: {check['status']}")
    ...
    def run_performance_analysis(self) -> str:
        ...
        try:
            ...
            results.append(
                f"💿 **Disk Usage**: {disk_percent:.1f}% ({disk.used // 1024**3} GB / {disk.total // 1024**3} GB)")

            # Network interfaces
            net_io = psutil.net_io_counters()
            results.append(
                f"🌐 **Network**: {net_io.bytes_sent // 1024**2} MB sent, {net_io.bytes_recv // 1024**2} MB received")
            ...
            if memory.percent > 85:
                results.append(
                    "  🚨 High memory usage detected - restart or close memory-intensive applications")
            if disk_percent > 90:
                results.append(
                    "  🚨 Low disk space - clean up old files or expand storage")
            ...
            results.append(
                "📦 **Note**: Install 'psutil' for detailed performance monitoring")
    ...
    def get_help_response(self) -> str:
        ...
        if VOICE_AVAILABLE:
            results.append("\n🎤 **Voice Commands**:")
            results.append(
                "  Use 'python main.py --voice' for voice interaction")
    ...
    def get_task_list(self) -> str:
        ...
        for task_name, task_info in self.task_registry.items():
            tasks.append(
                f"• {task_name}: {task_info.get('description', 'No description')}")
        return "\n".join(tasks)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="🤖 NoxAssistant - Intelligent Network Management")
            ...
            try:
                ...
                if voice_input:
                    ...
                    if any(wake in voice_input for wake in ["hey nox", "nox", "assistant"]):
                        ...
                        command = assistant.listen_for_voice_command(
                            timeout=10)
                        if command:
