...
class PluginInterface(ABC):
    ...
    def enable(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to enable plugin {self.manifest.id}: {e}")
            return False
    ...
    def disable(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to disable plugin {self.manifest.id}: {e}")
            return False
...
class SuiteAPI:
    ...
    def register_endpoint(self, path: str, method: str, handler: Callable, plugin_id: str):
        ...
        logger.info(
            f"Registered endpoint {method} {path} for plugin {plugin_id}")
    ...
    def register_ui_component(self, component_id: str, component_data: Dict, plugin_id: str):
        ...
        logger.info(
            f"Registered UI component {component_id} for plugin {plugin_id}")
    ...
    def validate_plugin(self, plugin_path: Path) -> bool:
        ...
        try:
            ...
            for pattern in dangerous_patterns:
                if pattern in code:
                    logger.warning(
                        f"Potentially dangerous code found: {pattern}")
                    return False
...
class PluginManager:
    ...
    def __init__(self, plugins_dir: str = "plugins", suite_api: SuiteAPI = None):
        ...
        logger.info(
            f"Plugin manager initialized with directory: {self.plugins_dir}")
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if manifest.sandbox_enabled and not self.security_sandbox.validate_plugin(temp_path):
                    logger.error(
                        f"Plugin {manifest.id} failed security validation")
                    return False
