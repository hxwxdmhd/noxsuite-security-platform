...
class AuditGate1:
    ...
    def test_webpanel_structure(self) -> Tuple[bool, str]:
        ...
        missing = [f for f in required_files if not (
            webpanel_dir / f).exists()]
        ...
        if compose_file.exists():
            ...
            for pattern in forbidden_patterns:
                if pattern in content:
                    violations.append(
                        f"docker-compose.yml contains '{pattern}'")
    ...
    def determine_status(self):
        ...
        if self.results["score"] == self.results["max_score"]:
            ...
            # Even numbered gates trigger unlocks
            if self.results["audit_gate"] % 2 == 0:
            ...
            self.results["recommendations"].append(
                "⚠️ Score ≥80 but <100. Fix remaining issues.")
        else:
    ...
    def run_full_audit(self) -> Dict[str, Any]:
        ...
        self.log(
            f"📊 Score: {self.results['score']}/{self.results['max_score']}")
...
def main():
    ...
    parser.add_argument(
        "--output", default="audit_results_1.json", help="Output file name")
