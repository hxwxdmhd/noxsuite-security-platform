...
class RestrictedImporter:
    ...
    def restricted_import(self, name, globals=None, locals=None, fromlist=(), level=0):
        ...
        if base_module not in self.allowed_modules:
            raise ImportError(
                f"Module '{name}' is not allowed in plugin sandbox")
...
class ResourceMonitor:
    ...
    def check_limits(self):
        ...
        if time.time() - self.start_time > self.limits.max_execution_time_seconds:
            raise RuntimeError(
                f"Plugin execution time exceeded {self.limits.max_execution_time_seconds}s")
        ...
        if memory_used > self.limits.max_memory_mb:
            raise RuntimeError(
                f"Plugin memory usage exceeded {self.limits.max_memory_mb}MB")
        ...
        if self.file_operations > self.limits.max_file_operations:
            raise RuntimeError(
                f"Plugin file operations exceeded {self.limits.max_file_operations}")
    ...
    def record_network_connection(self):
        ...
        if self.network_connections > self.limits.max_network_connections:
            raise RuntimeError(
                f"Plugin network connections exceeded {self.limits.max_network_connections}")
...
class SecurePluginLoader:
    ...
    def load_plugin_metadata(self, plugin_dir: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to load plugin metadata from {plugin_dir}: {e}")
            return None
    ...
    def load_plugin_secure(self, plugin_name: str, plugin_dir: Path) -> bool:
        ...
        try:
            ...
            if not metadata:
                logger.error(
                    f"Failed to load metadata for plugin {plugin_name}")
                return False
            ...
            with sandbox:
                result = sandbox.execute_plugin(
                    str(main_file), metadata.get('config', {}))
