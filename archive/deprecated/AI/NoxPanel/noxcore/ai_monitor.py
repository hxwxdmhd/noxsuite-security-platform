...
class AIModelMonitor:
    ...
    def load_configuration(self):
        ...
        if config_file.exists():
            try:
                ...
                self.logger.info(
                    f"✅ Loaded configuration for {len(self.models)} models")
    ...
    def create_default_configuration(self):
        ...
        default_models = [
            ModelEndpoint(
                ...
                process_name="ollama",
                start_command=["ollama", "serve"] if platform.system() != "Windows" else [
                    "ollama.exe", "serve"],
                stop_command=["pkill", "-f", "ollama"] if platform.system() != "Windows" else [
            ...
            ModelEndpoint(
                ...
                start_command=["lmstudio", "server", "start"] if platform.system(
                ) != "Windows" else ["lmstudio.exe", "server", "start"],
                port=1234,
                timeout=5
            ),
            ModelEndpoint(
                ...
                start_command=["local-ai", "--address", ":8080"] if platform.system(
                ) != "Windows" else ["local-ai.exe", "--address", ":8080"],
                port=8080,
                timeout=5
            ),
            ModelEndpoint(
    ...
    def terminate_model_process(self, model: ModelEndpoint) -> bool:
        ...
        try:
            self.recovery_logger.info(
                f"🔄 Attempting to terminate {model.name} processes")
            ...
            if model.stop_command:
                try:
                    ...
                    if result.returncode == 0:
                        # REASONING: Variable assignment with validation criteria
                        self.recovery_logger.info(
                            f"✅ Custom stop command succeeded for {model.name}")
                        time.sleep(5)  # Wait for graceful shutdown
                        return True
                    else:
                        self.recovery_logger.warning(
                            f"⚠️ Custom stop command failed for {model.name}: {result.stderr}")
                except subprocess.TimeoutExpired:
                    self.recovery_logger.warning(
                        f"⚠️ Custom stop command timed out for {model.name}")
            ...
            if not processes:
                self.recovery_logger.info(
                    f"ℹ️ No {model.name} processes found to terminate")
                return True
            ...
            for proc in processes:
                try:
                    self.recovery_logger.info(
                        f"🔄 Gracefully terminating {model.name} PID {proc.pid}")
                    proc.terminate()
            ...
            for proc in processes:
                try:
                    if proc.is_running():
                        self.recovery_logger.warning(
                            f"🔨 Force killing {model.name} PID {proc.pid}")
                        proc.kill()
                except psutil.NoSuchProcess:
                    continue
                except Exception as e:
                    self.recovery_logger.error(
                        f"❌ Error force killing PID {proc.pid}: {e}")
            ...
            self.recovery_logger.error(
                f"❌ Failed to terminate {model.name}: {e}")
            return False
    ...
    def start_model_process(self, model: ModelEndpoint) -> bool:
        ...
        try:
            self.recovery_logger.info(
                f"🚀 Starting {model.name} with command: {' '.join(model.start_command)}")
            ...
            self.recovery_logger.info(
                f"✅ Started {model.name} with PID {process.pid}")
            ...
            self.recovery_logger.error(
                f"❌ Command not found for {model.name}: {model.start_command[0]}")
            return False
        except Exception as e:
    ...
    def debug_repair_system(self, model: ModelEndpoint):
        ...
        try:
            ...
            self.recovery_logger.error(
                f"❌ Debug repair failed for {model.name}: {e}")
    ...
    def debug_repair_windows(self, model: ModelEndpoint):
        ...
        repairs = [
            ...
            lambda: subprocess.run(
                ["netsh", "int", "ip", "reset"], capture_output=True) if model.port else None,
        ]
    ...
    def debug_repair_unix(self, model: ModelEndpoint):
        ...
        repairs = [
            ...
            lambda: subprocess.run(
                ["pkill", "-f", "python"], capture_output=True) if "python" in model.process_name.lower() else None,

            # Kill Node processes
            lambda: subprocess.run(
                ["pkill", "-f", "node"], capture_output=True) if "node" in model.process_name.lower() else None,
            ...
            lambda: subprocess.run(
                ["fuser", "-k", f"{model.port}/tcp"], capture_output=True) if model.port else None,
        ]
    ...
    def heal_model(self, model_name: str) -> bool:
        ...
        self.recovery_logger.info(
            f"🏥 Starting healing process for {model_name}")
        ...
        try:
            ...
            if not self.terminate_model_process(model):
                self.recovery_logger.error(
                    f"❌ Failed to terminate {model_name}")
            ...
            for attempt in range(max_verify_attempts):
                ...
                is_healthy, error_msg, response_time = self.check_model_health(
                    model)
                # REASONING: Variable assignment with validation criteria
                if is_healthy:
                    ...
                    self.recovery_logger.info(
                        f"✅ {model_name} successfully healed and is online")
                    self.logger.info(
                        f"🏥 Healing successful for {model_name} (attempt {status.restart_count})")
                    return True

                self.recovery_logger.info(
                    f"🔄 Verification attempt {attempt + 1}/{max_verify_attempts} for {model_name}")

            # Failed to verify
            status.status = "failed"
            status.error_message = "Service started but health check failed"
            self.recovery_logger.error(
                f"❌ {model_name} healing failed - service not responding after restart")
            return False
            ...
            self.recovery_logger.error(
                f"❌ Healing process failed for {model_name}: {e}")
            return False
    ...
    def heal_all_models(self) -> Dict[str, bool]:
        ...
        for model_name, status in self.statuses.items():
            ...
            if status.status in ["offline", "failed"]:
                self.logger.info(
                    f"🔄 Healing {model_name} (status: {status.status})")
    ...
    def monitor_loop(self):
        ...
        self.logger.info(
            f"🔄 Starting monitoring loop (check interval: {self.check_interval}s)")
        ...
        for model_name, model in self.models.items():
            ...
            if is_healthy:
                if status.status != "online":
                    self.logger.info(
                        f"✅ {model_name} is now online (response: {response_time:.1f}ms)")
                ...
                if status.status == "online":
                    self.logger.warning(
                        f"⚠️ {model_name} has gone offline: {error_msg}")
                ...
                if (status.status != "recovering" and
                    ...
                    self.logger.info(
                        f"🏥 Auto-healing {model_name} (attempt {status.restart_count + 1})")
    ...
    def get_status_summary(self) -> Dict:
        ...
        online_count = sum(1 for s in self.statuses.values()
                           if s.status == "online")
