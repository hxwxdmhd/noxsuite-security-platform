...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(current_dir / "data" /
                            "logs" / "noxpanel_launcher.log"),
...
class NoxPanelLauncher:
    ...
    def run_comprehensive_checks(self):
        ...
        logger.info(
            f"✅ System checks completed: {passed_checks}/{total_checks} passed")
    ...
    def launch_all_components(self):
        ...
        self.system_status['components_running'] = len(
            [c for c in self.components.values() if c['status'] == 'running'])
    ...
    def _launch_component(self, component_id: str, component: Dict[str, Any]):
        ...
        try:
            ...
            if process.poll() is None:
                ...
                logger.info(
                    f"✅ {component['name']} launched successfully on port {component['port']}")
            else:
                component['status'] = 'error'
    ...
    def _perform_final_health_check(self):
        ...
        if health_percentage >= 80:
            ...
            logger.info(
                f"🎉 System is healthy! ({health_percentage:.1f}% components online)")
        elif health_percentage >= 50:
            self.system_status['health_status'] = 'warning'
            logger.warning(
                f"⚠️  System has warnings ({health_percentage:.1f}% components online)")
        else:
            self.system_status['health_status'] = 'critical'
            logger.error(
                f"❌ System is in critical state ({health_percentage:.1f}% components online)")
    ...
    def _print_launch_summary(self):
        ...
        logger.info(
            f"📊 System Status: {self.system_status['health_status'].upper()}")
        logger.info(
            f"⏰ Launch Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        logger.info(
            f"🔢 Components Running: {self.system_status['components_running']}/{self.system_status['total_components']}")

        logger.info("\n📋 Component Status:")
        for component_id, component in self.components.items():
            status_icon = "✅" if component['status'] == 'running' else "❌" if component['status'] == 'error' else "⏳"
            logger.info(
                f"  {status_icon} {component['name']}: {component['status'].upper()}")
            if component['status'] == 'running':
        ...
        if self.components['enhanced_gateway']['status'] == 'running':
            logger.info(
                f"  🎛️  Main Gateway: {self.components['enhanced_gateway']['url']}")
            logger.info(
                f"     → Unified platform management with health monitoring")
        ...
        if self.components['noxpanel_v5']['status'] == 'running':
            logger.info(
                f"  🤖 AI Panel: {self.components['noxpanel_v5']['url']}")
        ...
        if self.components['admin_panel']['status'] == 'running':
            logger.info(
                f"  ⚙️  Admin Panel: {self.components['admin_panel']['url']}")
        ...
        logger.info(
            "  ✅ Visual platform switcher (Heimnetz/AI Panel/Media Center/Security Hub/Tools)")
        logger.info("  ✅ Enhanced admin panel with role-based access")
        logger.info(
            "  ✅ Real-time system metrics (CPU, RAM, uptime, model status)")
        ...
        if self.system_status['health_status'] == 'healthy':
            logger.info(
                "  1. Open the Enhanced Gateway Platform in your browser")
            logger.info(
                "  2. Explore the platform switcher and health monitoring")
                    ...
                    for component_id, component in self.components.items():
                        if component['status'] == 'running':
                            ...
                            if process and process.poll() is not None:
                                logger.warning(
                                    f"⚠️  {component['name']} has stopped unexpectedly")
                                component['status'] = 'stopped'

                                # Attempt restart if required component
                                if component['required']:
                                    logger.info(
                                        f"🔄 Attempting to restart {component['name']}...")
                                    self._launch_component(
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    if result == 0:
                        # REASONING: Variable assignment with validation criteria
                        logger.warning(
                            f"Port {component['port']} is already in use")
                        return False
            except Exception:
                pass
...
def main():
    ...
    try:
        ...
        logger.info(
            "🎯 NoxPanel Launcher is running. Press Ctrl+C to stop all components.")
