...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/noxpanel_test.log',
...
def knowledge_search():
    ...
    dangerous_patterns = ['union select', 'drop table',
...
if __name__ == '__main__':
    ...
    try:
        ...
        if conn:
            ...
            count = conn.execute(
                'SELECT COUNT(*) as count FROM knowledge_items').fetchone()['count']

            if count == 0:
                # Add minimal sample data
                sample_data = [
                    ('Python Flask Tutorial', 'Flask web framework basics',
                     'documentation', 'python', '["python", "flask", "web"]', 1),
                    ('SQL Query Examples', 'Common SQL patterns',
                     'code', 'sql', '["sql", "database", "queries"]', 1),
                    ('Docker Commands', 'Useful Docker commands', 'script',
                     'bash', '["docker", "containers", "devops"]', 0),
                    ('JavaScript Async/Await', 'Modern async programming', 'code',
                     'javascript', '["javascript", "async", "promises"]', 0),
                    ('Git Workflow', 'Git branching strategy', 'documentation',
                     '', '["git", "workflow", "version-control"]', 0)
                ]
