...
def reset_admin_password():
    ...
    try:
        # Import models
        from mariadb_dev_setup import User, MariaDBDevSetup
        ...
        new_hash = bcrypt.hashpw("Admin123!".encode('utf-8'), 
        ...
        # Commit changes
        db.commit()
        logger.info(f"✅ Admin password reset for {admin.username} ({admin.email})")
        ...
        if not user:
            ...
            db.add(user)
            db.commit()
            logger.info(f"✅ Created user account: {user.username} ({user.email})")
