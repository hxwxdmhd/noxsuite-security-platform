        ...
        try:
            ...
            if "from emergency_auth_middleware import" not in content:
                ...
                for i, line in enumerate(lines):
                    if line.startswith("from flask import") or line.startswith(
                        "import flask"
                    ):
            ...
            while i < len(lines):
                ...
                if "@app.route(" in line and "/api/knowledge" in line:
                    ...
                    patched_lines.append(
                        f"# 🚨 EMERGENCY SECURITY PATCH - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                    )

                    logger.info(f"🔒 Patched route: {line.strip()}")

                elif "def " in line and any(
                    endpoint in line
                    for endpoint in ["knowledge", "search", "suggestions"]
                ):
                    # Function definition - add comment
                    patched_lines.append(
                        f"# 🚨 EMERGENCY AUTH PATCH APPLIED - {datetime.now().strftime('%Y-%m-%d')}"
                    )
...
"""
                        ...
                        self.patches_applied.append(
                            f"Integrated emergency auth in {app_file.name}"
                        )
                        logger.info(
                            f"✅ Emergency integration added to {app_file}")

                except Exception as e:
                    logger.error(
                        f"❌ Failed to integrate emergency auth in {app_file}: {e}"
                    )
    ...
    def apply_all_patches(self):
        ...
        if not knowledge_files:
            logger.warning(
                "⚠️ No knowledge API files found - creating emergency endpoint protection"
            )
        ...
        logger.info(
            f"✅ Emergency security patches completed: {len(self.patches_applied)} files modified"
        )
