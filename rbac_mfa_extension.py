...
try:
    from noxsuite_fastapi_server import app, get_db, get_current_user, User, Role, UserRole, UserSession, AuditLog, SessionLocal, Token
    from fastapi import Depends, HTTPException, status, Request, Body
    from fastapi.responses import JSONResponse
    from pydantic import BaseModel, Field
    from typing import List, Optional
except ImportError:
    print("Error: Cannot import FastAPI server. Make sure noxsuite_fastapi_server.py exists.")
    sys.exit(1)
...
async def verify_mfa_challenge(
    ...
    from noxsuite_fastapi_server import create_access_token, create_refresh_token, Token, log_audit_event
    import secrets
    ...
    try:
        ...
        if not user:
            log_audit_event(db, None, "mfa_verification_failed", "auth", 
                           {"reason": "user_not_found", "session_id": session_id}, request)
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        log_audit_event(db, None, "mfa_verification_failed", "auth", 
                       {"reason": "invalid_session", "session_id": session_id}, request)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid MFA session"
        )
    ...
    if not verify_totp(user.mfa_secret, mfa_verification.code):
        log_audit_event(db, user.id, "mfa_verification_failed", "auth", 
                       {"reason": "invalid_code"}, request)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        if UserSession:
            ...
            session_token = f"{session_unique_id}_{user.id}_{int(datetime.utcnow().timestamp())}"
        ...
        log_audit_event(db, user.id, "mfa_verification_success", "auth", {}, request)
    except Exception as e:
...
async def enable_mfa(
    ...
    if current_user.mfa_secret != mfa_enable.secret:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="MFA secret does not match"
        )
