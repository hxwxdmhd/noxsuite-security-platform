...
class BruteForceProtector:
    ...
    def record_failed_login(self, username: str, ip_address: str) -> Dict[str, Any]:
        ...
        ip_failed_attempts = sum(
            1
            for user, count in self.failed_logins.items()
    ...
    def cleanup_expired_blocks(self) -> Dict[str, int]:
        ...
        expired_accounts = [
            username
            for username, lock_time in self.locked_accounts.items()
            if now >= lock_time
        ]
        ...
        expired_ips = [
            ip for ip, block_time in self.blocked_ips.items() if now >= block_time
        ]
