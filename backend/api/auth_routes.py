...
@router.post(
    "/mfa/verify", response_model=TokenResponse, status_code=status.HTTP_200_OK
)
...
@router.post(
    "/mfa/setup", response_model=MFASetupResponse, status_code=status.HTTP_200_OK
)
...
async def complete_mfa_setup(
    ...
    result = auth_service.complete_mfa_setup(
        request.user_id, request.secret, request.totp_code
    )
...
@router.post(
    "/mfa/disable", response_model=MessageResponse, status_code=status.HTTP_200_OK
)
