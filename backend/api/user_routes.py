...
async def list_users(
    ...
    user: Dict[str, Any] = Depends(auth_middleware.require_permission(Permission.USER_VIEW))
):
    """
...
async def create_user(
    ...
    current_user: Dict[str, Any] = Depends(auth_middleware.require_permission(Permission.USER_CREATE))
):
    """
    Create a new user
...
async def get_user(
    ...
    if current_user["user_id"] != user_id and "admin" not in current_user["roles"]:
        if not auth_service.rbac_service.has_permission(current_user["user_id"], Permission.ADMIN_USERS):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def update_user(
    ...
    user_id: str = Path(...),
    current_user: Dict[str, Any] = Depends(auth_middleware.require_permission(Permission.USER_EDIT))
):
    """
...
async def delete_user(
    user_id: str = Path(...),
    current_user: Dict[str, Any] = Depends(auth_middleware.require_permission(Permission.USER_DELETE))
):
    """
    Delete a user
...
async def assign_roles(
    ...
    user_id: str = Path(...),
    current_user: Dict[str, Any] = Depends(auth_middleware.require_permission(Permission.ADMIN_USERS))
):
    """
    ...
    user_roles = [role.name for role in auth_service.rbac_service.get_user_roles(user_id)]
    user_permissions = [p.value for p in auth_service.rbac_service.get_user_permissions(user_id)]
...
async def remove_role(
    ...
    if user_id == current_user["user_id"] and role_name == "admin" and "admin" in current_user["roles"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
