...
def run_command(command, cwd=None):
    ...
    try:
        ...
        result = subprocess.run(
            command.split(), cwd=cwd, capture_output=True, text=True, timeout=300
        )
...
def quick_start():
    ...
    if not run_command(
        "pip install flask flask-cors flask-socketio psutil", cwd=project_root
    ):
...
""".strip()
    ...
    logger.info(
        "📊 API docs will be available at: http://localhost:5000/api/health")
    ...
    try:
        ...
        process = subprocess.Popen(
            [sys.executable, "app.py"], cwd=project_root)
