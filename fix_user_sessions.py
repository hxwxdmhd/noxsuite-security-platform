...
try:
    from backend.models.user import User, UserSession, Base, engine, SessionLocal
except ImportError:
    print("Error: Cannot import database models. Make sure you have run setup_database_simple.py first.")
    sys.exit(1)
...
def fix_user_sessions():
    ...
    try:
        ...
        duplicates = {token: sessions for token, sessions in sessions_by_token.items() 
                      if len(sessions) > 1}
        ...
        for token, dup_sessions in duplicates.items():
            print(f"Fixing {len(dup_sessions)} duplicate sessions for token: {token[:10]}...")
            ...
            for i, session in enumerate(dup_sessions[1:], 1):
                ...
                print(f"  Session {session.id}: {token[:10]} -> {new_token[:10]}")
