...
def init_auth_settings():
    ...
    if os.path.exists("rbac_config.json"):
        ...
        subprocess.run([sys.executable, "init_auth.py", "--import-config", "rbac_config.json"])
    else:
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Run NoxSuite MFA and RBAC implementation")
    ...
    # Init parser
    init_parser = subparsers.add_parser("init", help="Initialize auth settings")
    
    # Test parser
    test_parser = subparsers.add_parser("test", help="Run tests")
    test_parser.add_argument("--type", type=str, choices=["mfa", "rbac"], help="Test type to run")
    
    # Docker parser
    docker_parser = subparsers.add_parser("docker", help="Run with Docker Compose")
