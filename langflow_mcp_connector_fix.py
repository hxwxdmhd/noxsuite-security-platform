            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.langflow_url}/health") as resp:
                    if resp.status == 200:
                        ...
                        logger.info(f"   ❌ Langflow health failed: {resp.status}")
                        return False
                
                # Check API endpoints
                async with session.get(f"{self.langflow_url}/api/v1/auto_login") as resp:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if resp.status in [200, 201]:
                        ...
                        logger.info(f"   ⚠️ Project creation status: {resp.status}")
