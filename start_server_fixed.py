    ...
    def verify_imports(self):
        ...
        try:
            ...
            if register_func and callable(register_func):
                logger.info(
                    "✅ RBAC/MFA extension with register function verified")
            else:
                logger.error(
                    "❌ RBAC/MFA extension missing or missing register function"
                )
                return False
        ...
        try:
            with open(log_file, "w") as log:
                ...
                if self.server_process.poll() is None:
                    ...
                    logger.error(
                        f"❌ FastAPI server failed to start (exit code: {self.server_process.returncode})"
                    )
    ...
    def monitor_server(self):
        ...
        while self.keep_running:
            if self.server_process and self.server_process.poll() is not None:
                logger.warning(
                    f"Server terminated (exit code: {self.server_process.returncode})"
                )
