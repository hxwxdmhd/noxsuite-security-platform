        ...
        try:
            ...
            try:
                with urllib.request.urlopen(health_url, timeout=5) as response:
                    if response.getcode() == 200:
                        ...
                        logger.info(f"   ❌ Langflow health failed: {response.getcode()}")
                        return False
            ...
            try:
                ...
                with urllib.request.urlopen(api_url, timeout=5) as response:
                    logger.info(f"   📡 API endpoint status: {response.getcode()}")
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=5) as response:
                    ...
                    if 'text/event-stream' not in content_type:
                        logger.info("   ❌ SSE endpoint not configured properly")
                        return False
                    else:
            ...
            try:
                with urllib.request.urlopen(req, timeout=10) as response:
                    if response.getcode() in [200, 201]:
                        ...
                        logger.info(f"   ✅ Project created/verified: {result.get('id', 'unknown')}")
                    else:
                        logger.info(f"   ⚠️ Project creation status: {response.getcode()}")
            ...
            try:
                with urllib.request.urlopen(config_req, timeout=10) as response:
                    if response.getcode() in [200, 201]:
                        ...
                        logger.info(f"   ⚠️ MCP config status: {response.getcode()}")
            except urllib.error.HTTPError as e:
            ...
            try:
                with urllib.request.urlopen(req, timeout=5) as response:
                    ...
                    if 'text/event-stream' in content_type:
                        ...
                        logger.info(f"   ❌ SSE still returning: {content_type}")
                        return False
...
def main():
    ...
    logger.info(f"   MCP Connection: {'✅ OK' if connection_ok else '❌ Issues'}")
