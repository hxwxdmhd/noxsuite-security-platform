...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('final_production_deployment.log', encoding='utf-8'),
...
class FinalProductionEngine:
    ...
    def __init__(self):
        ...
        self.monitoring_services = {
            "prometheus": {"port": 9091, "container_name": "noxguard-production-prometheus"},
    ...
    def cleanup_existing_containers(self) -> bool:
        ...
        try:
            ...
            all_containers = list(self.services.values()) + list(self.monitoring_services.values())
            ...
            try:
                subprocess.run(
                    ["docker-compose", "-f", "docker-compose-final.yml", "down", "--remove-orphans"],
...
"""
            ...
            with open(dashboard_dir / "Dockerfile", 'w') as f:
                f.write(dockerfile.replace("8080", "8081").replace("noxuser", "dashuser"))
                ...
                "scrape_configs": [
                    {
                        ...
                        "static_configs": [{"targets": ["noxguard-production-api:8080"]}],
                        "metrics_path": "/health",
                        "scrape_interval": "30s"
                    },
                    {
                        "job_name": "noxguard-dashboard",
                        "static_configs": [{"targets": ["noxguard-production-dashboard:8081"]}],
                        "metrics_path": "/status",
                        "scrape_interval": "30s"
                    }
                ]
    ...
    def deploy_final_stack(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker-compose", "-f", "docker-compose-final.yml", "up", "-d", "--build", "--remove-orphans"],
                cwd=self.base_dir,
                capture_output=True,
                text=True,
                timeout=400
            ...
            if result.returncode == 0:
                ...
                logger.error(f"FAILED: Deployment error: {result.stderr[:500]}")
                return False
            ...
            for attempt in range(3):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            data = response.json()
                            result["health_score"] = data.get("health_score", 90)
                        except:
                    ...
                    if attempt == 2:  # Last attempt
                        ...
                        logger.warning(f"Service {service_name} unreachable after 3 attempts: {e}")
                    else:
    ...
    def run_final_validation(self) -> dict:
        ...
        for service_name, config in self.services.items():
            health_result = self.validate_production_health(service_name, config)
        ...
        for service_name, config in self.monitoring_services.items():
            try:
                response = requests.get(f"http://localhost:{config['port']}", timeout=15)
                ...
                if is_healthy:
                    total_score += 50
                logger.info(f"Monitoring {service_name}: {'HEALTHY' if is_healthy else 'UNHEALTHY'}")
            except Exception as e:
                validation_results["monitoring"][service_name] = {
                    "status": "UNREACHABLE",
                    "accessible": False
        ...
        validation_results["production_ready"] = validation_results["overall_health"] >= self.health_target
        
        self.system_health = validation_results["overall_health"]
        
        logger.info(f"SUCCESS: Final validation complete. Health: {self.system_health:.1f}%")
    ...
    def generate_final_report(self) -> str:
        ...
        try:
            ...
            deployment_duration = (datetime.now() - self.deployment_start).total_seconds() / 60
            ...
            report = {
                "final_production_deployment_report": {
                    ...
                    "production_services": {
                        ...
                        "total_deployed": len(self.services) + len(self.monitoring_services)
                    },
                    "access_points": {
                    ...
                    "production_readiness": "PRODUCTION_READY" if self.system_health >= self.health_target else "CONDITIONAL",
                    "final_recommendation": "DEPLOY_TO_PRODUCTION" if self.system_health >= self.health_target else "REVIEW_REQUIRED"
                }
            }
            ...
            with open(summary_path, 'w') as f:
                ...
                f.write(f"============================================\n")
                f.write(f"Deployment Time: {self.deployment_start.isoformat()}\n")
                ...
                f.write(f"Final Recommendation: {'PRODUCTION_DEPLOYMENT_VALIDATED' if self.system_health >= self.health_target else 'NEEDS_REVIEW'}\n")
    ...
    def run_final_deployment(self) -> dict:
        ...
        try:
            ...
            final_results = {
                ...
                "recommendation": "PRODUCTION_DEPLOYMENT_VALIDATED" if self.system_health >= self.health_target else "REVIEW_REQUIRED"
            }
...
def main():
    ...
    print(f"Services: {results.get('services_deployed', 0)} + {results.get('monitoring_deployed', 0)} monitoring")
