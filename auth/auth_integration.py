...
from auth.rbac_service import RBACService, Permission, Role
...
class AuthIntegrationService:
    ...
    def __init__(self, 
        ...
        self.active_sessions: Dict[str, Dict[str, Any]] = {}  # user_id -> session info
    ...
    def _generate_auth_tokens(self, user_id: str) -> Dict[str, Any]:
        ...
        roles = [role.name for role in self.rbac_service.get_user_roles(user_id)]
        permissions = [p.value for p in self.rbac_service.get_user_permissions(user_id)]
    ...
    def refresh_token(self, refresh_token: str) -> Dict[str, Any]:
        ...
        if user_id not in self.active_sessions or self.active_sessions[user_id].get("refresh_token") != refresh_token:
            return {
                "status": AuthStatus.FAILED,
    ...
    def verify_token(self, access_token: str) -> Dict[str, Any]:
        ...
        if user_id not in self.active_sessions or self.active_sessions[user_id].get("access_token") != access_token:
            return {
    ...
    def setup_mfa(self, user_id: str) -> Dict[str, Any]:
        ...
        uri = self.totp_service.get_provisioning_uri(user_id, secret, "NoxSuite")
    ...
    def complete_mfa_setup(self, user_id: str, secret: str, totp_code: str) -> Dict[str, Any]:
        """
    ...
    def disable_mfa(self, user_id: str, verification_code: str) -> Dict[str, Any]:
        ...
        if not (self.totp_service.verify_totp(user_id, verification_code) or 
                self.totp_service.verify_backup_code(user_id, verification_code)):
            return {
                "status": AuthStatus.FAILED,
...
if __name__ == "__main__":
    ...
    if auth_result["status"] == AuthStatus.MFA_REQUIRED:
        ...
        mfa_result = auth_service.verify_mfa(user_id, "123456")  # Simulated TOTP code
    ...
    if mfa_setup["status"] == AuthStatus.SUCCESS:
        ...
        complete_setup = auth_service.complete_mfa_setup(user_id, secret, "123456")  # Simulated TOTP
        ...
        # Disable MFA
        disable_result = auth_service.disable_mfa(user_id, "123456")  # Simulated TOTP
