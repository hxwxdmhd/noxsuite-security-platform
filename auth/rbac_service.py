...
if __name__ == "__main__":
    ...
    for role_name, role in rbac_service.roles.items():
        ...
        print(
            f"    Permissions: {', '.join(sorted(p.value for p in role.permissions))}"
        )
    ...
    print("\nPermission Checks:")
    print(
        f"user1 (admin) has ADMIN_ACCESS: {rbac_service.has_permission('user1', Permission.ADMIN_ACCESS)}"
    )
    print(
        f"user2 (user) has USER_VIEW: {rbac_service.has_permission('user2', Permission.USER_VIEW)}"
    )
    print(
        f"user2 (user) has ADMIN_ACCESS: {rbac_service.has_permission('user2', Permission.ADMIN_ACCESS)}"
    )
    print(
        f"user3 (moderator) has USER_EDIT: {rbac_service.has_permission('user3', Permission.USER_EDIT)}"
    )
    ...
    print(
        f"user1 (admin) has ADMIN_ACCESS after import: {new_rbac.has_permission('user1', Permission.ADMIN_ACCESS)}"
    )
